...
namespace Talent.Web.TalentClass
{
    public partial class TalentClassCx : System.Web.UI.Page
    {
        ...
        public static string GetTalentClassList(int pagesize, int index, string querystr, int total)
        {
            ...
            List<Model.TB_RC_TalentClass> list = tcClass.GetModelList(pagesize, index, querystr, itempager);
            foreach (var apply in list)
            {
                sb.Append("<tr target='sid_class' rel='" + apply.TC_No + "'>");
                sb.Append("<td><input name='ids' value='" + apply.TC_No + "' type='checkbox'></td>");
                sb.Append("<td>" + apply.TC_No + "</td>");
                sb.Append("<td>" + apply.TC_Class + "</td>");
                sb.Append("<td><DIV class='colorPicker_controlset'><DIV style='BACKGROUND-COLOR:" + apply.TC_Color+ ";width:16px' class='colorPicker-picker'>&nbsp;</DIV></DIV></td>");
                sb.Append("</tr>");
            }
            return sb.ToString();
        }
        ...
        {
            Model.TB_RC_TalentClass talent = new Model.TB_RC_TalentClass();
            talent.TC_Class = Common.Constants.getReqValue(req, "TC_Class");
            talent.TC_Color = Common.Constants.getReqValue(req, "TC_Color");

            int count = tcClass.GetTotal(" TC_Class='" + talent.TC_Class + "' or TC_Color ='" + talent.TC_Color + "' ");
            if (count > 0) {
                message = "人才类别或颜色已存在";
        ...
        public static StringWriter ReportTalentClass(string queryStr)
        {
            ...
            List<Model.TB_RC_TalentClass> list = tcClass.GetModelList(queryStr);
            foreach (var apply in list)
            {
                sw.Write("=\"" + apply.TC_No + "\"");
                sw.Write(",=\"" + apply.TC_Class + "\"");
                sw.WriteLine(",=\"" + apply.TC_Color + "\"");
            }
            sw.Close();
            return sw;
        }
