...
function initUI(_box){
	...
	// 这里放其他第三方jQuery插件...
	$("a[name='TalentDel']", $p).click(function (event) {
	    TalentInfo.DelRow($(this), event);
     });

     // download
     $("a[name=download]", $p).each(function () {
         $(this).click(function (event) {
             var $this = $(this);
             var $parent = navTab.getCurrentPanel();
             var form = _getPagerForm($parent, {pageNum:0});
             var params = $(form).serialize();
             $this.attr("href", $this.attr("src") + "?" + params);
             return true;
         });
     });

     // 颜色
     $("input[name=TC_Color]", $p).each(function () {
         $('#TC_Color').colorPicker();
     });

     $("#MapContainer", $p).each(function () {
         map.initialize();
     });
