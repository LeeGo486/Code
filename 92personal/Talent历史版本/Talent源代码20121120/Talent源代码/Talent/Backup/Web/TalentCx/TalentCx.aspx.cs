...
namespace Talent.Web.TalentCx
{
    public partial class TalentCx : System.Web.UI.Page
    {
        ...
        public  string GetTalentList(int pagesize, int index, string querystr, int total)
        {
            ...
            foreach (var obj in list)
            {
              ...
              //  sb.Append("</td><td>");
                ...
                {
                    sb.Append("</td><td>");
                    sb.Append("<a href='TalentInfo/TalentDocument.aspx?CB_No=" + obj.TI_CB_No + "&TI_No=" + obj.TI_No + "&status=99' target='dialog' mask=true  rel='ducument_dia' title='' width='400' height='500'>简历" + tddb.GetTotal((obj.TI_CB_No == null ? 0 : obj.TI_CB_No.Value), obj.TI_No) + "封</a>");
                }
                sb.Append("</td></tr>");  
            }
            return sb.ToString();
        ...
        public static StringWriter ReportTalent(string queryStr)
        {
            ...
            int itemindex = 0;
            sw.WriteLine("序号,公司名称,公司所在地,品牌或企业,行业,人才类别,姓名,性别,联系方式,工作经历,来源,推荐人,推荐日期,推荐岗位,特点,是否面试,面试结果,跟进人,维护时间,维护结果");
            foreach (var obj in list)
            {
                itemindex++;
                sw.Write("=\"" + itemindex + "\"");
