...
/// 类名：MDM2_BaseInfo
/// 类说明：获取GP店铺相关信息（店铺信息，店铺营业员信,取GP店铺前缀）无验证
/// 创建人：郭琦琦 
/// 创建日期：2014-11-06 
/// 修改人：
/// 修改日期：
namespace HZY.COM.WS.MDM2
{
    ...
    public class MDM2_BaseInfo : HZYLogicBase, ILogicGetData
    {
        public MDM2_BaseInfo()
        {
        ...
        public DataSet GetData()
        {
            try
            {
                ...
                {
                    //取GP店铺所有信息
                    if (dt_OPTYPE.Rows[i][0].ToString().ToUpper() == "GETDEPOTINFO")
                    {
                        string strSQL = @"
                    ...
                    //取GP店铺所有营业员信息(纯纯2列,编号,名称)
                    else if (dt_OPTYPE.Rows[i][0].ToString().ToUpper() == "GETEMPLOYEEBASE")
                    {
                        string depotid = dt_List.Rows[0]["depotid"].ToString();//店铺编号
