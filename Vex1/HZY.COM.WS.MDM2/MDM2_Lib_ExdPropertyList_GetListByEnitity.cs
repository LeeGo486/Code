...
/// 类名：MDM2_Lib_ExdPropertyList_GetListByEnitity
/// 类说明：获取MDM实体的属性列表
/// 创建人：马卫清
/// 创建日期：2013-09-10
/// 修改人：
...
namespace HZY.COM.WS.MDM2
{
    ...
    /// 获取MDM实体的属性列表
    /// </summary>
    public class MDM2_Lib_ExdPropertyList_GetListByEnitity : LogicBase, ILogicGetData
    {
        public MDM2_Lib_ExdPropertyList_GetListByEnitity()
        {
            this.m_SessionCheck = true;

        }

        private Dbconn conn = new Dbconn("MDM_Master2");
        ...
        private DataSet Get(DataTable dtRequest)
        {
            ...
            DataSet ds = conn.GetDataSetForPageList(@"
                    ...
                    SELECT  t2.MDM_Content 实体名称 ,
                            ...
                            t1.Remark 备注,
                            t1.Timestamps
                    FROM    [dbo].[Lib_ExdPropertyList] t1 With(nolock)
                            LEFT JOIN dbo.Lib_MDM_Config t2 With(nolock) ON t1.MDM_Entity = t2.MDM_Entity
        ...
        private void Delete(DataTable dtRequest)
        {
            ...
            for (int i = 0; i < dtRequest.Rows.Count; i++)
            {
                ...
                strValue[0] = str属性编号;
                DataSet dstemp = conn.GetDataSetBySP("SP_0022_Delete_Property", strParam, strValue);
        ...
        private void Insert(DataTable dtRequest)
        {
            ...
            for (int i = 0; i < dtRequest.Rows.Count; i++)
            {
                ...
                strValue[3] = str字段类型;
                strValue[4] = str备注;
                DataSet dstemp = conn.GetDataSetBySP("SP_0020_Create_Property", strParam, strValue);
                if (dstemp.Tables.Count > 0 && dstemp.Tables[0].Rows.Count > 0)
                {
                    dt_EditResult.Rows.Add(new object[] { false, dstemp.Tables[0].Rows[0][0].ToString() });
                }
        ...
        private void Update(DataTable dtRequest)
        {
            ...
            for (int i = 0; i < dtRequest.Rows.Count; i++)
            {
                ...
                strValue[3] = str字段类型;
                strValue[4] = str备注;
                DataSet dstemp = conn.GetDataSetBySP("SP_0021_Update_Property", strParam, strValue);
                if (dstemp.Tables.Count > 0 && dstemp.Tables[0].Rows.Count > 0)
                {
                    dt_EditResult.Rows.Add(new object[] { false, dstemp.Tables[0].Rows[0][0].ToString() });
                }
