...
/****** Object:  StoredProcedure [API_GLERP].[Sp_PRCD_ReturnDetail_TEST]    Script Date: 09/12/2013 10:35:21 ******/
SET ANSI_NULLS ON
GO
...
CREATE proc [API_GLERP].[Sp_PRCD_ReturnDetail_TEST]
...
-----自动生成	
	...
	DECLARE @PRCD_PRC_ReturnId VARCHAR(50)
	DECLARE @PRCD_LmUser  VARCHAR(50)
...
--设置帮助
	if @InputFormat in ('AML','【,】')  
		begin
		    ...
		    '【《W》POS_退货单号,】【《W》退货数量,】【《W》退货金额,】【《W》退货原因,】【《W》状态,】'+
		    '【《W》创建人,】【《W》创建时间,】【《W》修改人,】【《W》最后修改时间,】' 
	
			set @HelpString=@HelpString+char(13)+char(10)+
			'【操作类型,NEW】【实体,B02_Bill.PRCD_ReturnDetail】【新增内容,必填】'+
			'【《N》退货单编号,】【《N》商品ID,】【《N》单位,】【《N》采购原币价,】【《N》采购本币价,】'+
			'【《N》吊牌价,】【《N》入仓ID,】【《N》储位,】【《N》退货数量,】【《N》POS_退货单号,】'+
			'【《N》状态,】【《N》创建人,】【《N》创建时间,】【《N》修改人,】【《N》最后修改时间,】 ' 
			 
			set @HelpString=@HelpString+char(13)+char(10)+
			'【操作类型,UPDATE】【实体,B02_Bill.PRCD_ReturnDetail】【更新内容,必填】'+
			'【《N》退货单编号,】【《N》商品ID,】【《N》单位,】【《N》采购原币价,】【《N》采购本币价,】'+
			'【《N》吊牌价,】【《N》入仓ID,】【《N》储位,】 【《N》退货数量,】【《N》POS_退货单号,】'+
			'【《N》状态,】【《N》修改人,】' + 
			'【《W》集团,】【《W》事业体,】'+
			...
			set @HelpString=@HelpString+char(13)+char(10)+
			'【操作类型,DELETE】【实体,B02_Bill.PRCD_ReturnDetail】【更新内容,必填】【条件内容,】'+
			'【《N》状态,】【《N》最后修改人,】【《N》修改时间,】'+
 ...
 PRINT @SqlString
		...
		BEGIN
		  ...
		  print @SelectFieldList1
            ...
            LEFT JOIN B02_Bill.PRCD_ReturnDetail         ON PRCD_PR1D_OrderDetailId=PR1D_OrderDetailId  
     ...
     --   set @PRCD_POriginalPrice=isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《N》采购原币价'),'') 
       ...
       /* SELECT  @PRCD_POSRetCode=ISNULL(MAX(PRCD_POSRetCode),''),
		...
		--set @PRA_PerchaseTotal=isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《N》采购总数'),'')
         ...
         set @List=isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《W》退货单明细Get_XML'),'')
 
		exec [~FrameDB].API_XML.SelectXMLAsTable 
		@List
		,'/ROOT/LIST/ROW'
		,2
		,'( PRCD_PD0_ProductId  varchar(50),PRCD_POriginalPrice	varchar(50),PRCD_PCurrencyPrice varchar(50),
			PRCD_SCurrencyPrice varchar(50),PRCD_Total varchar(50),PRCD_UnitID  varchar(50),PRCD_Reson	varchar(50),
			PRCD_PR1D_OrderDetailId varchar(50)
		 )'
		,@XmlTempTableName output
 
		
	    SET @SqlString='INSERT INTO B02_Bill.PRCD_ReturnDetail(PRCD_CONO,PRCD_DIVI,	
		PRCD_ReturnDetailId,PRCD_PRC_ReturnId,PRCD_PD0_ProductId,PRCD_POriginalPrice,PRCD_PCurrencyPrice, ' 
 		+'PRCD_SCurrencyPrice,PRCD_RetDeptID,PRCD_POSRetCode,PRCD_Total,PRCD_UnitID,'
 		+'PRCD_Amount,PRCD_Reson,PRCD_RgUser,PRCD_RgDt,PRCD_LmUser,PRCD_LmDt,PRCD_UptNo,PRCD_Status,PRCD_PR1D_OrderDetailId)'
		
		+' SELECT '''+@PRCD_CONO+''','''+@PRCD_DIVI+''',newID(),'''+@PRC_ReturnId+''',PRCD_PD0_ProductId,PRCD_POriginalPrice,PRCD_PCurrencyPrice,'
		+'PRCD_SCurrencyPrice,'''+@PRCD_RetDeptID+''','''',PRCD_Total,PRCD_UnitID,' 
		+'cast(cast(PRCD_POriginalPrice as decimal(20,6))*cast(PRCD_Total as decimal(20,6)) as decimal(20,6)),PRCD_Reson,'
		+''''+@PRC_RgUser+''','''+@PRCD_RgDt+''','''+@PRC_RgUser+''','''+@PRCD_LmDt+''',''0'','
		+''''+@PRC_Status+''',PRCD_PR1D_OrderDetailId'
...
if @mdl='UPDATE'  
	begin
       ...
       -- set @PRCD_POriginalPrice=isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《N》采购原币价'),'') 
       set @PRCD_LmUser=isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《N》最后修改人'),'')
       -- PRINT @PRC_ReturnId
		SELECT @PRCD_Amount=ISNULL(ISNULL(@PRCD_POriginalPrice,'0') *cast(@PRCD_Total as DECIMAL(20,0)),'0')
    ...
    set @List=isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《W》退货单明细Get_XML'),'')
		...
		,'/ROOT/LIST/ROW'
		,2
		,'(PRCD_POriginalPrice	varchar(50),PRCD_Total varchar(50),PRCD_Reson	varchar(50),
			PRCD_Status varchar(50),PRCD_ReturnDetailId varchar(50)
		 )'
		,@XmlTempTableName output
  
  		SET @SqlString='UPDATE  A SET A.PRCD_UptNo=CAST(A.PRCD_UptNo AS INT)+1,A.PRCD_Status=B.PRCD_Status, '
		+'A.PRCD_LmDt='''+@PRCD_LmDt+''','
		+'A.PRCD_Amount=cast(cast(B.PRCD_POriginalPrice as decimal(20,6))*cast(B.PRCD_Total as decimal(20,6)) as decimal(20,6)),'
		+'A.PRCD_LmUser='''+@PRCD_LmUser+''',A.PRCD_Total=B.PRCD_Total,A.PRCD_Reson=B.PRCD_Reson FROM B02_Bill.PRCD_ReturnDetail A,'+ @XmlTempTableName+ ' B '
		+' WHERE A.PRCD_ReturnDetailId=B.PRCD_ReturnDetailId'
  ...
  if @mdl='DELETE'  
	begin
       ...
       -- PRINT @PRC_ReturnId
    	...
    	SET @SqlString='UPDATE  A SET A.PRCD_UptNo=CAST(A.PRCD_UptNo AS INT)+1,A.PRCD_Status=B.PRCD_Status, '
		+'A.PRCD_LmDt='''+@PRCD_LmDt+''','
		+'A.PRCD_LmUser='''+@PRCD_LmUser+''',FROM B02_Bill.PRCD_ReturnDetail A,'+ @XmlTempTableName+ ' B '
		+' WHERE A.PRCD_ReturnDetailId=B.PRCD_ReturnDetailId'
  ...
  IF @mdl='NEW'  
    begin
		...
		For Select distinct PRCD_ReturnDetailId From B02_Bill.PRCD_ReturnDetail 
		where PRCD_PRC_ReturnId=@PRCD_PRC_ReturnId and PRCD_LmDt=@PRCD_LmDt
		and @PRCD_POSRetCode=''
		Open READ_Design 
		Fetch Next From READ_Design Into @PRCD_ReturnDetailId
		...
		Begin
				...
				UPDATE 	B02_Bill.PRCD_ReturnDetail   SET PRCD_POSRetCode=@PRCD_POSRetCode
				WHERE PRCD_ReturnDetailId=@PRCD_ReturnDetailId
