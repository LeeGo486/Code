...
$(function () {
    //var htmlobj = $.ajax({
    //    url: GetWSRRURL('ef2747d0-4075-4616-8979-c80396b99b00') ,
...
function Edit_OnClick(IsCopy) {
    ...
    $('#form_Edit #txtURL').val(selected_Row.url);    //设置URL
    ...
    $('#form_Edit').form('submit', {
        onSubmit: function (e) {
            ...
            if (endExecuteTime < nowTime) {
                $.messager.alert("提示", "任务最终执行时间不能小于当前时间", 'error');
                return false;
            }
            ...
            data[data.length] = { "name": "txtURL", "value": $('#txtURL').val() };
    ...
    $('#tblLogInfo').datagrid(
           {
               ...
               columns: [[
                   ...
                   {
                       field: 'execute_result', title: '结果', width: 40, sortable: true,
                       formatter: function (val) {
                           if ("True" == val) {
                               return "成功";
                           } else {
                   ...
                   {
                       field: 'execute_message', title: '错误信息', width: 150, search: true, sortable: true,
                       formatter: function (val) {
                           if (val.length > 18) {
...
$.extend($.fn.validatebox.defaults.rules, {
    ...
    UserExecute: {
        validator: function (value) {
            return value.match(/^(([\u4e00-\u9fa5]){2,4})(,(([\u4e00-\u9fa5]){2,4}))*$/);
        },
        message: '格式不正确,请输入中文,并用逗号隔开,如"孔明,诸葛亮,诸葛孔明"'
    }
});
