...
$(function () {
    ...
    $('#fa4_department').combotree({
        ...
        onSelect: function (data) { 
            $('#fa4_cono').attr('value', data.attributes.OZ1_CONO);
            $('#fa4_divi').attr('value', data.attributes.OZ1_DIVI);
        },
...
function turnToDetail(act, row) {
    if (act == "add") { 
        ...
        $('#fa4_department').combotree('setValue', '');
        $('#fa4_id').attr('value', '');
        $('#fa4_cono').attr('value', '');
        $('#fa4_divi').attr('value', '');
        ...
        $('#fa4_department').combotree('setValue', row.fa4_department);
...
function stateContral(state) { 
    if (state == '已审核' || state == '已取消') { 
        ...
        $('#fa4_department').combotree('disable');
    } else {
        //按钮控制     $('#btnSave').show();
        $('#btnTurn').show();
        if (state == '已提交') {
        ...
        $('#fa4_department').combotree('enable');
    } 
}
...
function saveCheck() {
    ...
    //保存中文部门名称
    $('#fa4_department').combotree('setValue', $('#fa4_department').combotree('getText'));

    //主信息部分校验
    if ($('#fa4_department').combotree('getValue') == '') {
