...
var m_Filter = '';  //主的条件
var m_Filter1 = ''; //查询条件
var m_OrderBy = ' ORDER BY fa1_poapplycode DESC '; //排序方式
...
$(function () {
    ...
    m_BudgetTypes.push({ "code": "全部", "codename": "全部" });
    //预算类别 下拉框设置 
    $('#cxBudgetType').combobox({
        panelWidth: "145",
        panelHeight: "80",
        editable: false,
        multiple: false,
    ...
    $(m_DatagridId).datagrid({
		...
		columns:[[  		         
		    ...
		    { field: 'fa1_id', title: 'FA1_ID', width: 100, hidden: true },
            { field: 'fa1_cono', title: 'FA1_CONO', width: 100, hidden: true },
	        { field: 'fa1_divi', title: 'FA1_DIVI', width: 100, hidden: true },
	        { field: 'fa1_poapplycode', title: '申购单号', width: 135, sortable: true },
	        { field: 'fa1_genuscompanycode', title: 'FA1_GenuscomPanyCode', width: 105, sortable: true, hidden: true },
		    { field: 'fa1_genuscompanyname', title: '所属公司', width: 75, sortable: true },
	        { field: 'fa1_state', title: '状态', width: 100
			    , formatter: function (val) { return basedata_getCodename(m_States, val); }
	        },
            { field: 'fa1_applyuser', title: '申请人', width: 60 },
	        { field: 'fa1_employeeid', title: '员工号', width: 60 },
	        { field: 'fa1_applydt', title: '申请日期', width: 75 },
	        { field: 'fa1_deptid', title: 'FA1_DeptID', width: 100, hidden: true },
	        { field: 'fa1_deptname', title: '部门名称', width: 105, sortable: true },
	        { field: 'fa1_centercode', title: '成本中心', width: 80 },
	        { field: 'fa1_centername', title: 'FA1_CenterName', width: 105, hidden: true },
	        { field: 'fa1_applyamount', title: '申请总金额', width: 100, align: "right"
			    , formatter: function (val) { return dataUtil_formatNum(val); }},
	        { field: 'fa1_confirmamount', title: '确认总金额', width: 100, align: "right"
			    , formatter: function (val) { return dataUtil_formatNum(val); }},
		    { field: 'fa1_assetstype', title: '资产类别', width: 80 
			    , formatter: function (val) { return basedata_getCodename(m_AssetsTypes, val); }
		    },
            { field: 'fa1_budgettype', title: '预算类别', width: 80
			    , formatter: function (val) { return basedata_getCodename(m_BudgetTypes, val); }
            },
	        { field: 'fa1_projectcode', title: '项目代码', width: 100 },
	        { field: 'fa1_advancedcode', title: '高薪代码', width: 100 },
	        { field: 'fa1_departmentdirector', title: 'FA1_DepartmentDirector', width: 105, hidden: true },
		    { field: 'fa1_divisionmanager', title: 'FA1_DivisionManager', width: 100, hidden: true },
	        { field: 'fa1_itdirector', title: 'FA1_ITDirector', width: 100, hidden: true },
	        { field: 'fa1_purchasemanager', title: 'FA1_PurchaseManager', hidden: true },
	        { field: 'fa1_shareservicecenterdirector', title: 'FA1_ShareServiceCenterDirector', width: 105, hidden: true },
		    { field: 'fa1_financedirector', title: 'FA1_FinanceDirector', width: 100, hidden: true },
	        { field: 'fa1_isreject', title: 'FA1_IsReject', width: 100, hidden: true },
	        { field: 'fa1_comment', title: 'FA1_Comment', width: 100, hidden: true },
	        { field: 'fa1_rgdt', title: '创建时间', width: 105},
		    { field: 'fa1_rguser', title: '创建人', width: 80 },
	        { field: 'fa1_lmdt', title: '修改时间', width: 105 },
	        { field: 'fa1_lmuser', title: '修改人', width: 80 },
	        { field: 'fa1_uptno', title: '修改次数', width: 80 }
		]],
		pagination:false,
		rownumbers:true, 
		toolbar: '#tjcxk',
   ...
   if (uType == 0) { //无权限
       ...
       m_Filter = m_Filter + ' AND fa1_applyuser=\'' + m_UserInfo.UserName + '\'';
   } else if (uType == 2) { //部门长
       var mdepts = basedata_getMDeptCodes(m_UserInfo.empCode) + '\'-2\'';
       m_Filter = m_Filter + ' AND FA1_DeptID in (' + mdepts + ')';
       ...
       m_Filter = m_Filter + ' AND ( OZ1_ComCode1 in (' + morgs
           + ') OR  FA1_DeptID  in (' + mdepts + '))';
   } else if (uType == 3) { //事业部部门长
       var mdepts = basedata_getMDeptCodes(m_UserInfo.empCode) + '\'-2\''; 
       m_Filter = m_Filter + ' AND ( FA1_DeptID1  in (' + mdepts
           + ') OR  FA1_DeptID  in (' + mdepts + '))';
   } else if (uType == 4) { //信息专员 
       m_Filter = m_Filter + ' AND  FA1_AssetsType = 1  ';
   } else if (uType == 5) { //信息总监 
       var mdepts = basedata_getMDeptCodes(m_UserInfo.empCode) + '\'-2\'';
       m_Filter = m_Filter + ' AND (FA1_AssetsType = 1 OR FA1_DeptID in (' + mdepts + '))';
   } else if (uType == 5.1) { //财务经理 
       var myyorgid = peoa_GetOrgID(1); 
       var mqtcwbid = peoa_GetOrgID(3);
       if (m_UserInfo.empCode == '1412188') { // 高晓红
           m_Filter = m_Filter + ' AND OZ1_ComCode1 = \'' + myyorgid + '\' AND FA1_DeptID1 not in (\'' + mqtcwbid + '\') ';
       } else {
           m_Filter = m_Filter + ' AND ( OZ1_ComCode1 not in (\'' + myyorgid + '\') '
                    + ' OR FA1_DeptID1 = \'' + mqtcwbid + '\' )';
       }
   } else if (uType == 6) {  //采购经理 和 行政总监 和 固定资产运维
       m_Filter = 'AND 1=1 ';
   } 
...
function clearText() { 
    ...
    $('#cxBudgetType').combobox('setValue', '');
}
...
function searchByCons()  { 
	...
	if ($('#cxBillCode').attr('value') != '') {
	    cfilter = cfilter + ' AND fa1_poapplycode LIKE \'%' + $('#cxBillCode').attr('value') + '%\'';
	}
	if ($('#cxDeptName').attr('value') != '') {
	    cfilter = cfilter + ' AND fa1_deptname LIKE \'%' + $('#cxDeptName').attr('value') + '%\'';
	}
	if ($('#cxAppPer').attr('value') != '') {
	    cfilter = cfilter + ' AND fa1_applyuser LIKE \'%' + $('#cxAppPer').attr('value') + '%\'';
	} 
	if ($('#cxCostCenter').attr('value') != '') {
	    cfilter = cfilter + ' AND fa1_centercode LIKE \'%' + $('#cxCostCenter').attr('value') + '%\'';
	} 
	if ($('#cxState').combobox('getValue') != '' && $('#cxState').combobox('getValue') != '全部') {
	    if ($('#cxState').combobox('getValue') == '20') { 
	        cfilter = cfilter + ' AND fa1_state LIKE \'2_\'';
	    } else { 
	        cfilter = cfilter + ' AND fa1_state = \'' + $('#cxState').combobox('getValue') + '\'';
	    }
	} 
	if ($('#cxAssetsType').combobox('getValue') != '' && $('#cxAssetsType').combobox('getValue') != '全部') {
	    cfilter = cfilter + ' AND  fa1_assetstype = \'' + $('#cxAssetsType').combobox('getValue') + '\'';
	}
	
	if ($('#cxAppDate').attr('value') != '') {
	    cfilter = cfilter + ' AND  convert(varchar(10),fa1_applydt,120) LIKE \'' + $('#cxAppDate').attr('value') + '%\'';
	}
	if ($('#cxBudgetType').combobox('getValue') != '' && $('#cxBudgetType').combobox('getValue') != '全部') {
	    cfilter = cfilter + ' AND  fa1_budgettype = \'' + $('#cxBudgetType').combobox('getValue') + '\'';
	}
...
function   actLoadData(page,number)   {   
	...
	var curl = GetWSRRURL(peoa_GetWSRRURL(3)) + "&type=GetHs&XML="+ encodeURIComponent(cXML) ; //拼接url
    //alert(cXML);	
	$.ajax({
	    url: curl,
	    type: "GET",
...
function view(act) {
    ...
    if (act == 'actdjmx') { 
        ...
        if (rowData) {
            var title = '申购单';
            var curl = ''; 
            curl = '../../../WEB/PE_OA/FA/FA_APPQuery.html' + '?v0=' + rowData.fa1_poapplycode;
            addTab(title + rowData.fa1_poapplycode, curl);
        } else {
            alert('请先选择一条记录！');
        }
    } 
       ...
       var cXML = '<Page>' + '1' + '</Page><Num>' + '100' + '</Num>'
        + '<Cons1>' + rowData.fa1_poapplycode + '</Cons1>'
       ...
       if (act == 'actspls') { 
           if(rowData){ 
               ...
               var itextc = '单据编号：' + rowData.fa1_poapplycode + '\n\n';
               var itext = '';
