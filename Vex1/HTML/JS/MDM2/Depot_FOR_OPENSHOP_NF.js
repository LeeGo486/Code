...
var editIndex;
var m_RowData;
var m_ViewTableName = "V_L0013_FOR_OPENSHOP_NF";
...
$(function () {
    ...
    //txtCom
    $('#txtCom').combobox({
        url: GetComboxWSRRURL('4d173b24-6efd-419e-8efa-ce9e5138ad36') + "&MDMClass=0001&PropertyName=" + escape("公司中文简称") + "&Where=" + escape("公司中文简称") + " in " + escape("('纳卉','卓莱雅')"),
        required: true,
        panelHeight: 'auto',
        editable: false,
        valueField: 'id',
...
function onclick_SelectArea_Comfirm() {
    ...
    var ed = dg.datagrid('getEditor', { index: index, field: '城市' });

    $.fn.datagrid.defaults.editors.selectButton.setValue(ed.target, strPrefecture)

    var ed = dg.datagrid('getEditor', { index: index, field: '区县' });

    $.fn.datagrid.defaults.editors.selectButton.setValue(ed.target, strCity)


    var ed = dg.datagrid('getEditor', { index: index, field: '城市定义' });
...
function AddNewRow() {
    if (endEditing()) {
        //SetBodyIDEdit(true);
        var rowData = { body_id: '', body_id_l0006: '11111111-1111-1111-1111-1111111111111', 冻结状态: '0', 省份: "无", 城市: "无", 区县: "无", 城市定义: "" };
