...
//var selected_Param_Row;  全局变量,暂时不用
...
function Edit_OnClick() {
    ...
    document.getElementById("txtSupplier_Name").readOnly = true
    $('#divEditSupplier').window({
        title: "编辑",
        width: 600,
        height: 530,
        modal: true
    ...
    //判断联系电话是否有值，如果没有就赋初始值
    if (selected_Row.su0_phone == "") {
        $('#txtSupplier_Phone').val('只能输入数字与“-”')   //初始值
    }
    else {
...
function Add_Row() {
    if (endEditing()) {
        if (selected_Row != null) {
            $('#txtSupplier_Name').val('');

            $('#txtSupplier_Address').val('');
            $('#txtSupplier_Contact').val(''); ;
            $('#txtSupplier_Phone').val('只能输入数字与“-”');
            $('#txtSupplier_Bank').val('');
            $('#txtSupplier_BankNo').val('');
            $('#txtSupplier_Bankadd').val('');
            $('#txtBank_Swift').val('');    
        }
        $('#divEditSupplier').window({
            title: "新增",
