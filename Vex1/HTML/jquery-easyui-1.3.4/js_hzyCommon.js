...
function GetLoginURL(param) {
    return GetWSRRURL('550fc164-adc8-4f88-bc9d-73c4248050bc', param);
}
...
function GetHighChartsWSRRURL(pstrWSID, pchartType, pcategories) {
    var strReturn = GetWSRRURL(pstrWSID);
    strReturn += "&RequestType=HighChart";

    if (!pchartType) {
        pchartType = "common";
    }
    strReturn += "&chartType=" + chartType;

    if (!pcategories) {
        pcategories = "";
    }
    strReturn += "&categories=" + pcategories;
...
function ReplaceSpecialXMLString(strSrcXML) {
    var strReturnXML = "";
    strReturnXML = strSrcXML.toString().replaceAll("&", "&amp;", false).replaceAll("<", "&lt;", false).replaceAll(">", "&gt;", false).replaceAll("\n","\\n",false);
    return strReturnXML;
}
...
/*
    作者: LeeGo
    创建日期: 2015-03-12
    取来的数组，有相同字段需要拼凑成多行的方法。
    data是数据，
    optype是类型，
    judge是判断是否换行的字段。（用数组中最后一个不重复的name属性的值来做标准）。
    注意，数组是否为空的判断请在调用前判断。
*/
function GetFormArray(data, optype,judge) {
    var updatedData = '<Root>';
    updatedData += "<OPTYPE>" + optype + "</OPTYPE>";
    updatedData += '<List>';
    updatedData += "<Rows>";
    ...
    do {
        var dataRow = data.shift();
        dataLength = data.length;
        if (dataRow.name != judge) {
            updatedData += "<" + dataRow.name + ">" + dataRow.value + "</" + dataRow.name + ">";
        }
        else if (dataRow.name == judge && dataLength == 0) {
            updatedData += "<" + judge + ">" + dataRow.value + "</" + judge + "></Rows>";
        }
        else if (dataRow.name == judge && dataLength != 0) {
            updatedData += "<" + judge + ">" + dataRow.value + "</" + judge + "></Rows><Rows>";
        };
    } while (0 != dataLength);
...
var Formater = {
    ...
    Account1: function (value, rec, index) {
        this.formatDataType = "Account1";

        if (value == undefined) {
            return "";
        }
        return $.formatNumber(value, { format: "#,##0.0000", locale: "cn" });
    },

    Account2: function (value, rec, index) {
        this.formatDataType = "Account2";
         
        if (value == undefined) {
            return "";
        }
        return $.formatNumber(value, { format: "#,##0.0", locale: "cn" });
    },
