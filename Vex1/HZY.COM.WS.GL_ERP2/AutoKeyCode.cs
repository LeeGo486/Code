            ...
            try
            {
                ...
                if (pstrCodeType == "服装童装配饰")
                {
                    ...
                    strMaxPurchaseCode = m_conn.GetDataTableFirstValue(@"
                        ...
                        FROM    [B02_BILL].[PO1_POHead]
                                where PO1_POCode like '24%'").ToString();
                    //如果最大号为空，放默认值2400000
                    if (strMaxPurchaseCode == "")
                    {
                        nMaxPurchaseCode = 2400000;
                    }
                    else
                    {
                        //取最大号，加1
                ...
                else if (pstrCodeType == "新增合同编号")
                {
                    ...
                    FROM    [B02_BILL].[PC1_POContractHead]
                            where PC1_RegionCode = '" + pstrParam0 + "' AND PC1_POTypeCode = '" + pstrParam1 + "'").ToString();
                    if (strMaxContractCode == "")
                    {
                        strMaxContractCode = pstrParam0 + pstrParam1 + strYear + strMonth + "001";
                    }
                    else
                    {
                        string strRight = strMaxContractCode.Substring(6, 3);      //取流水号
                ...
                else if (pstrCodeType == "新增上货编号")
                {
                    ...
                    ////日期
                    //string strNow = DateTime.Now.Year.ToString().Substring(2, 2);
          
                    //Int32 intMonth = Convert.ToInt32(DateTime.Now.Month.ToString());
                    //Int32 intDay = Convert.ToInt32(DateTime.Now.Day.ToString());

                    ////月
                    //if ( intMonth < 10){ 
                    //    strNow += "0" + intMonth.ToString();
                    //}else{
                    //    strNow += intMonth.ToString();
                    //}

                    ////日
                    //if (intDay < 10)
                    //{
                    //    strNow += "0" + intDay.ToString();
                    //}
                    //else {
                    //    strNow += intDay.ToString();
                    //}
                ...
                else if (pstrCodeType == "家具家饰")
                {
                    ...
                    strMaxPurchaseCode = m_conn.GetDataTableFirstValue(@"
                                        SELECT  
                                                MAX(PO1_POCode) PO1_POCode
                                        FROM    [B02_BILL].[PO1_POHead]
                                                where PO1_POCode like '25%'").ToString();
                    //如果为空放默认值2500000，如果不为空，数据库最大号加1
                    if (strMaxPurchaseCode == "")
                    {
                        nMaxPurchaseCode = 2500000;
                    }
                    else
                    {
                        nMaxPurchaseCode = Convert.ToInt32(strMaxPurchaseCode) + 1;
                ...
                else if (pstrCodeType == "新增到货计划编号")
                {
                    ...
                    MaxDeliveryplanDataCode = m_conn.GetDataTableFirstValue(@"
                                       SELECT MAX(AP1_APCode)
                                              ...
                                              WHERE AP1_APCode LIKE 'PL" + strDateTime + "%'").ToString();
                ...
                else if (pstrCodeType == "新增付款申请单")
                {
                    ...
                    string MaxPR1_RequestCode = m_conn.GetDataTableFirstValue(@"
                                        SELECT  MAX(PR1_RequestCode)
                                                ...
                                                WHERE PR1_RequestCode LIKE 'PR" + pstrParam0 + "%'").ToString();
                ...
                else if (pstrCodeType == "新增项次")
                {
                    ...
                    string MaxPO2_LISeq = m_conn.GetDataTableFirstValue(@"
                                        SELECT  MAX(PO2_LISeq)
                                                ...
                                                WHERE PO2_POCode = '" + pstrParam0 + "'").ToString();
                    if (MaxPO2_LISeq == "")
                    {
                        nMaxPO2_LISeq = 1;
                    }
                ...
                else if (pstrCodeType == "新增付款单")
                {
                    ...
                    string MaxPB1_PaymentBill = m_conn.GetDataTableFirstValue(@"
                                        SELECT  MAX(PB1_PayBillCode)
                                                ...
                                                WHERE PB1_PayBillCode LIKE 'PY" + pstrParam0 + "%'").ToString();
                ...
                else if (pstrCodeType == "新增供应商")
                {
                    if (pstrParam1 == "C")//如果是国内，取参数0、1、2 分类、币种、特征 为条件获取最大值
                    {
                        string MaxSupplierCode = m_conn.GetDataTableFirstValue(@"
                                        SELECT  MAX(SP1_SupplierCode) 
                                                ...
                                                WHERE  SP1_SupplierCode LIKE '" + pstrParam0 + pstrParam1 + pstrParam2 + "%'").ToString();
                        if (MaxSupplierCode == "")
                        {
                        ...
                        {
                            //如果存在，截取最后3位加1
                            MaxSupplierCode = MaxSupplierCode.Substring(0, MaxSupplierCode.Length - 3) + (Convert.ToInt32(MaxSupplierCode.Substring(3, 3)) + 1).ToString("000");
                        }
                    ...
                    else
                    {
                        ...
                        string MaxSupplierCode = m_conn.GetDataTableFirstValue(@" SELECT MAX(SP1_SupplierCode)
                                                                                          FROM [B01_MDM].[SP1_Supplier]
                                                                                          WHERE SP1_SupplierCode LIKE 'A%' AND LEN(SP1_SupplierCode)=5").ToString();
                        if (MaxSupplierCode == "")
                        {
                            MaxSupplierCode = "A0001";
                        }
                        else
                        {
                            //取最大号加1
                            MaxSupplierCode = MaxSupplierCode.Substring(0, MaxSupplierCode.Length - 4) + (Convert.ToInt32(MaxSupplierCode.Substring(1, MaxSupplierCode.Length - 1)) + 1).ToString("0000");
                        }
                        return MaxSupplierCode;
                    }
                }
                ...
                else if (pstrCodeType == "获取SKC最大号")
                {
                    string strMaxSKCCode = m_conn.GetDataTableFirstValue(@"
                                        SELECT MAX(ST1_StyleCode) maxskc
                                                FROM [B01_MDM].[ST1_Style]
                                                WHERE ST1_StyleCode LIKE '" + pstrParam0 + "%'").ToString();
                    ...
                    {
                        //strMaxSKCCode = strMaxSKCCode.Substring(0, strMaxSKCCode.Length - 4) + (Convert.ToInt32(strMaxSKCCode.Substring(6, 3)) + 1).ToString("000");
