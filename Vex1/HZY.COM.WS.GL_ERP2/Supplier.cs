    ...
    public class Supplier : HZYLogicBase, ILogicGetData
    {
        ...
        private void Get(string pstrfilter)
        {
            string strSQL = @"
                            ...
                            WHERE  1=1 AND  
                            SP1_Desp LIKE '%" + pstrfilter + "%' OR SP1_Name LIKE '%" + pstrfilter + "%'  Order by SP1_SupplierCode " + m_hzyPageInfo.Where;
        ...
        private void EDIT()
        {
            ...
            int nCountYN = m_conn.GetDataTableRowCount("SELECT * FROM [B01_MDM].[SP1_Supplier] WHERE SP1_SupplierCode='" + strSP1_SupplierCode + "'");
            ...
            if (nCountYN == 0)
            {
                ...
                //判断供应商简称或者全称存在就不给修改
                int nCount = m_conn.GetDataTableRowCount("SELECT * FROM [B01_MDM].[SP1_Supplier] WHERE sp1_name='" + strSp1_Name + "' or SP1_Desp ='"+ strSp1_Desp+"'");
                ...
                //传入参数，生成最大号
                //string MaxPurchaseCode = AutoKeyCode.GetMaxKeyCode("新增供应商", strSP1_TypeCode, strSP1_RegionCode, strSP1_FeatureCode, m_conn);
                //构建新表
                ...
                ds.Tables["List"].Rows[0]["SP1_DIVI"] = "D01";
                ds.Tables["List"].Rows[0]["SP1_CONO"] = "GL";
                ...
                ds.Tables["list"].Rows[0]["sp1_suppliercode"] = strSP1_SupplierCode;
                bResult = m_conn.Insert("[B01_MDM].[SP1_Supplier]", ds.Tables["list"]);
            }
            else
            {
                //判断供应商简称或者全称存在就不给修改
                //int nCount = m_conn.GetDataTableRowCount("SELECT * FROM [B01_MDM].[SP1_Supplier] SP1_SupplierCode != " + strSP1_SupplierCode + " and sp1_name='" + strSp1_Name + "' or SP1_Desp ='" + strSp1_Desp + "'");

                //if (nCount != 0)
                //{
                //    throw new Exception("新修改的供应商：" + strSp1_Name + "已经存在，信息错误！");
                //}
                ...
                ds.Tables["List"].Rows[0]["SP1_LmDt"] = timeNow;
                ds.Tables["List"].Rows[0]["SP1_LmUser"] = hzyMessage.User_Name_CN;
                bResult = m_conn.Update("[B01_MDM].[SP1_Supplier]", ds.Tables["list"], "sp1_suppliercode='" + strSP1_SupplierCode + "'");
            }
