...
if (jQuery) (function ($) {
    $.extend($.fn, {
        ajaxUpload: function (o) {
            ...
            // 初始化
            if ($('#ajaxupload_dialog').html() == null) {
                $('body').append('<div id="ajaxupload_dialog"></div>');
            }
            $('#ajaxupload_dialog').dialog(o.dialog);
            $(_this).bind('click', function () {
                $(_this).blur();
                $('#ajaxupload_dialog')
                    .html('<p class="ajaxupload_p_1">上传的路径是 <span id="ajaxupload_path"> ' + o.path + ' </span></p><p class="ajaxupload_p_2"><input id="ajaxupload_button" type="button" value="浏览文件" /></p>')
                    .dialog('open');

                o.ajaxupload.action += '&path=' + $('#ajaxupload_path').html();
                new AjaxUpload('ajaxupload_button', o.ajaxupload);
            });
        }
    });
})(jQuery);