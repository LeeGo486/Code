...
namespace HZY.COM.WS.Hawk
{
    public class Payment : HZYLogicBase, ILogicGetData
    {
        ...
        private bool UpdatePaymentRequisitHead(string strRequestCode, string strDIVI)
        {
            bool bResult = false;
            string sltSql = @"SELECT COUNT(*) TOTAL,
	                                (SELECT COUNT(*) FROM B02_BILL.PR2_PaymentRequisitDetail WHERE PR2_RequestCode=@param0 AND PR2_DIVI=@param1 AND PR2_State='99') CANCEL,
	                                (SELECT COUNT(*) FROM B02_BILL.PR2_PaymentRequisitDetail WHERE PR2_RequestCode=@param0 AND PR2_DIVI=@param1 AND PR2_State='85') PAID 
                                FROM B02_BILL.PR2_PaymentRequisitDetail WHERE PR2_RequestCode=@param0 AND PR2_DIVI=@param1";
            ...
            string strSQL = @"
                                SELECT [PR2_ID]
                                  ...
                                  FROM [Hawk].[B02_BILL].[PR2_PaymentRequisitDetail] A with(nolock)
                                        JOIN [B02_BILL].[PO2_PODetail] B with(nolock) ON A.PR2_PO2_ProductCode=B.PO2_PD1_ProductCode AND A.PR2_DIVI=B.PO2_DIVI
                                  WHERE 1=1 " + m_hzyPageInfo.Where;
            ds_Return = m_conn.GetDataSetForPageList(strSQL, m_hzyPageInfo);
        }
        #endregion
        ...
        private bool UpdatePaymentRequisitDetail(string strPOcode, string strPaymentCode, double ratio, double useMoney, double Deposit_payamount, string strDIVI, string doWhat)
        {
            ...
            if (doWhat == "insert")
            {
                ...
                strSQL += @"SELECT [PO2_CONO],[PO2_DIVI],'" + strPaymentCode + "',[PO2_LISeq],[PO2_LSSeq],[PO2_PD1_ProductCode],[PO2_OCurrencyAmount]*" + ratio + " shareMoney," +
                                          useMoney + "*" + "[PO2_OCurrencyAmount]/" + Deposit_payamount + " poolMoney,[PO2_OCurrencyAmount]*" + ratio + "-" + useMoney + "*" + "[PO2_OCurrencyAmount]/" + Deposit_payamount +
                                          " payMoney,[PO2_State],'" + timeNow + "','" + m_hzyMessage.User_Name_CN + "','" +
                                          timeNow + "','" + m_hzyMessage.User_Name_CN + "',0 FROM B02_BILL.PO2_PODetail WHERE PO2_State='15' AND [PO2_DIVI]='" + strDIVI + "' AND [PO2_POCode]='" + strPOcode + "'";
            ...
            else
            {
                string strSQL1 = @" UPDATE [B02_BILL].[PR2_PaymentRequisitDetail] SET PR2_ShareAmount=T.shareAmount,PR2_PoolMoney=T.poolMoney,PR2_PayMoney=T.payMoney 
                                    FROM (  SELECT PO2_POCode,PO2_DIVI,PO2_PD1_ProductCode,PO2_OCurrencyAmount*" + ratio + @" shareAmount," + useMoney + "*" + "PO2_OCurrencyAmount/" + Deposit_payamount + " poolMoney,PO2_OCurrencyAmount*" + ratio + "-" +
                                                   useMoney + "*" + "[PO2_OCurrencyAmount]/" + Deposit_payamount + " payMoney " +
                                          @"FROM B02_BILL.PO2_PODetail 
		                                    WHERE PO2_POCode=(SELECT RP1_PO1_POCode FROM B02_BILL.RP1_POReceiptHead WHERE RP1_ReceiptCode='" + strPaymentCode + "' AND RP1_DIVI='" + strDIVI + @"')) t,[B02_BILL].[PR2_PaymentRequisitDetail]
		                            WHERE T.PO2_DIVI=PR2_DIVI AND T.PO2_PD1_ProductCode=PO2_PD1_ProductCode";
                m_conn.Excute(strSQL1);
                return true;
            }
        }
        #endregion
        ...
        private void EditPaymentDetail()
        {
            ...
            string strRequestCode = ds.Tables["list"].Rows[0]["PR2_RequestCode"].ToString();//获取付款申请单号
            string strDIVI = ds.Tables["list"].Rows[0]["pr2_divi"].ToString(); //获取DIVI
            ...
            {
                string strPB1_PayBillCode = AutoKeyCode.GetMaxKeyCode("新增付款申请单", strPOcode, strDIVI, "", m_conn);
                int code = Convert.ToInt32(strPB1_PayBillCode.Substring(9, 2));

                //创建付款单表            
                Table.Columns.Add("PB1_ID");
                Table.Columns.Add("PB1_CONO");
                Table.Columns.Add("PB1_DIVI");
                Table.Columns.Add("PB1_PayBillCode");
                Table.Columns.Add("PB1_PR1_RequestCode");
                Table.Columns.Add("PB1_State");
                Table.Columns.Add("PB1_PaymentAmount");
                Table.Columns.Add("PB1_PaymentTotalAmount");
                Table.Columns.Add("PB1_PaymentDeductionAmount");
                Table.Columns.Add("PB1_Comment");
                Table.Columns.Add("PB1_RgDt");
                Table.Columns.Add("PB1_RgUser");
                Table.Columns.Add("PB1_LmDt");
                Table.Columns.Add("PB1_LmUser");
                Table.Columns.Add("PB1_UptNo");
                ...
                for (int i = 0; i < ds.Tables["list"].Rows.Count - 1; i++)
                {
                    ...
                    dr1["PB1_ID"] = System.Guid.NewGuid();//strPB1_PayBillCode
                    dr1["PB1_CONO"] = "HYFG";
                    dr1["PB1_DIVI"] = ds.Tables["list"].Rows[i]["pr2_divi"].ToString();
                    dr1["PB1_PayBillCode"] = strPB1_PayBillCode.Substring(0, strPB1_PayBillCode.Length - 2) + code.ToString();
                    dr1["PB1_PR1_RequestCode"] = strRequestCode;
                    dr1["PB1_State"] = "85";
                    dr1["PB1_PaymentAmount"] = ds.Tables["list"].Rows[i]["pr2_paymoney"].ToString();
                    dr1["PB1_PaymentTotalAmount"] = ds.Tables["list"].Rows[i]["pr2_shareamount"].ToString();
                    dr1["PB1_PaymentDeductionAmount"] = ds.Tables["list"].Rows[i]["pr2_poolmoney"].ToString();
                    dr1["PB1_Comment"] = "";
                    dr1["PB1_RgDt"] = timeNow;
                    dr1["PB1_RgUser"] = hzyMessage.User_Name_CN;
                    dr1["PB1_LmDt"] = timeNow;
                    dr1["PB1_LmUser"] = hzyMessage.User_Name_CN;
                    dr1["PB1_UptNo"] = "0";
            ...
            if (bResult)
            {
                ...
                bResult = UpdatePaymentRequisitHead(strRequestCode, strDIVI);

                if (bResult)
                {
                    if (strState == "85") //定金付款,在付款单插入相应的数据
                    {
                        bResult = m_conn.Insert("[B02_BILL].[PB1_PaymentBill]", Table);
                    }
                    if (strState == "90") //定金退款
                    {
            ...
            if (bResult)
            {
                dt_EditResult.Rows.Add(new object[] { true, "取消成功" });
            }
            else
            {
                dt_EditResult.Rows.Add(new object[] { false, "取消失败" });
            }
        }
        #endregion
