    ...
    public class Purchase : HZYLogicBase, ILogicGetData
    {
        ...
        private void EditAPODetail()
        {
            ...
            for (int i = 0; i < dList.Rows.Count; i++)
            {
                ...
                dr["AP2_APCode"] = dList.Rows[i]["ap1_APCode"].ToString();
                ...
                dr["AP2_APlanDt"] = Convert.ToDateTime(dList.Rows[i]["ap1_aplandt"].ToString());
                dr["AP2_RgDt"] = timeNow;
            ...
            if (bResult)
            {
                ...
                //更新主表到货计划总数
                string strAPCode = dList.Rows[0]["ap1_APCode"].ToString();
                string strDIVI = dList.Rows[0]["PO2_DIVI"].ToString();
                string strSQLupt = @"
                                    UPDATE [B02_BILL].[AP1_APOHead] 
                                    SET AP1_APlanNumber=(SELECT SUM(AP2_APlanNumber) PlanNumber 
                                                            FROM [B02_BILL].[AP2_APODetail] WITH(NOLOCK)   
                                                            WHERE AP2_DIVI=AP1_DIVI AND AP2_APCode=AP1_APCode AND AP2_State='35')
                                    WHERE AP1_DIVI='" + strDIVI + "' AND AP1_APCode='" + strAPCode + "'";
                int rCount = m_conn.Excute(strSQLupt);
                if (rCount == 1)
                {
        ...
        private void GETAPOLIST()
        {
            ...
            string strSQL = @"            
                                ...
                                SELECT A.AP2_APCode,AP2_LINumber,AP2_LSNumber,AP2_APlanNumber,AP2_PD1_ProductCode,Convert(varchar(10),B.AP1_APlanDt,120) AP1_APlanDt
                                ...
                                SELECT A.*,E.ST3_ST1_StyleCode,E.ST3_Size,(po2_number - APlanNumberSums) differences,PO2_PD1_SupperierStyle,PO2_PD1_SupperierColor,PO2_PD1_SupperierSize,PO2_PD1_ProductCode,
                                ...
                                SELECT [PO2_ID]
                                  ...
                                  join (
		                                  ...
		                                  from [Hawk].[B01_MDM].ST2_StyleColor t2
			                                join [Hawk].[B01_MDM].ST3_StyleColorSize t3
				                                on t2.ST2_SKCCode=t3.ST3_ST2_SKCCode) t4
	                                on t1.PO2_PD1_ProductCode=t4.ST3_SKUCode where 1=1 AND PO2_State='35' " + m_hzyPageInfo.Where;
