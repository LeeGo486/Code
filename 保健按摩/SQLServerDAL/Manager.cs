...
namespace EPReper.SQLServerDAL
{
    public partial class Manager : IManager
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,username,isAdmin,repair,phonenum,works,states from Manager ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
};
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables[0].Rows[0]["works"] != null && ds.Tables[0].Rows[0]["works"].ToString() != "")
                {
                    model.works = ds.Tables[0].Rows[0]["works"].ToString();
                }
        ...
        public bool Update(Model.Manager model)
        {
            ...
            parameters[3].Value = model.works;
