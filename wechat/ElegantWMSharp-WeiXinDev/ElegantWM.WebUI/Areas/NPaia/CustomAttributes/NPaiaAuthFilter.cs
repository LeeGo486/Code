...
{
    public class NPaiaAuthFilter : ActionFilterAttribute
    {
        ...
        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            //过滤这个url，查询vip积分的
            if (filterContext.HttpContext.Request.RawUrl.Contains("/SearchVipScore"))
            {
                return;
            }
            ...
            if (!string.IsNullOrEmpty(filterContext.HttpContext.Request["sid"]) && !string.IsNullOrEmpty(filterContext.HttpContext.Request["oid"]))
            {
                ...
                if (WMFactory.WXLKRegMemberFans.GetCount(f => f.AccountId == sid && f.OpenId == oid && f.IsUsing == 0) <= 0)
                {
                    ...
                    {
                        filterContext.Result = new RedirectResult("~/NPaia/VIP/CreateVIP?sid=" + sid.ToString() + "&oid=" + oid + "&ReturnUrl=" + filterContext.HttpContext.Server.UrlEncode(filterContext.HttpContext.Request.RawUrl));
                        return;
                    }
                }
