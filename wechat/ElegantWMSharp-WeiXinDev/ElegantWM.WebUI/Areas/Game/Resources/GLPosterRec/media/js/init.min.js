...
(function(){var version,script;for(var i=document.scripts.length-1;i>=0;i--){script=document.scripts[i];if(script.src&&script.src.indexOf('init.min.js')>=0){version=(/ver\=[\d\.]+/ig).exec(script.src);version=(version&&version.length)?version[version.length-1]:'ver=0.1'}};seajs.config({/*preload:['app.min.js'],*/map:[[/^\.(css|js)$/,function(url){return url+=(url.indexOf('?')===-1?'?':'&')+version}]],debug:false});seajs.use('page/main')})();