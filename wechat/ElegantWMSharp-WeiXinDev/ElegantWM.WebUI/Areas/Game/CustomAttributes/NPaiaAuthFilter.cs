    ...
    public class NPaiaAuthFilter : ActionFilterAttribute
    {
        ...
        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            if (!string.IsNullOrEmpty(filterContext.HttpContext.Request["sid"]) && !string.IsNullOrEmpty(filterContext.HttpContext.Request["oid"]))
            {
                ...
                //判断是否注册了（绑定）
                if (WMFactory.WXFans.GetCount(f => f.AccountId == sid && f.OpenId == oid && f.IsUsed == 0) == 0)
                {
                    filterContext.Result = new RedirectResult("http://mp.weixin.qq.com/s?__biz=MjM5NjI5MTk5MA==&mid=207319828&idx=1&sn=182a7c5d3c3921cf0c81f10d2e041912#rd");
                    return;
                }
            }//如果sid为空
