...
{
    [Description("满意度问题")]
    public class HRQuestionController : BaseController
    {
        [Page]
        [Description("满意度问题")]
        ...
        {
            int totalCount = 0;
            var list = WMFactory.HRQuestion.FindByPage(start, limit, out totalCount,
        ...
        [Description("更新")]
        [HttpPut]
        public JsonResult Put(HR_Question _t)
        {
            IHRQuestionService service = WMFactory.HRQuestion;
            HR_Question entity = service.GetById(_t.Id.ToString());
            entity = AutoMapper<HR_Question, HR_Question>.Convert(_t, entity);
        ...
        [Description("新增")]
        [HttpPost]
        public JsonResult Post(HR_Question _t)
        {
            if (WMFactory.HRQuestion.Insert(_t))
        ...
        [Description("删除")]
        [HttpDelete]
        public JsonResult Delete(HR_Question _t)
        {
            if (WMFactory.HRQuestion.Delete(s => s.Id == _t.Id ))
