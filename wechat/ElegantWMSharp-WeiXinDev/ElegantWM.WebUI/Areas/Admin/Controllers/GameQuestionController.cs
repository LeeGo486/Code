...
{
    [Description("为爱积分")]
    public class GameQuestionController : BaseController
    {
        [Page]
        [Description("为爱积分")]
        ...
        {
            int totalCount = 0;
            var list = WMFactory.GameQuestion.FindByPage(start, limit, out totalCount,
        ...
        [Description("更新")]
        [HttpPut]
        public JsonResult Put(Game_Question _t)
        {
            IGameQuestionService service = WMFactory.GameQuestion;
            Game_Question entity = service.GetById(_t.Id.ToString());
            entity = AutoMapper<Game_Question, Game_Question>.Convert(_t, entity);
        ...
        [Description("新增")]
        [HttpPost]
        public JsonResult Post(Game_Question _t)
        {
            if (WMFactory.GameQuestion.Insert(_t))
        ...
        [Description("删除")]
        [HttpDelete]
        public JsonResult Delete(Game_Question _t)
        {
            if (WMFactory.GameQuestion.Delete(s => s.Id == _t.Id ))
