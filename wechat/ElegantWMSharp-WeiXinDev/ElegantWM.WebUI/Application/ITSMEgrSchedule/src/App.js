...
Ext.define('Ext.calendar.App', {
    ...
    constructor : function() {
        ...
        this.egrStore = Ext.create('Ext.data.Store', {
            fields: ['EgrName'],
            proxy: {
                type: 'ajax',
                url: '/Admin/ITSMEngineer/GetAll',
                reader: {
                    type: 'json',
                    root: 'data'
                }
    ...
    showEditWindow : function(rec, animateTarget){
        if(!this.editWin){
            this.editWin = Ext.create('Ext.calendar.form.EventWindow', {
                calendarStore: this.calendarStore,
                egrStore:this.egrStore,
                listeners: {
                    'eventadd': {
                        fn: function(win, rec){
                            win.hide();
                            rec.data.IsNew = false;
                            rec.data.Id = Ext.data.IdGenerator.get('uuid').generate();
                            rec.data.TagId = rec.data.Id;
    ...
    updateTitle: function(startDt, endDt){
        ...
        if(Ext.Date.clearTime(startDt).getTime() == Ext.Date.clearTime(endDt).getTime()){
            ...
            if(startDt.getMonth() == endDt.getMonth()){
                p.setTitle('【' + Ext.Tools.FormatDate(new Date(), "M") + '】 ' + fmt(startDt, 'Y-m-d') + ' 至 ' + fmt(endDt, 'Y-m-d'));
            }
            else{
                p.setTitle('【' + Ext.Tools.FormatDate(new Date(), "M") + '】 ' + fmt(startDt, 'Y-m-d') + ' 至 ' + fmt(endDt, 'Y-m-d'));
            }
