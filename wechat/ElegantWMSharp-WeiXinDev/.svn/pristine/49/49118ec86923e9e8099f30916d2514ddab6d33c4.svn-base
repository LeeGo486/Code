...
{
    public class DLWXSiteAuthFilter : ActionFilterAttribute
    {
        ...
        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            if (!string.IsNullOrEmpty(filterContext.HttpContext.Request["sid"]) && !string.IsNullOrEmpty(filterContext.HttpContext.Request["oid"]))
            {
                ...
                //判断是否注册了（绑定）
                if (WMFactory.Wsrr.GetCustomerInfoCommonInterface("1", "DL", oid, "", "", "", "").Rows[0]["phone"].ToString() == "")
                {
                    if (filterContext.HttpContext.Request.IsAjaxRequest())
                    {
                    ...
                    {
                        filterContext.Result = new RedirectResult("~/DLWXSite/VIP/CreateVIP?sid=" + sid.ToString() + "&oid=" + oid + "&ReturnUrl=" + filterContext.HttpContext.Server.UrlEncode(filterContext.HttpContext.Request.RawUrl));
                        return;
                    }
                }
