  ...
  if id="" then
    ...
    if ds.eof then
      Err.Raise 50001,"create_indent.edit","对不起,没有找到该单据,请重新选择!"
			...
			setdate=ds("setdate")
			getdate=ds("getdate")
			backrate=cdbl(reNull(ds("backrate")))*100
...
case "create" '新增界面
  Call getGant("create",0,formid)
    ...
    getdate=get_date(dateadd("d",15,date))
	
sql="select isnull(bhrate,0) as bhrate from J_Merchant WHERE MerchantId='"&merchantid&"'"
ds.source = sql
ds.open
if ds.eof and ds.bof then
	backrate="0"
else
	backrate=cdbl(ds(0))*100
end if
ds.close
set ds=nothing
	
    backdate=get_date(dateadd("d",30,date))
    session("id")=indentid
...
<%sub show%>
<form action="?bar=<%=bar%>" method="post" name="form1" id="form1" onSubmit="return checkform();">
  <table width="480"  border="0" align="center" class="bar">
    <tr>
      <td height="19" align="center" class="f14">补货单</td>
    </tr>
  </table>
...
<tr class="label">
    ...
    <tr class="label">
      ...
      <td align="right">补货单号:</td>
      <td><input name="indent" type="text" id="indent" value="<%=indent%>" size="22" class="intra13" maxlength="22">
      </td>
    </tr>
<tr>
    ...
    <tr>
      ...
      <td><input name="backrate" type="text" id="backrate" value="<%=backrate%>" size="22" maxlength="10" <%if session("cf_pos_backrate_change") = false then response.write "readonly"%>>
        %</td>
    </tr>
    <tr>
...
sub save
  ...
  if backdate="" then backdate=get_date(dateadd("d",30,date))
  if cdbl(backrate)>0 then backrate = backrate/100
  ...
  if ds.eof then
	...
	"backdate,comment,d_type,cr_name,cr_date,get_depotid,parentid,chk_sure,sure,ov_sure)"& _
	 ...
	 qts(getdate) &","& qts(selltype) &","& backrate &",'"&backdate&"',"& qts(comment) &",2"& _
	...
	if bar="1" then
	  response.Write("window.location.href='ajax/2061_indent.asp?id="&indentid&"';")
	else
