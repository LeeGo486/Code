...
if seachid="" then
		sql="select a.outindentid,a.selltype,a.set_depotid,a.depotid,a.chk_sure,a.sure,a.chk_name,a.chk_date,a.sure_date,a.sure_name,"&_
		    "(select sum(b.nums) from d_outindentPROsub b where a.outindentid=b.outindentid) as sqnums,"& _
			"(select sum(c.nums*c.s_price) from d_outindentPROsub c where a.outindentid=c.outindentid) as sqsums,"& _
			...
			"case when isnull(a.backrate,0)=1 then '正常退货' else '特殊退货' end as backrate,Taxonomy from d_outindentpro a "&_
		...
		case 4
  		...
  		sql="select a.outindentid,a.selltype,a.set_depotid,a.depotid,a.chk_sure,a.sure,a.chk_name,a.chk_date,a.sure_date,a.sure_name,"&_
			...
			"(select e.d_name from j_depot e where a.depotid=e.depotid ) as getdptname, "&_
			"a.setdate,a.ov_sure,a.ov_name,a.ov_date,a.comment,a.cr_date,a.cr_name,"&_
			"case when isnull(a.backrate,0)=1 then '正常退货' else '特殊退货' end as backrate,a.Taxonomy from d_outindentpro a "&_
	  ...
	  if psprice then t.SetField "sqsums","申请金额","50",1
