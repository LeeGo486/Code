...
{ var str,obj,typ,nums;
  ...
  obj = chkclothing('2065_outindent_save.asp?date()',str);
  if(obj=='true')
  {
     var i=parseInt($("#calNum").text());
	 i=i+(typ*nums);
     $("#calNum").text(i.toString(10));
	 //getjson('GetDataJson.asp?date()','act=d_outindentset&id='+$("#id").val());
	 getRow();
  }
  else
  {
...
rs.source = "select chk_sure from d_outindentset where outindentid="&qts(id)
rs.open
if not rs.eof then
if rs(0)>0 then
...
<table width="100%" border="1" class="f14" bordercolordark="#FFEFEF">
  <thead>
    <tr align="center"><td>款 号</td><td>款式名称</td><td>颜 色</td><td>尺 码</td><td>数 量</td><td>单 价</td><td>总额</td></tr>
