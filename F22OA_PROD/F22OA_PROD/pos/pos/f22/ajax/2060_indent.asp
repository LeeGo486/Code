...
{ var str,obj,typ,nums;
  ...
  obj = chkclothing('2061_indent_save.asp?date()',str);
  if(obj=='true')
  {
     PlayScanSound();
	 ...
	 getjson('GetDataJson.asp?date()','act=d_indentbh&id='+$("#id").val());
  }
...
rs.source = "select chk_sure from d_indent where indentid="&qts(id)
rs.open
if not rs.eof then
if rs(0)>0 then
...
sql = "select a.depotid,b.d_name,a.crdate from d_indent a,j_depot b where a.indentid="&qts(id)&" and a.depotid=b.depotid"
rs.source = sql
rs.open
if rs.eof then
  response.End()
...
<table width="100%" border="0" class="f14">
  <tr>
    <td>单号: <%=id%> 供货地: <%=setdptname%>(<%=setdepotid%>) 制单日期: <%=cr_date%></td>
  </tr>
</table>
