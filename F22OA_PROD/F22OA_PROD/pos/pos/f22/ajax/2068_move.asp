...
{ var str,obj,typ,nums;
  ...
  obj = chkclothing('2068_move_save.asp?date()',str);
  if(obj=='true')
  {
     PlayScanSound();
	 ...
	 getjson('GetDataJson.asp?date()','act=d_moveget&id='+$("#id").val());
  }
  else if(obj=='flow'){
...
rs.source = "select getchk_sure from d_move where moveid="&qts(id)
rs.open
if not rs.eof then
if rs(0)>0 then
...
sql = "select a.set_depotid,b.d_name,a.setsure_date from d_move a,j_depot b where a.moveid="&qts(id)&" and a.set_depotid=b.depotid"
rs.source = sql
rs.open
if rs.eof then
  response.End()
