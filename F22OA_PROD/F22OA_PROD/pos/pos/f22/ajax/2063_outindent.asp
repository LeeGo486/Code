...
{ var str,obj,typ,nums;
  ...
  obj = chkclothing('2063_outindent_save.asp?date()',str);
  if(obj=='true')
  {
     PlayScanSound();
	 ...
	 getjson('GetDataJson.asp?date()','act=d_outindentpro&id='+$("#id").val());
  }
...
rs.source = "select chk_sure from d_outindentpro where outindentid="&qts(id)
rs.open
if not rs.eof then
if rs(0)>0 then
