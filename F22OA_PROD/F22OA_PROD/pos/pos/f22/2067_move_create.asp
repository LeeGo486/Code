...
function checkform(){
  ...
  with(document){
	...
	remark = trim(form1.comment.value);
...
case "edit" '打开界面
  if id="" then
    ...
    sql="Select a.moveid,a.move,a.set_depotid as setdepotid,b.d_name as dptname,a.get_depotid as getdepotid,c.d_name as getdptname, "&_
	"a.indentid,a.setdate,a.getdate,a.backrate,a.backdate,a.set_comment,a.selltype From d_move a,j_depot b,j_depot c "&_
	" Where a.set_depotid=b.depotid and a.get_depotid=c.depotid and a.moveid="&qts(id)
	'de sql
    ds.source = sql
    ds.open
    if not ds.eof then
...
<form name="form1" method="post" action="" onSubmit="return checkform();">
  ...
  <table width="480"  border="0" align="center" class="tcontent">
    ...
    <tr>
      ...
      <td><input name="move" type="text" id="move" value="<%=move%>" class="intra13" size="22" maxlength="22"></td>
    </tr>
    <tr>
      <td align="right">发货仓店:</td>
      <td><input name="set_depotid" type="text" id="set_depotid" value="<%=set_depotid%>" size="22" maxlength="10" readonly>
          <input type="button" name="Submit" value="..." onClick="selectMD('sm',0);" disabled>
          <input name="sdptname" type="text" class="tcontent" id="sdptname" 
          style="border:none;width:200px;" value="<%=setdptname%>" size="10" readonly></td>
    </tr>
    <tr>
      ...
      <td><input name="get_depotid" type="text" id="get_depotid" value="<%=get_depotid%>" size="22" maxlength="10" readonly>
          ...
          <input name="edptname" type="text" class="tcontent" id="edptname" style="border:none;width:200px;"
           value="<%=getdptname%>" size="10" readonly>
    ...
    <tr>
      ...
      <td><input name="setdate" type="text" id="setdate" onFocus="setday(this)" value="<%=setdate%>" size="22" maxlength="10" readonly></td>
    </tr>
    <tr>
...
<%sub save
  ...
  if ds.eof then
    Call getGant("create",0,session("formid"))
    '保存新增
	sql="insert into d_move(set_depotid,get_depotid,moveid,move,setdate,getdate,selltype,backrate," & _
	" backdate,set_comment,setcr_name,setcr_date,setchk_sure,set_sure,getchk_sure,get_sure) "& _
	 " values("& qts(set_depotid) &","& qts(get_depotid) &","& qts(moveid) &","& qts(move) &","& qts(setdate) &","& qts(setdate) &","& _
	...
	if bar="1" then
	  response.Redirect("ajax/2067_move.asp?id="&moveid+"&get_depotid="&get_depotid)
	else
	  response.Write("showModalDialog('f22_main.asp?id="&moveid&"&pksname=move&opt=move&numName=set_nums','es','dialogWidth:700px;dialogHeight:500px;center: yes;help:no;resizable:yes;status:no');")
