...
if show="Z" then
   sql="select sum(set_nums) as set_nums from d_msellsub where msellid="&qts(id)
else
sql = " select a.clothingid,a.colorid,a.sizeid,a.set_nums,a.x_price,a.j_cost,a.j_price,b.c_name"& _
	  "  "& _
	  " from d_msellsub as a ,j_color b"& _
	  " where a.colorid=b.colorid and a.msellid="&qts(id)
	  'de sql
end if
rs.source = sql
rs.open
...
if show="Z" then%>
<table width="100%" border="1" class="f14" bordercolordark="#FFEFEF"><tr height="80"><td align="center"><font size="8" color="#FF0000"><%=rs("set_nums")%></font></td></tr></table>
<%else%>
<table width="100%" border="1" class="f14" bordercolordark="#FFEFEF">
  ...
  <%dim rs1
    ...
    rs1.open "select sum(set_nums) as nums,sum(set_nums*j_price) as sums from d_msellsub where msellid='"&id&"'",cn
