...
function createbill(i){
 ...
 if(indentid!=""){
	...
	if(confirm('是否跳转到查询界面?')){window.location.href="search_frame.asp?jh=''&pagename=%B2%E9%D1%AF%B5%F7%B0%CE%B7%A2%BB%F5%B5%A5"
	}
 }else{
    alert("请先选择通知单!");
 }
...
<form name="form1" method="get" action="">
  <tr>
	...
	<input name="dtype" type="radio" value="1" <%if request.QueryString("dtype")="1" then response.Write("checked")%> >处理中&nbsp;&nbsp;
	<input name="dtype" type="radio" value="0" <%if request.QueryString("dtype")<>"1" then response.Write("checked")%>  >全部状态&nbsp;&nbsp;
   </td>
  </tr>
  <tr>
  <td>
	...
	<input type="button" name="bu3" value="生成调货单" onClick="createbill(30)">
