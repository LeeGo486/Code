...
if show="Z" then
   sql="select sum(nums) as nums from d_inventorysub where inventoryid="&qts(id)
else
sql = " select a.clothingid,a.colorid,a.sizeid,a.nums as nums,isnull(a.x_price,0) as x_price,isnull(a.j_price,0) as j_price,b.c_name"& _
	  " from d_inventorysub as a ,j_color b"& _
	  " where a.colorid=b.colorid and a.inventoryid="&qts(id)&" and isnull(a.nums,0)<>0"
end if
	  'de sql
rs.source = sql
rs.open
...
<table width="100%" border="1" class="f14" bordercolordark="#FFEFEF">
  ...
  <%dim rs1
    ...
    rs1.open "select sum(nums) as nums,sum(nums*x_price) as sums from d_inventorysub where inventoryid='"&id&"'",cn
