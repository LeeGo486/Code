...
if clothingid<>"" then
  sql = " select top 1 isnull(a.getchk_sure,0) as getchk_sure from d_move a,d_movesub b where a.moveid=b.moveid and a.moveid="&qts(id)&" and clothingid="&qts(clothingid)
  rs.source = sql
  rs.open
  if rs.eof then
...
response.Redirect("2068_move_barcode_sub.asp?id="&id&"&show="&show)
%>