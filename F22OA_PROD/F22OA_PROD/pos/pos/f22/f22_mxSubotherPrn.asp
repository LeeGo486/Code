...
Dim setdptid,getdptid,zdnum,zdsuredate,zdsurename,zdcomment,Taxonomy
...
<script   language="javascript" defer="defer">   
  ...
  try{
                ...
                //更改以下参数可以调整打印格式BEGIN
                //纸张高度(mm)
                eprint.pageHeight = 178.00;
                 //纸张宽度(mm)
                eprint.pageWidth = 210.00;
...
id = Trim(Request.QueryString("id"))
  ...
  if  pksname <> "outindentset" then
    sql = "Select Distinct t.boxid 序号, t.styleid 款式编号, jst.s_name 款式名称, jc.colorid 色号, jc.c_name 色名 "
  else
    sql = "Select Distinct t.boxid 序号, t.styleid 款式编号, jst.s_name 款式名称,jst.package 包装方式, jc.colorid 色号, jc.c_name 色名 " 
  end if 	
  ...
  Case "d_outindentprosub", "d_outindentchksub", "d_outindentsub"
    ...
    sql = sql & ", sum(t.nums) 数量, isnull(t.s_price,0) 结算价, sum(isnull(t.s_price,0)*nums) as 结算金额, isnull(t.x_price,0) 现价,sum(isnull(t.x_price,0) * nums) 现价金额 " & _
        " From " & TableName & " as t, j_style jst,j_color jc" & _
        " Where t." + FkName + "='" & id & "' And t.styleid=jst.styleid And t.colorid=jc.colorid " & _
        " Group By t.boxid,t.colorid,t.s_price,t.x_price,t.styleid,jst.s_name,jst.package,jc.colorid,jc.c_name " & _
        " Order By t.styleid "
 ...
 Case "d_movenoticesub" '通知单
  ...
  if session("formid")=2019 or session("formid")=2066 then
   ...
   elseif session("formid")=2065 then
        sql="select a."&setdptid&",b.d_name as setdname,a."&getdptid&",c.d_name as getdname,convert(char(10),a."&zdsuredate&",126) as suredate,a."&zdsurename&","&_
	     "(select sum(d."&zdnum&") from "&tn&" d where d."&pkname&"=a."&pkname&") as znums,"&_
		 "(select sum(d."&zdnum&"*d.s_price) from "&tn&" d where d."&pkname&"=a."&pkname&") as zsums,a.selltype,a."&zdcomment&" as comment,Taxonomy  "&_
		 "from d_"&pksname&" a,j_depot b,j_depot c where a."&setdptid&"=b.depotid and a."&getdptid&"=c.depotid and a."&pkname&"='"&id&"'" 
   else
     sql="select a."&setdptid&",b.d_name as setdname,a."&getdptid&",c.d_name as getdname,convert(char(10),a."&zdsuredate&",126) as suredate,a."&zdsurename&","&_
	     "(select sum(d."&zdnum&") from "&tn&" d where d."&pkname&"=a."&pkname&") as znums,"&_
	 ...
	 getdname="收货仓店："&rs("getdname")
        ...
        if session("formid")=2065 then
            Taxonomy="分类标准："&rs("Taxonomy")
        end if
	 znums="总数量："&rs("znums")
	 zsums="总金额："&rs("zsums")
  ...
  str="<table width=100% border=0 cellspacing=0 bordercolor=0 bordercolordark=0 style='font-size:12px'>"&_
	  ...
	  "<tr><td>"&selltype&"</td><td>"&getdname&"</td><td>"&zsums&"</td></tr>"

    if session("formid")=2065 then '分类标准
    	str=str&"<tr><td colspan=3>"&Taxonomy&"</td></tr>"
    end if

	str=str&"<tr><td colspan=3>"&comment&"</td></tr></table>"
  response.write str
  ...
  While Not fds.EOF
   ...
   For i = 1 To fds.Fields.Count - 3
	 ...
	 if fds.Fields(i).Name="序号" or fds.Fields(i).Name="款式编号" then
	    list(i)="合计"
	 elseif fds.Fields(i).Name="款式名称" or fds.Fields(i).Name="包装方式" or fds.Fields(i).Name="色号" or fds.Fields(i).Name="色名" or fds.Fields(i).Name="现价" or fds.Fields(i).Name="结算价" then
	    list(i)="0"
	 else
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="noprint">
  <tr> 
    ...
    <td><div align="center"> 
        <input type="button" name="print" value="打印此单" style="BACKGROUND-COLOR: #FFFFFF; BORDER-BOTTOM: 1px solid; BORDER-LEFT: 1px solid; BORDER-RIGHT: 1px solid; BORDER-TOP: 1px solid; FONT-SIZE: 9pt" onClick="javascript:pagePrint()">
	
      </div></td>
  </tr>
</table>
<script language="javascript" defer>
  window.setTimeout(pagePrint,0);
</script>