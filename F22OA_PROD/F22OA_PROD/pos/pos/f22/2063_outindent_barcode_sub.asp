...
if show="Z" then
   sql="select sum(nums) as nums from d_outindentprosub where outindentid="&qts(id)
else
sql = " select a.clothingid,a.colorid,a.sizeid,a.nums,a.x_price,a.cost,a.j_price,b.c_name "& _
	  " from d_outindentprosub as a ,j_color b"& _
	  " where a.colorid=b.colorid and a.outindentid="&qts(id)&" order by a.crdate desc"
	  'de sql
end if
rs.source = sql
rs.open
...
<table width="100%" border="1" class="f14" bordercolordark="#FFEFEF">
  ...
  <%dim rs1
    ...
    rs1.open "select sum(nums) as nums,sum(nums*x_price) as sums from d_outindentprosub where outindentid='"&id&"'",cn
