...
if show="Z" then
   sql="select sum(sq_tjnums) as nums from d_outindentsqsub where outindentsqid="&qts(id)
else
sql = " select a.clothingid,a.colorid,a.sizeid,a.sq_tjnums,a.x_price,a.j_cost,a.j_price,b.c_name"& _
	  "  "& _
	  " from d_outindentsqsub as a ,j_color b"& _
	  " where a.colorid=b.colorid and a.outindentsqid="&qts(id)&" order by a.crdate desc"
	  'de sql
end if
rs.source = sql
rs.open
...
<table width="100%" border="1" class="f14" bordercolordark="#FFEFEF">
  ...
  <%dim rs1
    ...
    rs1.open "select sum(sq_tjnums) as nums,sum(sq_tjnums*x_price) as sums from d_outindentsqsub where outindentsqid='"&id&"'",cn,1,1
	...
	sums_x_price = sums_x_price + cdbl(rs("x_price"))*rs("sq_tjnums")
