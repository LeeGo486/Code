...
{if(theform.depotid.value==''){alert('请选择订货仓库！');return false;}
 else
 {
...
<form action="2061_retailstockDH.asp" method="get" name="form2" target="is" onSubmit="return checkform2()">
    <tr>
      <td>订货方式:
        <input type="radio" name="act" value="0" checked="checked">按库存订货
        <input type="radio" name="act" value="1">按销售订货</td>
...
<form action="2061_retailstockDH.asp" method="get" name="form1" target="is">
    <tr>
      <td><font size=2>订货仓库</font>
      ...
      <td align=right><input type="button" name="Submit" value="确认订货" onClick="return checkform1(this.form)"> </td>
    </tr>
</form>	
</table>
...
<form action="2061_retailstockDH.asp" method="post" name="form3">
 <tr align=center bgcolor="#E9F6F5">
   ...
   <td><font size=2>订 货</font></td>
 </tr>
<%on error resume next
set rs=server.createobject("adodb.recordset")
if act="0" then
   sql1="select a.clothingid,a.styleid,d.s_name,d.x_price,a.colorid,e.c_name,a.sizeid,isnull(b.k_num,0) as kc,0 as xs into #retailstockdh1 "&_
   ...
   sql2="insert into #retailstockdh1 "&_
   ...
   sql3="select clothingid,max(styleid) as styleid,max(s_name) as s_name,avg(x_price) as x_price,max(colorid) as colorid,"&_
        "max(c_name) as c_name,max(sizeid) as sizeid,sum(kc) as kc,sum(xs) as xs into #retailstockdh2 from #retailstockdh1 group by clothingid"
   cn.execute "drop table #retailstockdh1"
   cn.execute "drop table #retailstockdh2"
   cn.execute sql1
   cn.execute sql2
   cn.execute sql3
else
   sql1="select a.clothingid,a.styleid,c.s_name,c.x_price,a.colorid,f.c_name,a.sizeid,isnull(e.k_num,0) as kc,isnull(b.nums,0) as xs into #retailstockdh2 "&_
	    "from j_clothing a,(select b.clothingid,sum(b.nums) as nums from d_retail a,d_retailsub b "&_
        "where a.retailid=b.retailid and a.sure>0 and a.depotid='"&session("dm").system_depotid&"' "&_
...
rs.open "select clothingid,styleid,s_name,x_price,colorid,c_name,sizeid,kc,xs from #retailstockdh2 order by clothingid",cn,1,1
i=1
if not rs.eof then
while not rs.eof%>
...
rs.close
'主单
no=GetId("d_indent","indentid","DH",session("dm").system_userid)
s="insert into d_indent (indentid,indent,MerchantId,setdate,sure,chk_sure,depotid,getdate,d_type,cr_name,chk_name,chk_date,"&_
  ...
  "('"&no&"','"&no&"','"&merchid&"',getdate(),0,0,'"&depotid&"',getdate(),1,'"&session("dm").system_username&"','"&session("dm").system_username&"',"&_
  "getdate(),getdate(),0,'"&session("dm").system_username&"',getdate(),getdate(),"&rate&",'订货',getdate()+30,'"&session("dm").system_depotid&"','"&parentid&"')"
cn.execute s

for i=0 to ubound(clothing)
  ch=split(clothing(i),"^")
  'gp.open "t18tj_GetClothingPrice 2,'"&session("dm").system_depotid&"','"&trim(ch(0))&"','',1",cn
  gp.open "t18tj_GetClothingPrice 3,'"&session("dm").system_depotid&"','"&trim(ch(0))&"','',0",cn
...
set rs=nothing
set gp=nothing
response.write("<br><br><center>订货单 "&no&" 已确认，总订数 "&nums&",可以到单据订货单查询界面查询！</center>")
End Sub%>
</body>
</html>