...
if clothingid<>"" then
  ...
  sql = "select * from d_outindentsetsub where outindentid="&qts(id)&" and clothingid="&qts(clothingid)
  rs.source = sql
  rs.open
  if rs.eof then
...
response.Redirect("2065_outindent_barcode_sub.asp?id="&id&"&show="&show)
%>