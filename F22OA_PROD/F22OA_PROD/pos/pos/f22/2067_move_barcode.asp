...
sql = "select a.getdepotid,a.getdptname as get_depotname,crdate from v22d_move a where moveid="&qts(id)
rs.source = sql
rs.open
if rs.eof then
  response.End()
else
  setdepotid=rs("getdepotid")
...
<table width="100%" border="0">
  <tr>
    <td><form action="2067_move_barcode_save.asp" method="get" name="form1" target="is" onSubmit="return chkform();">
      条码:
    ...
    <td><input name="bu1" type=button value="显示明细" onclick="javascript:document.form1.show.value='M';document.all.is.src='2067_move_barcode_sub.asp?id=<%=id%>&show=M';">   
	    <input name="bu2" type=button value="显示总数" onclick="javascript:document.form1.show.value='Z';document.all.is.src='2067_move_barcode_sub.asp?id=<%=id%>&show=Z';">
	    <input name="bu3" type="button" value="返回查询界面" onClick="javascript:location.href='search_frame.asp?action=search'">
	</td>
  </tr>
</table>
<iframe  name="is" src="2067_move_barcode_sub.asp?<%=request.ServerVariables("QUERY_STRING")%>" width="100%" height="86%" frameborder="0"></iframe>
