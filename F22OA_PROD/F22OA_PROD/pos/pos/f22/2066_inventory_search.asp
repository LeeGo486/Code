...
//		if ( ('<%=request.QueryString("ext")%>'=='1') || ('<%=request.QueryString("ext")%>'==''))
//		{
...
'盘点单
ss=ss&" and left(a.inventoryid,2)='PD' "
...
if seachid="" then
	 ...
	 sql="SELECT a.inventoryid,a.depotid,b.d_name as dptname,case when a.d_type=0 then '全局' else '局部' end as d_type,"&_
	  ...
	  t.SetField "nums","盘点数量","50",1
		if psprice then t.SetField "Sums","盘点金额","60",1
	  t.SetField "cynums","差异数量","50",2
		if psprice then t.SetField "cysums","差异金额","60",2
