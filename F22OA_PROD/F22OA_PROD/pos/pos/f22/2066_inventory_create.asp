...
case "delmany"
  Call getGant("del",0,session("formid"))
  sql="delete from d_inventory where inventoryid in ("+(id)+")"
  cn.execute sql
  set cn=nothing
  call close("删除盘点单成功!")
case "save"
...
case "create"
  ...
  inventoryid=getid("d_inventory","inventoryid","PD",UserId)
...
<input name="bar" type="hidden" id="bar" value="<%=bar%>">
  ...
  <table width="480"  border="0" align="center" class="tcontent">
    ...
    <tr>
      <td align="right">&nbsp;</td>
      <td><input name="stype" type="radio" value="0" <%if cdbl(d_type)=0 then response.write("checked") end if%>>
      全局盘点
        <input type="radio" name="stype" value="1" <%if cdbl(d_type)=1 then response.write("checked") end if%>>
      局部盘点</td>
    </tr>
    <tr>
  ...
  <table width="480"  border="0" align="center" class="tcontent">
    <tr>
      ...
      <input name="Submit" type="button" value="取消(X)" onClick="window.location='2066_index.asp';" accesskey="x"></td>
    </tr>
  </table>
</form>
<%end sub%>
...
sub save
  ...
  if ds.eof then
	...
	if bar="1" then
	  response.Write("window.location.href='ajax/2066_inventory.asp?id="&inventoryid&"';")
	else
	response.Write("showModalDialog('f22_main.asp?id="&inventoryid&"&pksname=inventory&opt=inventory&numName=nums','es','dialogWidth:700px;dialogHeight:500px;center: yes;help:no;resizable:yes;status:no');")
