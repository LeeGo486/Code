...
function initAd(){
		...
		var file=form2.mefile.value;
		var extname=file.substring(file.lastIndexOf(".")+1,file.length);
		
		extname = extname.toLowerCase();
		if (extname!="gif"&&extname!="bmp"&&extname!="jpeg"&&extname!="jpg"&&extname!="png")
		{
		   alert("上传照片格式不正确，请上传图片！");
...
<form method="post" action="vip_Imgprocess.asp" enctype="multipart/form-data" name="form2" onSubmit="initAd();">
  <input type=file name="mefile" size="8" class=text>
  <input type="submit" name="btnup" size="4" value="上传照片"  alt="上传的图片大小不能超过500k,图片比例100x135最佳" class="text" >
