...
document.writeln('<iframe id=meizzDateLayer Author=wayx   frameborder=0   style="position:   absolute;   width:160px; height:211px;   z-index:   9998;   display:   none"></iframe>');
strFrame='<style>';
...
for(j=0;j<5;j++){ 
	...
	for (var i=0;i<7;i++){
	strFrame+='<td width=20 height=18 id=meizzDay'+n+' style="font-size:12px" Author=meizz onclick=parent.meizzDayClick(this.innerText,0)></td>';
	n++;
	}
	strFrame+='</tr>';
}
strFrame+='   <tr align=center Author="wayx">';
for   (i=35;i<39;i++){
	strFrame+='<td width=20 height=18 id="meizzDay'+i+'" style="font-size:12px" Author=wayx onclick="parent.meizzDayClick(this.innerText,0)"></td>'
}
...
function   GetDOW(day,month,year)           //求某天的星期几
    {
        var   dt=new   Date(year,month-1,day).getDate()/7;   return   dt;
    }
...
function   meizzToday()     //Today   Button
    {
				var d= new Date();
        meizzTheYear = d.getFullYear();
        meizzTheMonth = d.getMonth()+1;
        today=d.getDate() ;
        //meizzSetDay(meizzTheYear,meizzTheMonth);
        if(outObject){
