...
<%
  with response
	...
	rs.open "select isnull(count(*),0) as cnt from o_threads where crname='"&request.cookies("username")&"' and isov=0",conn,1,1
	...
	Sql="select a.typeid,max(b.typename) as typename from o_threads a,o_type b "&_
	    "where a.typeid=b.typeid and a.crname='"&request.cookies("username")&"' and a.isov=0 group by a.typeid"
    rs.open Sql,conn,1,1
	set tmp=server.createobject("adodb.recordset")
	while not rs.eof
 	  tmp.open "select isnull(count(*),0) as cnt from o_threads where crname='"&request.cookies("username")&"' and isov=0 and typeid="&rs("typeid"),conn,1,1
	  .write("<tr onMouseMove=""this.bgColor='#FFFFCC'"" onMouseOut=""this.bgColor=''"">")
...
select case typeid
  case 0 Sql="select a.*,c.statename,d.username as clusername,c.color,"&_
             ...
             "where a.typeid=b.typeid and a.stateid=c.stateid and a.isov=0 and a.crname='"&request.cookies("username")&"' "&_
