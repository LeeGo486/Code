...
<tr valign="middle">
   <td nowrap onMouseMove="this.bgColor='#aaaaff'" onMouseOut="this.bgColor='cornsilk'"
			align="center" bgcolor="cornsilk" style="CURSOR: hand" onClick="location='flowallnolist.asp?tname=<% =escape("所有未完成记录")%>'" vAlign="baseline">
			<img src="../icon/menu/diary.gif" width="16" height="16"> 所有未完成记录</td>
   <td nowrap onMouseMove="this.bgColor='#aaaaff'" onMouseOut="this.bgColor='cornsilk'"
			align="center" bgcolor="cornsilk" style="CURSOR: hand" onClick="location='flowallyeslist.asp?tname=<% =escape("所有已完成记录")%>'" vAlign="baseline">
			<img src="../icon/menu/diary.gif" width="16" height="16"> 所有已完成记录</td>
</tr>
</table>
...
<input type="submit" name="submit" value=" 查 询 " class=CommonButton>
</td></tr>
</form>
</table>
...
if fdate<>"" and ldate<>"" then 
  with response
	...
	rs.open "select isnull(count(*),0) as cnt from o_threads where isov=0 "&_
	...
	Sql="select a.typeid,max(b.typename) as typename from o_threads a,o_type b "&_
	    "where a.typeid=b.typeid and a.isov=0 "&_
	...
	while not rs.eof
 	  tmp.open "select isnull(count(*),0) as cnt from o_threads where isov=0 "&_
...
select case typeid
  case 0 Sql="select a.*,c.statename,d.username as clusername,c.color,"&_
             ...
             "where a.typeid=b.typeid and a.stateid=c.stateid and a.isov=0 "&_
			 "and a.clname=d.userid and convert(char(10),a.crdate,126) between '"&fdate&"' and '"&ldate&"' order by c.orders desc"
  case else Sql="select a.*,c.statename,d.username as clusername,c.color,"&_
                "case when convert(char(10),a.crdate,126)=convert(char(10),getdate(),126) then 1 else 0 end as shownew "&_
                "from o_threads a,o_type b,o_state c,j_user d "&_
                "where a.typeid=b.typeid and a.stateid=c.stateid and a.isov=0 "&_
				"and a.typeid="&typeid&" and a.clname=d.userid and convert(char(10),a.crdate,126) between '"&fdate&"' and '"&ldate&"' order by c.orders desc"
end select
rs.open Sql,conn,1,1
While not rs.eof
  ...
  response.write("<td align=center><a href=""javascript:if(confirm('确定删除吗？')){document.location='?act=right&typeid="&rs("typeid")&"&fdate="&fdate&"&ldate="&ldate&"&tid="&rs("tid")&"'}"">删除</a></td></tr>")
rs.movenext
Wend
rs.close
end if%>
