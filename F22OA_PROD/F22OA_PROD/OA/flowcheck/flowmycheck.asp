...
if fdate<>"" and ldate<>"" then 
  with response
	...
	rs.open "select isnull(count(*),0) as cnt from o_threads where crname<>'"&request.cookies("username")&"' "&_
	        "and convert(char(10),crdate,126) between '"&fdate&"' and '"&ldate&"' "&_
			"and tid in (select tid from o_threadsub where rpname='"&request.cookies("username")&"' group by tid)",conn,1,1
	...
	Sql="select a.typeid,max(b.typename) as typename from o_threads a,o_type b "&_
	    "where a.typeid=b.typeid and a.crname<>'"&request.cookies("username")&"' "&_
		"and convert(char(10),a.crdate,126) between '"&fdate&"' and '"&ldate&"' "&_
		"and a.tid in (select tid from o_threadsub where rpname='"&request.cookies("username")&"' group by tid) group by a.typeid"
    rs.open Sql,conn,1,1
	set tmp=server.createobject("adodb.recordset")
	while not rs.eof
 	  tmp.open "select isnull(count(*),0) as cnt from o_threads where crname<>'"&request.cookies("username")&"' "&_
	           "and typeid="&rs("typeid")&" and convert(char(10),crdate,126) between '"&fdate&"' and '"&ldate&"' "&_
			   "and tid in (select tid from o_threadsub where rpname='"&request.cookies("username")&"' group by tid)",conn,1,1
	  .write("<tr onMouseMove=""this.bgColor='#FFFFCC'"" onMouseOut=""this.bgColor=''"">")
...
select case typeid
  case 0 Sql="select a.*,c.statename,d.username as clusername,c.color,"&_
             ...
             "where a.typeid=b.typeid and a.stateid=c.stateid and a.crname<>'"&request.cookies("username")&"' "&_
			 "and a.lastname*=d.userid and convert(char(10),a.crdate,126) between '"&fdate&"' and '"&ldate&"' "&_
			 "and a.tid in (select tid from o_threadsub where rpname='"&request.cookies("username")&"' group by tid) "&_
  ...
  case else Sql="select a.*,c.statename,d.username as clusername,c.color,"&_
				...
				"and a.tid in (select tid from o_threadsub where rpname='"&request.cookies("username")&"' group by tid) order by a.crdate desc,c.orders desc"
end select
rs.open Sql,conn,1,1
While not rs.eof
