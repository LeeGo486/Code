<%@ page language="C#" autoeventwireup="true" inherits="buyproduct_manager_Track_BP_TrackIngList, App_Web_bp_trackinglist.aspx.40cff5d2" %>
...
GB_myShow = function(caption, url, /* optional */ height, width, callback_fn) {
    ...
    return win.show(url);
           ...
           function GoUrlShowAddTrack(pid,sid) 
           {
		      var url="BP_TrackDetail.aspx?SupmaterID="+pid+"&SupplyerID="+sid
	          LoadWindow(url,860,653);
	          document.forms[0].submit();
	       }
	       function GoUrlShowOverTrack(pid) 
           {
		      var url="BP_TrackOver.aspx?SupmaterID="+pid
		      question = confirm("你确认要结案吗，结案后将不可再编辑跟进");
		              
		              if(question)
		              {
		                LoadWindow(url,660,353);
	                    document.forms[0].submit();
		              }
		              else
		              {
...
<uc1:uc_pagehead id="Uc_pagehead1" runat="server" />
   ...
   <table   width="98%" class="tbsh">
       ...
       <td> <asp:CheckBox ID="CheckBoxAll" runat="server"  Text="所有跟进中" /></td>
    ...
    <tr>
    <td>
        目前状态:超时为红色<span style="background-color:Red" ><font color="red"> 超时为红色</font></span><!--，不超时为绿色<span style="background-color:green" ><font color="green"> 超时为绿色</font></span>-->
        <br />
        应结案状态:行为黄色<span style="background-color:Yellow" ><font color="Yellow"> 超时为红色</font></span>
        </td>
    </tr>
    <tr>
    <td align="left">
                        ...
                        <ItemTemplate>
                          <img src="../../../icon/go3.gif" border="0" />
                           <asp:LinkButton ID="lbtnAdd" runat="server" CausesValidation="false" 
                                  OnClientClick='<%# EidtTrackAdd(Eval("SupmaterID").ToString(),Eval("SupplyerID").ToString()) %>'
                        CommandName="Check" Text='<%# Eval("TrackName").ToString()==""?"尚未开始":  Eval("TrackName").ToString()%>' >
                         </asp:LinkButton>
                           
                        </ItemTemplate>
                    </asp:TemplateField>
                        <asp:TemplateField SortExpression="isover" HeaderText="结案状态" >
                       <ItemTemplate>
                           <asp:Label ID="Lblisover" runat="server" Text='<%# Eval("isover").ToString()%>'></asp:Label>
                        </ItemTemplate>
                          <ItemStyle CssClass="hidden" />
                            <HeaderStyle CssClass="hidden" />
                            <FooterStyle CssClass="hidden" />
                    </asp:TemplateField>
                     <asp:TemplateField HeaderText="结案">
                        <ItemTemplate>
                          <img src="../../../icon/go3.gif" border="0" />
                           <asp:LinkButton ID="lbtnover" runat="server" CausesValidation="false" 
                                  OnClientClick='<%# EidtTrackOver(Eval("SupmaterID").ToString()) %>'
                        CommandName="Check" Text='结案' >
                         </asp:LinkButton>
