<%@ page language="C#" masterpagefile="~/cashcard/main/sys/SysMaster.master" autoeventwireup="true" inherits="web_crcolumnsub, App_Web_web_crcolumnsub.aspx.9597e00c" title="建立权力组" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ctplhder" Runat="Server">
    ...
    <script id="Infragistics" type="text/javascript" charset="gb2312">
<!--
var cnt="0";
...
function UltraWebGrid1_BeforeRowDeletedHandler(gridName, rowId)
{
    ...
    var aspname=row.getCellFromKey("aspname").getValue();	
    var comment=row.getCellFromKey("comment").getValue();	
    var parentaspxid=getclassid();	

    if(confirm("真的要删除当前记录吗? 警告：所有子项目将同时被删除！->"+aspxid))
    {
       var url="web_crcolumnsubxml.aspx?SaveType=3&aspxid="+aspxid+"&readme="+escape(readme)+"&comment="+escape(comment)+"&parentaspxid="+parentaspxid+"&aspname="+escape(aspname);
...
function user_editsave(gridName)
{
    ...
    if (aspname==null) 
     ...
     if (aspxid==null)
     {
       var url="web_crcolumnsubxml.aspx?SaveType=1&aspxid="+aspxid+"&readme="+escape(readme)+"&comment="+escape(comment)+"&parentaspxid="+parentaspxid+"&aspname="+escape(aspname);
     }
     else
     {
       var url="web_crcolumnsubxml.aspx?SaveType=2&aspxid="+aspxid+"&readme="+escape(readme)+"&comment="+escape(comment)+"&parentaspxid="+parentaspxid+"&aspname="+escape(aspname);
     }
    return update_srv(url);
...
function UltraWebGrid1_BeforeRowUpdateHandler(gridName, rowId)
{
    ...
    if (row!=null)
    {
  	   var classname=row.getCellFromKey("readme").getValue();
  	   if (classname==null)
  	   {
  	      showmsg("项目名称不能为空！");
  	      return 1;
         ...
         showmsg("正在处理..."+classname);　   
  	     if (user_editsave(gridName)==0)
  	     {
  	       if (cnt!="0")
  	       {
  	          row.getCellFromKey("aspxid").setValue(cnt);
  	          cnt="0";
  	       }
  	       return 0;
  	     }
...
// -->
    ...
    <igtbl:UltraWebGrid ID="UltraWebGrid1" runat="server" Browser="Xml" Height="366px" >
        <Bands>
            <igtbl:UltraGridBand>
        ...
        <DisplayLayout AllowAddNewDefault="Yes" AllowColSizingDefault="Free" AllowDeleteDefault="Yes"
            ...
            <FrameStyle BackColor="White" BorderColor="Blue" BorderStyle="Inset" BorderWidth="1px"
                Font-Names="宋体" Font-Size="9pt" Height="366px">
            </FrameStyle>
...
<BorderDetails ColorTop="White" WidthLeft="1px" WidthTop="1px" ColorLeft="White"></BorderDetails>
    ...
    <div><asp:Label ID="lblrow" runat="server" Width="106px"></asp:Label>
    <asp:Label ID="lblFeedBack" runat="server" ForeColor="Red" Width="277px"></asp:Label>
    </div> 
</asp:Content>
