...
select @cardid=cardid,@insums=-insums,@bounty=-bounty,@dbname=dbname from loggingid
 where loggingid=@loggingid

insert into d_logging(cardid,insums,rate,bounty,crname,crdate,sure_date,comment,depotid,dbname)
select cardid,-insums,rate,-bounty,crname,getdate(),getdate(),comment,depotid,dbname from loggingid
 where loggingid=@loggingid
