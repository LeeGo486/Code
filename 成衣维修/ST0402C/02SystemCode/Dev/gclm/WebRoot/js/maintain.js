     ...
     //-------------------------------------------
      	...
      	//var guid='<%out.print(request.getParameter("guid"));%>';
      	 $.post(	'/gclm/rest/getDetail/S2',
          		...
          		//详细表单数据
          					...
          					if(null!=e[0].dplx){
          						if(e[0].dplx=="1"){document.getElementById("dplx").value = "自营";}
          						else if(e[0].dplx=="2"){document.getElementById("dplx").value = "加盟";} 
          					};
          					...
          					//if(null!=e[0].sldgxm){$('#sldgxm').combogrid('setValue',e[0].sldgxm);};
          					if(null!=e[0].sldg){$('#sldg').attr('value',e[0].sldg);};
          					if(null!=e[0].sldgxm){$('#sldgxm').attr('value',e[0].sldgxm); };
      ...
      function saveMaintain(id,zt){
    	  $('#'+id).linkbutton('disable');  
      	 ...
      	 zbfh_fhdz ,zbfh_shr ,zbfh_shrdh ,zbfh_bz,sldgxm;
      	...
      	if('save0123456'==id || 'save00'==id){
      		 var g = $('#sldg').combogrid('grid');	// get datagrid object
      	...
      	// alert(ztold+"ssss"+zt);return;
       	...
       	//品鉴判断
      		...
      		{//登记
      			...
      			if(  nx=='1' && $('#wxgys').combogrid('getText')!=""){
      				wxr = $('#wxgys').combogrid('getText');
      			}else if(nx=='0' && $('#wxr2').combogrid('getText')!=""){
             ...
             function(wxdh1){
			     ...
			     document.getElementById("wxdh").value=wxdh;
      			...
      			'wxr':wxr,'wxsj':wxsj,'zbfh_ysgs':zbfh_ysgs ,'zbfh_ysdh':zbfh_ysdh ,'zbfh_shdz':zbfh_shdz ,
      	     	'zbfh_fhdz':zbfh_fhdz ,'zbfh_shr':zbfh_shr ,'zbfh_shrdh':zbfh_shrdh ,'zbfh_bz':zbfh_bz,'gysbh':gysbh,'gysmc':gysmc,'wxjefw':wxjefw,'sldgxm':sldgxm
      	     	};
      			
      			$.post(	'/gclm/rest/saveRegister/save',
             		params,
             		function(reinfo){
      				var e = eval("(" + reinfo + ")");
	      				//
	      				...
	      				if("save0123456"==id||"save00"==id){
	      					...
	      					if("save07"==id||"save08"==id){
	          					...
	          					if(zt==3 && e[0].flag != undefined && "error"== e[0].flag){ 
	          						zt = 1;
	          					}
	          				} 
	      					//保存成功    列表 数据是否刷新？  
          ...
          function getdpvalue(depotid)
          {
      		...
      		$.post(	'/gclm/rest/getFilterdp/withCom',
     ...
     //---------------------------------------------
      ...
      //顾客维修改变子框的权限输入
		...
		function getNxwxje(xxd){
			$('#wxjefw').validatebox({});
			$.post('/gclm/rest/getTybmb/wxjefw',
	    ...
	    //详细页面 根据url参数 初始化页面 
	      function getDetailView(guid,zt){
	      	//var guid='<%out.print(request.getParameter("guid"));%>';
	      	 $.post(	'/gclm/rest/getDetail/S2',
	      			{'guid':guid},function(detail) {
	      					var e = eval("(" + detail + ")");
	          		...
	          		//详细表单数据
	          					...
	          					if(null!=e[0].dpbm){$('#dpbm').attr('value',e[0].dpbm);};
	          					if(null!=e[0].dpmc){$('#dpmc').attr('value',e[0].dpmc);};
