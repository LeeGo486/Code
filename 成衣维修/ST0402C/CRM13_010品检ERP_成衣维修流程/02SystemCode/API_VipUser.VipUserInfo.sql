...
/****** Object:  StoredProcedure [API_VipUser].[VipUserInfo]    Script Date: 04/23/2014 14:44:30 ******/
SET ANSI_NULLS ON
GO
...
CREATE proc [API_VipUser].[VipUserInfo]
...
--设置帮助
	if @InputFormat in ('AML','【,】') 
		begin
			...
			set @HelpString=
			'【操作类型,GET】【实体,VIP_USER】【返回内容,必填】【条件内容,】【《W》VIP卡号,】 '			
...
--设置默认值
	...
	begin
		...
		set @MapString = 
			'【客户ID,				VIPID~VIPID,							String,'														     +'】' + 
            '【客户姓名,			VIPNAME~VIPNAME,						String,'														     +'】' + 
            '【客户手机号码,		MOBTEL~MOBTEL,							String,'														     +'】' + 
            '【《W》VIP卡号,		VIPCODE,								String,'+ isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《W》VIP卡号'),'')	      +'】' 
		...
		if @WhereFieldList='' 
		begin
			Set @SqlString='SELECT  ' +  @SelectFieldList + ' FROM VIP_USER 
 
		 WHERE  1=1 ORDER BY VIPCODE ' 
