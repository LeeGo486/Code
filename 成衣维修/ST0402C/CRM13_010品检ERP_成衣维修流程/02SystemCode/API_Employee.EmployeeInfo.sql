...
/****** Object:  StoredProcedure [API_Employee].[EmployeeInfo]    Script Date: 04/23/2014 14:42:07 ******/
SET ANSI_NULLS ON
GO
...
CREATE proc [API_Employee].[EmployeeInfo]
...
--设置帮助
	if @InputFormat in ('AML','【,】') 
		begin
			...
			set @HelpString=
			'【操作类型,GET】【实体,J_EMPLOYEE】【返回内容,必填】【条件内容,】【《W》店铺ID,】 '			
...
--设置默认值
	...
	begin
		...
		set @MapString = 
			'【客户经理ID,				EMPLOYEEID~EMPLOYEEID,	                        String,'														     +'】' + 
            '【客户经理姓名,				NAMES~NAMES,			                String,'														     +'】' + 
            '【客户经理电话,				TELEPHONE~TELEPHONE,			                String,'														     +'】' + 
		...
		if @WhereFieldList='' 
		begin
			Set @SqlString='SELECT  ' +  @SelectFieldList + ' FROM J_EMPLOYEE 
