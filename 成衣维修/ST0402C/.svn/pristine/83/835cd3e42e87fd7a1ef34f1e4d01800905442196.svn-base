...
@Path(value = "/rest")
@Service
public class downloadRest implements ServletResponseAware
{
	/*
	 * 
	 *  测试类 未正式使用
	...
	@POST
	@Path(value = "/getExcelList/{message}")
	 public String outPut(@PathParam(value = "message") String message,@Context HttpServletRequest request,@Context HttpServletResponse response){
		//导出数据查询条件
		this.response = response;
				int page = 1;
				int maxResult = 200;
	...
	public void setResponseHeader() {
		try {
			...
			response.setHeader("Content-Disposition", "attachment;filename="
					+ java.net.URLEncoder.encode("导出数据.xls", "UTF-8"));
			// 客户端不缓存
	...
	/** 导出数据 */
	private void exportExcel(OutputStream os,List<MaintainRegister> list) throws IOException {
