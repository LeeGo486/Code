  ...
  function isExistKH(){
	...
	xmlhttp.onreadystatechange=function() {
		if (xmlhttp.readyState==4 && xmlhttp.status==200){
    		...
    		if(isExists=='0'|| isExists==null || isExists=='' || isExists=='null'){
    		    alert('款号不存在,请输入正确款号！');
    		...
    		if(isExistsArray[2]==null ||isExistsArray[2]==""){
...
function uploadImageFile(){
	if(document.getElementById("dh").value==null || document.getElementById("dh").value==''){
		alert("必须先保存维修登记表！");
		return;
	}
...
<body>
	...
	<html:form action="/eidtMaintenanceBill" styleId="eidtMaintenanceBillForm" method="post" enctype= "multipart/form-data" >
		...
		<div align="right" style="width: 80%"> <html:button property="save_sumit" onclick="save_submit();" 
			 	style="font-size: 13px; border-style: outset; ">保存维修单</html:button></div>		
		...
		<div style="height: 70%; width: 90%; OVERFLOW-Y: auto">
			...
			<table class="table_thin">
					...
					<tr>
						...
						<td id="SLDGTD" class="td_enabled"> 
					 		<select  id="SLDG" name="SLDG" onchange="getEmployeeTel()" class="text_enabled"> </select>
						</td>
					...
					<tr >
						...
						<td colspan="5"  class="td_enabled" > 
							<html:textarea property="BZXX" styleClass="text_enabled" style="height: 30px"></html:textarea>
						</td>
					</tr>
			...
			<br><br>
			<div align="right" style="width: 90%"> <html:button property="save_sumit" onclick="save_submit();" 
			 	style="font-size: 13px; border-style: outset; ">保存维修单</html:button></div>
		</div>
