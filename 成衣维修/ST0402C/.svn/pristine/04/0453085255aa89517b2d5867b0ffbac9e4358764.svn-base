...
function fileUtil_UploadFile(formId,savepath,filename,datagridId,billType,createName, isFileLinkId) {
	$('#'+formId).form('submit', {
		async: false,
		url:'/expenseServer/resteasy/file/upload?filename='+encodeURIComponent(filename)+'&savepath='+encodeURIComponent(savepath),
...
function fileUtil_DownloadFile(filename,path){
	var url='/expenseServer/resteasy/file/download?filename='+path+filename;
	$.ajax({
 	    url:url,
 	    type: 'get',
 	    async: false,
...
function fileUtil_DownloadFile1(filename,path){
	var url='/expenseServer/resteasy/file/download?filePath='+path+filename +'&fileName='+filename 
	+'&s='+new Date().getTime();  
