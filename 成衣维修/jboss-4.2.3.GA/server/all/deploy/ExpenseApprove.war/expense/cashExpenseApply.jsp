...
<script type="text/javascript">
  ...
  var subjectMoneyIsRight = '0';
	...
	function modifyExpense(modifyFlag,obj,desc){
		...
		xmlhttp.open("post","ajax.ajaxServlet",true);
		xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		xmlhttp.send("action=addCashExpense&modifyFlag="+modifyFlag+"&mapKey="+objIDs[0]+"&field="+objIDs[1]+"&fieldValue="+value);
		handleThread++;
	}
  ...
  function changeAllCost(id,name,obj){
	...
	function addExpense(modifyFlag,tableID){
		...
		if(tableID=='100'&&arrayTr.length==14){
			alert('费用用途最多只允许填写12条记录，如果超过请再申请！');
			return;
		}
		...
		xmlhttp.open("post","ajax.ajaxServlet",true);
		xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		xmlhttp.send("action=addCashExpense&modifyFlag="+modifyFlag+"&mapKey="+newSerial+'&SN='+newSN);
		handleThread++;
	...
	function deleteExpense(modifyFlag,tableID){
		...
		xmlhttp.onreadystatechange=function() {
				...
				if(rs=='1'){
					...
					for(var i=0;i<arrayTr.length;i++ ){
						...
						if($(thisTr).find('input').eq(1).attr('checked')){
							...
							}else if(thisTrID.substring(0,1)=='2'){
			...
			xmlhttp.open("post","ajax.ajaxServlet",true);
			xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			xmlhttp.send("action=addCashExpense&modifyFlag="+modifyFlag+"&mapKey="+mapKeyString);
  	...
  	function checkMustFill(){
  		...
  		if($('#mobilephone').attr('value')==null || $('#mobilephone').attr('value')==''){
  			alert('移动电话不能为空！');
  			return false;
  		}
  		if($('#leadername').attr('value')==null || $('#leadername').attr('value')==''){
  			alert('上司不能为空！');
  			return false;
  		}
  		
  		var hiddenPurposeTotalCost = $('#hiddenPurposeTotalCost').attr('value');
  	...
  	function initForm(){
  		...
  		if($('#result').attr('value')=='1'){
  			alert('单号为'+$('#cashExpenseNo').attr('value')+'申请单保存成功');
  		}
 ...
 function showBudgt(){
	...
	<div id="mainDiv" style="width: 95%">
		...
		<html:form action="/cashExpenseApply" styleId="cashExpenseForm" method="post" >
			...
			<table width="100%" border="0" cellpadding="2" cellspacing="1"
				...
				<tr>
					...
					<td class="td_text" width="19%" >
						<html:text property="applyStateName" styleClass="text_read_red"></html:text>
					</td>
					...
					<td class="td_text" colspan="3">						
						<html:text styleId="cashExpenseNo" property="cashExpenseNo" styleClass="text_read_red"></html:text>					
					</td>
		...
		<div
			style="position: relative; display: inline-block; width: 100%; height: 70px;">
			<img alt="" src="<%= path%>/img/img1.JPG"
				style="position: absolute; z-index: -1; left: 0; top: 0; width: 100%; height: 20">
			...
			<table width="100%" border="0" cellpadding="2" cellspacing="1"
				...
				<tr>
					...
					<td class="td_text" width="19%">						
						<html:text property="applyDate" styleClass="text_read_common" readonly="true" style="width:95%"></html:text>
					</td>
					...
					<td class="td_text" width="19%">
						<html:text styleId="sendInvoiceDate" property="sendInvoiceDate" onclick="setday(this)" readonly="true" style="width:95%"></html:text>
					</td>
					...
					<td class="td_text" width="19%">
						<html:text styleId="bankAccount" property="bankAccount"  styleClass="text_write_common" onchange="validateBankCode()"></html:text>
					</td>
				...
				<tr>
					...
					<td class="td_text">
						<html:text styleId="budget" property="budget" styleClass="text_read_common"></html:text>
					</td>
					...
					<td class="td_text">
						<html:text styleId="usedAmount" property="usedAmount" styleClass="text_read_common"></html:text>
					</td>
				...
				<tr>
					...
					<td class="td_text">						
						<html:text property="telephone" styleClass="text_write_common" style="width:95%"></html:text>
					</td>
					...
					<td class="td_text">
						<html:text styleId="mobilephone" property="mobilephone" styleClass="text_write_common" style="width:95%"></html:text>
					</td>
					...
					<td class="td_text">
						<html:text styleId="leadername" property="leadername" styleClass="text_write_common" style="width:95%"></html:text>
					</td>
		...
		<div >
			...
			<table id='200' width="100%" border="0" cellpadding="2" cellspacing="1"
				...
				<tr id="2000-tr" style="display: none">
					...
					<td class="td_text_center">
						<input id="2000-shopCostCenterCode" name="店铺成本中心" type="text" value="" class="text_write_common" onchange="modifyExpense('1',this,'店铺成本中心');getShopNameByCS(this)">
					</td>
					
					<td class="td_text_center">
						<input id="2000-shopName" name="店铺名"  type="text" value="" class="text_write_common" onchange="modifyExpense('1',this,'店铺名')">
					</td>
					...
					<td class="td_text_center">
						<input id="2001-comment" name="备注" type="text" 	class="text_write_common" onchange="modifyExpense('1',this,'备注')">
					</td>
				</tr>
				...
				<tr id="<%=2101+i%>-tr">
					...
					<td class="td_text_center">
						<input id="<%=2101+i%>-shopCostCenterCode" name="店铺成本中心" type="text" value="<%=shopCost.getShopCostCenterCode()%>" class="text_write_common" onchange="modifyExpense('1',this,'店铺成本中心');getShopNameByCS(this)">
					</td>
					
					<td class="td_text_center">
						<input id="<%=2101+i%>-shopName" type="text" name="店铺名" value="<%=shopCost.getShopName()%>" class="text_write_common" onchange="modifyExpense('1',this,'店铺名')">
					</td>
					...
					<td class="td_text_center">
						<input id="<%=2101+i%>-comment" type="text" name="备注"  value="<%=shopCost.getComment()%>"	class="text_write_common" onchange="modifyExpense('1',this,'备注')">
					</td>
				</tr>
				...
				<tr id="2001-tr">
					...
					<td class="td_text_center">
						<input id="2001-shopCostCenterCode" name="店铺成本中心" type="text" value="" class="text_write_common" onchange="modifyExpense('1',this,'店铺成本中心');getShopNameByCS(this)">
					</td>
					
					
					<td class="td_text_center">
						<input id="2001-shopName" name="店铺名"  type="text" value="" class="text_write_common" onchange="modifyExpense('1',this,'店铺名')">
					</td>
		...
		<div>
			...
			<table id='300' width="100%" border="0" cellpadding="2" cellspacing="1"
				...
				<tr id="3000-tr" style="display: none">
					...
					<td class="td_text_center">
						<select id="3000-categoryCode" name="大类" onchange="getaccountList(this,'大类');modifyExpense('1',this,'大类');" style="width: 100%">
					...
					<td class="td_text_center">
						...
						<select  id="3000-subjectCode" name="所属科目" name="subjectCode"  onchange="getAccountCode(this);" style="width: 100%">
							</select>
					</td>
				...
				<tr id="<%=3101+i%>-tr">
					...
					<td class="td_text_center">
						...
						<select id="<%=3101+i%>-categoryCode" name="大类"  onchange="getaccountList(this,'大类');modifyExpense('1',this,'大类');" style="width: 100%" >
					...
					<td id="<%=3101+i%>-subjectCodeTD" class="td_text_center_nodisplay">
						...
						<select  id="<%=3101+i%>-subjectCode" name="所属科目"  onchange="getAccountCode(this);" style="width: 100%">
							</select>
					</td>
				...
				<tr id="3001-tr">
					...
					<td class="td_text_center">
						<select id="3001-categoryCode" name="大类" onchange="getaccountList(this,'大类');modifyExpense('1',this,'大类');" style="width: 100%">
					...
					<td class="td_text_center">
						...
						<select  id="3001-subjectCode" name="所属科目" name="subjectCode"  onchange="getAccountCode(this);" style="width: 100%">
							</select>
					</td>
