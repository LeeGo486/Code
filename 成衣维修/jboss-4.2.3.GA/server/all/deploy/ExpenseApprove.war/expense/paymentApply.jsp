...
<%@page import="ep.org.branch.expense.dojo.PaymentSubjectCost"%>
...
<script type="text/javascript">
 ...
 var subjectMoneyIsRight = '0'; 	
	...
	function modifyExpense(modifyFlag,obj,desc){
		...
		xmlhttp.open("post","ajax.ajaxServlet",true);
		xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		xmlhttp.send("action=addPaymentDetail&modifyFlag="+modifyFlag+"&mapKey="+objIDs[0]+"&field="+objIDs[1]+"&fieldValue="+value);
		handleThread++;
	}
	...
	function addExpense(modifyFlag,tableID){
		...
		xmlhttp.open("post","ajax.ajaxServlet",true);
		xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		xmlhttp.send("action=addPaymentDetail&modifyFlag="+modifyFlag+"&mapKey="+newSerial+'&SN='+newSN);
		handleThread++;
	...
	function deleteExpense(modifyFlag,tableID){
		...
		xmlhttp.onreadystatechange=function() {
			...
			xmlhttp.open("post","ajax.ajaxServlet",true);
			xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			xmlhttp.send("action=addPaymentDetail&modifyFlag="+modifyFlag+"&mapKey="+mapKeyString);
 ...
 /* function chargeTotalCostByPurpose(obj){
  ...
  function print(){
  	...
  	if(stateCode=="100"){
  		...
  		var url = $('#webcontent').attr('value')+'/paymentApply.do?action=getPrintData&expenseNo='+expenseNo;	
...
//获取科目预算金额与已经金额
 ...
 function getShopNameByCS(obj){
		...
		xmlhttp.onreadystatechange=function() {
			if (xmlhttp.readyState==4 && xmlhttp.status==200){
				...
				if(res!=''){
					...
					var shopName = $('#'+key+'-costCenterName');
					shopName.attr('value',res);
					modifyExpense('1',shopName,'成本中心名称');
				}
				handleThread--;
			}else if(xmlhttp.readyState==4 && xmlhttp.status!=200){
	...
	<body  onload="initForm()">
		...
		<input type="hidden" id="hiddenPurposeTotalCost"  value="<%=request.getAttribute("purposeTotalCost")%>">
		<input type="hidden" id="hiddenSubjectTotalCost" value="<%=request.getAttribute("subjectTotalCost")%>">
	...
	<html:form action="/paymentApply" styleId="paymentApplyForm" method="post">
		...
		<div
			...
			<table width="100%" border="0" cellpadding="2" cellspacing="1"
				...
				--%>
					...
					<td class="td_text" >
						<html:text styleId="vendorNo" property="vendorNo" styleClass="text_write_common" ></html:text>
					</td>
				...
				<tr>
					<td class="td_field">其他要求说明：</td>
					<td  class="td_text" colspan="5">
					
						<textarea id="payComment" name="payComment" class="text_write_common"  rows="2" cols="20"></textarea>
		...
		<div>
			...
			<table id='100' width="100%" border="0" cellpadding="2" cellspacing="1"
				...
				<tr>
					...
					<td class="td_field_title" width="46%">
						费用用途
					</td>



					<td id="purposeTotalCost" class="td_field_title" width="10%">
						金额(<%=NumberUtil.formatFloatDecimal(2,(String)request.getAttribute("purposeTotalCost"))%>)
					</td>

					<td class="td_field_title" width="10%">
						使用日期
					</td>
				...
				<tr id="1000-tr" style="display: none">
					...
					<td class="td_text_center">
						<input id="1000-invoiceCode" name="发票号" type="text" onchange="modifyExpense('1',this,'发票号')">
					</td>
					
					<td class="td_text_center">
						<input id="1000-costCenterCode" name="成本中心代码" type="text" onchange="modifyExpense('1',this,'成本中心代码');getShopNameByCS(this);">
					</td>
					
					<td class="td_text_center">
						<input id="1000-costCenterName" name="成本中心名称" type="text" onchange="modifyExpense('1',this,'成本中心名称')">
					</td>
				...
				<tr id="<%=1101+i%>-tr" >
					...
					<td class="td_text_center">
						<input id="<%=1101+i%>-invoiceCode" name="发票号" type="text" value="<%=purposeCost.getInvoiceCode() %>" onchange="modifyExpense('1',this,'发票号')">
					</td>
					
					<td class="td_text_center">
						<input id="<%=1101+i%>-costCenterCode" name="成本中心代码" type="text" value=<%=purposeCost.getCostCenterCode() %> onchange="modifyExpense('1',this,'成本中心代码');getShopNameByCS(this);">
					</td>
					
					<td class="td_text_center">
						<input id="<%=1101+i%>-costCenterName" name="成本中心名称" type="text" value=<%=purposeCost.getCostCenterName() %> onchange="modifyExpense('1',this,'成本中心名称')">
					</td>
				...
				<tr id="1001-tr">
					...
					<td class="td_text_center">
						<input id="1001-invoiceCode" name="发票号" type="text"  onchange="modifyExpense('1',this,'发票号')">
					</td>
					
					<td class="td_text_center">
						<input id="1001-costCenterCode" name="成本中心代码" type="text"  onchange="modifyExpense('1',this,'成本中心代码');getShopNameByCS(this);">
					</td>
					
					<td class="td_text_center">
						<input id="1001-costCenterName" name="成本中心名称" type="text"  onchange="modifyExpense('1',this,'成本中心名称')">
					</td>
				</tr>
				<%} %>
			</table>
			...
			<table id='300' width="100%" border="0" cellpadding="2" cellspacing="1"
				...
				<tr>
					...
					<td id="subjectTotalCost" class="td_field_title" width="12.2%">
						金额(<%=NumberUtil.formatFloatDecimal(2,(String)request.getAttribute("subjectTotalCost"))%>)
					</td>
					<td class="td_field_title" width="19.2%">
						备注
					</td>
