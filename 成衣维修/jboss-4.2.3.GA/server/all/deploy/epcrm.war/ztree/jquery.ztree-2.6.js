...
(function($) {
	...
	//--------------------俞晓东增加
	function repairSubChk(setting, checked,parantNodes) {
		if (setting.checkable) {
			for (var son = 0; son < parantNodes.length; son++) {
				var treeNode = parantNodes[son];
				treeNode[setting.checkedCol] = checked;
				setSonNodeCheckBox(setting, treeNode, checked);
			}
		}
	...
	function zTreePlugin(){
		return {
			...
			//------------------------------俞晓东-------------增加
			checkSubNodes : function(checked,parentNodes) {
				repairSubChk(this.setting, checked,parentNodes);
			},
			//------------------------------俞晓东-------------增加
