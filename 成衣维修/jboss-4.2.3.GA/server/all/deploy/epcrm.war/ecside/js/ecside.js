...
if (!form[pageid]){
	...
	//form.action=ecsideObj.DEFAULT_ACTION;注释掉原因是会引起翻页问题
	...
	try {
		if (ecsideObj.doPreload){
			...
			//ecsideObj.ajaxSubmit();   注释的原因是不使用AJAX，同时会引起两次提交
			form.submit();
		}
	}catch (e){
		//try {
...
ECSideUtil.doSort=function(event,columnAlias,sortT1,formid,columnHearderId){
	...
	//form.action=ecsideObj.DEFAULT_ACTION;注释掉，因为会引起排序问题
...
var ecsideObj=ECSideUtil.getGridObj(formid);
	...
	//try {
		//ecsideObj.ajaxSubmit(); 注释是因为容易引起2次提交，目前并没有使用AJAX功能
	//}catch (e2){
		form.submit();
	//}
};
