...
namespace DBHelper
{
    ...
    public abstract class DbHelperSQL
    {
        ...
        public static bool Insert(object obj)
        {
            ...
            for (int i = 0; i < pis.Length; i++)
            {
                ...
                if (pis[i].Name.ToString().ToLower() != ModelInfo[1].ToLower())
                {
                    ...
                    if (pis[i].PropertyType.Name.ToString().ToLower() == "datetime" 
                        || pis[i].PropertyType.Name.ToString().ToLower() == "string"
                        || pis[i].PropertyType.Name.ToString().ToLower() == "boolean"
                        )
            ...
            sql=string.Format(sql, Columns.ToString().TrimEnd(','), Values.ToString().TrimEnd(','));
            if (ExecuteSql(sql) > 0)
                return true;
            else
                return false;
        ...
        public static string CreateAllocationId(string userid)
        {
            using (SqlConnection connection = new SqlConnection(connstring))
            {
                SqlCommand cmd = new SqlCommand("CUSF22.dbo.SP_WBY_GenerateBillNo", connection);
                cmd.CommandType = CommandType.StoredProcedure;

                cmd.Parameters.Add("@TableName", SqlDbType.VarChar);
                cmd.Parameters["@TableName"].Value = "F22NJ.dbo.d_move";

                cmd.Parameters.Add("@ColumnName", SqlDbType.VarChar);
                cmd.Parameters["@ColumnName"].Value = "moveid";

                cmd.Parameters.Add("@Prefix", SqlDbType.VarChar);
                cmd.Parameters["@Prefix"].Value = PubConstant.GetConnectionString("BsC");

                cmd.Parameters.Add("@BillDate", SqlDbType.DateTime);
                cmd.Parameters["@BillDate"].Value = DateTime.Now;

                cmd.Parameters.Add("@UserID", SqlDbType.VarChar);
                cmd.Parameters["@UserID"].Value = userid;

                cmd.Parameters.Add("@Result", SqlDbType.VarChar, 22).Direction = ParameterDirection.Output;

                String billNo = "";
                connection.Open();
                cmd.ExecuteNonQuery();
                billNo = cmd.Parameters["@Result"].Value.ToString();
                connection.Close();
                return billNo;
            }
        }
        ...
        //执行插入更新等存储过程
        public static DataTable RunProc(string procname, IDataParameter[] param)
        {
            using (SqlConnection connection = new SqlConnection(connstring))
            {
                SqlCommand cmd = BuildQueryCommand(connection, procname, param);
