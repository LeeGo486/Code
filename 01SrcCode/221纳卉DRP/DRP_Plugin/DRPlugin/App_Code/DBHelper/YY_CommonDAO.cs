...
namespace DBHelper
{
    public class YY_CommonDAO<T> where T : new()
    {
        ...
        {
            string sql = "SELECT depotid, "
                       + "       d_name "
                       + "FROM   j_depot "
                       + "WHERE  LEFT(depotid, 1) <> LEFT('{0}', 1) "
                       + "       AND parentid = ( "
                       + "               SELECT parentid "
                       + "               FROM   j_depot "
                       + "               WHERE  depotid = '{0}' "
                       ...
                       + "               WHERE  depotid = '{0}' "
                       + "           ) "
                       + "       AND freeze = 0 And d_name not like '%代销%' "
                       + "       AND depotid IN (SELECT depotid "
                       + "                       FROM   dbo.j_branddepot "
                       + "                       WHERE  drandid IN (SELECT drandid "
                       + "                                          FROM   j_branddepot "
                       + "                                          WHERE  depotid = '{0}'))";
            sql = string.Format(sql, depotid.Replace("'","").Replace("--",""));
            return DbHelperSQL.Query(sql).Tables[0];

        }

        public DataTable GetShopName(string depotid)
        {
            string sql = "select d_name from j_depot where depotid='{0}'";
            sql = string.Format(sql, depotid.Replace("'", "").Replace("--", ""));
            return DbHelperSQL.Query(sql).Tables[0];
        }
        /// <summary>
        ...
        {
            return DbHelperSQL.CreateAllocationId(userid);
        }

        public DataTable GetClotingInfo(string depotid, string clothingId)
        {
            IDataParameter[] param = new SqlParameter[5];
            param[0] = new SqlParameter("@dtype", 3);
            param[1] = new SqlParameter("@depotid", depotid);
            param[2] = new SqlParameter("@clothingid", clothingId);
            param[3] = new SqlParameter("@pricedate", "");
            param[4] = new SqlParameter("@rate_lx", 3);
            return DbHelperSQL.RunProc("t18tj_GetClothingPrice", param);
        }
