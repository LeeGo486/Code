...
namespace HZY.COM.WS.PE_OA
{
    public class PE_OA_HR_OverTimes : LogicBase, ILogicGetData
    {
        ...
        public DataSet GetData()
        {
            try
            {
                ...
                if (strType == "GetHs" || strType == "GetDs"
                    ...
                    || strType == "GetSYSConfig" || strType == "GetOTSpecialDay")
                {
                ...
                else if (strType == "SaveOTSpecialDay") // 加班配置
                {
                    ds = SaveRows7(conn, strType);
                }
                ...
                else if (strType == "CalculateMoney")//加班工资计算
                {
                    ds = CalculateMoney(conn, strType);
                }
        ...
        /// </summary>
        /// <returns></returns>
        public DataSet SaveRows7(Dbconn conn, String strType)
        {
            DataSet ds_Return = new DataSet();
            string strActor = m_request.Tables[0].Rows[0]["actor"].ToString();
            string strXML = m_request.Tables[0].Rows[0]["XML"].ToString();
            ...
            for (int i = 0; i < dtOptypes.Rows.Count; i++)
            {
                ...
                {
                    //如果包含修改日期，就循环移除
                    if (dsXML.Tables[i + 1].Columns.Contains("hr10_lmdt"))
                    {
                        dsXML.Tables[i + 1].Columns.Remove("hr10_lmdt");
                    }
                    DataTable dt_List1 = dsXML.Tables[i + 1];
                    bResult = conn.Insert("B02_BILL.HR10_OTSpecialDay", dt_List1);
                }
                //如果类型包含，update，解析成表，做update
                else if (dtOptypes.Rows[i][0].ToString().ToUpper() == "UPDATE")
                {
                    ...
                    ArrayList ListKey = new ArrayList();
                    ListKey.Add("hr10_id");
                    bResult = conn.Update("B02_BILL.HR10_OTSpecialDay", dt_List2, ListKey);
                }
                //如果类型包含，DELETE，解析成表，做DELETE
                else if (dtOptypes.Rows[i][0].ToString().ToUpper() == "DELETE")
                {
                    ...
                    for (int k = 0; k < dt_List3.Rows.Count; k++)
                    {
                        string strID1 = dt_List3.Rows[k]["hr10_id"].ToString();
                        bResult = conn.Delete("B02_BILL.HR10_OTSpecialDay", "hr10_id=" + "'" + strID1 + "'");
                    }
                }
            }
            conn.CommitTransaction();
