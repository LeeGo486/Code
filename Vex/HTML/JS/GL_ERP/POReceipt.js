...
//声音文件
var m_soundRight, m_soundWrong;
//汇总数据
var m_planSum, m_realSum


//初始化各类数据
$(document).ready(function () {
    $('#cmbSupplierCode').combobox({
        url: GetComboxWSRRURL('c87cc16f-388d-4397-a323-8d8063b7344f', "") + "&XML=" + GetFormJson([], 'cmbGetSupplier'),
        type: 'post',
        mode: 'remote',
        valueField: 'id',
        textField: 'text',
        ...
        onSelect: function (rec) {
            ...
            $('#cmbPOCode').combobox({
                url: GetComboxALLColumnWSRRURL('c87cc16f-388d-4397-a323-8d8063b7344f') + "&XML=" + GetFormJson(data, 'cmbGetPo'),
                type: 'post',
                valueField: 'id',
                textField: 'text',
                panelHeight: '300',
                onSelect: function (rec) {
                    ...
                    data[data.length] = { "name": "txtwhere", "value": rec.id };
                    $('#cmbAPCode').combobox({
                        url: GetComboxWSRRURL('c87cc16f-388d-4397-a323-8d8063b7344f', "", "AP1_APCode", "AP1_APCode") + "&XML=" + GetFormJson(data, 'cmbGetAP'),
                        type: 'post',
                        valueField: 'id',
                        textField: 'text',
                        panelHeight: '300'
...
function Search() {
    ...
    if (SearchData[0]["value"] == "" && SearchData[1]["value"] == "" && SearchData[2]["value"] == "" && SearchData[3]["value"] == "" && SearchData[4]["value"] == "") {
        ...
        $('#tab_list').datagrid({
            url: GetWSRRURL('c87cc16f-388d-4397-a323-8d8063b7344f'),
...
function POReceiptWindows(type) {
    ...
    $('#cmbWareHouse').combobox({
        required: true,
        url: GetComboxWSRRURL('c87cc16f-388d-4397-a323-8d8063b7344f', "", "WH1_WareHouseCode", "WH1_Name") + "&XML=" + GetGetJson([], 'cmbWareHouse'),
        type: 'post',
        editable: false,
        valueField: 'id',
        textField: 'text',
    ...
    if (m_SelectRow == undefined && m_SelectDetail == undefined) {
        ...
        if (m_type == 'Eligible') {
            $('#wintab_list').datagrid({
                ...
                footer: [[
                    { field: 'totle', title: '数量差异', width: 80, align: 'right', formatter: Formater.Nums },
                ]],
                onLoadSuccess: function (data) {
                    m_planSum = 0; m_realSum = 0;

                    for (i = 0; i < data.total; i++) {
                        m_planSum += +data.rows[0].plannumber;
                        m_realSum += +data.rows[0].number;
                    };
                },
            ...
            if (type != "check") {
                //初始化不合格原因码
                $('#cmbFailure').combobox({
                    url: GetComboxWSRRURL('c87cc16f-388d-4397-a323-8d8063b7344f') + "&XML=" + GetFormJson([], 'cmbGetFailure'),
                    type: 'post',
                    editable: false,
                    valueField: 'id',
                    textField: 'text',
...
function saveDate(m_type, m_audit) {
    ...
    var j = 0;
    for (i = 0; i < saveDate.total; i++) {
        var num = saveDate.rows[i].rp2_number;
        if (num != undefined) {
            //计划数量
            planNum = saveDate.rows[i].plannumber;
            //Detail
            arrayDetail[j] = { RP2_ID: "" };
            arrayDetail[j].RP2_CONO = "GL";
            arrayDetail[j].RP2_DIVI = "D01";
    ...
    arrayHead[arrayHead.length] = { "name": "txtRP1_CONO", "value": "GL" };
    arrayHead[arrayHead.length] = { "name": "txtRP1_DIVI", "value": "D01" };
...
function btnAudit(m_recode) {
    ...
    data[data.length] = { "name": "txtCONO", "value": "GL" };
