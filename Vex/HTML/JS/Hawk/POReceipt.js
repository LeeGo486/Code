...
var m_RgUser
    ...
    , m_SelectRow = null
    , m_SelectDetail = null
    , m_IndexHead = undefined
    , m_IndexDetail = undefined
    , m_comp = $.cookie("compName");

//初始化各类数据
$(document).ready(function () {
    editCell();

    $("#btnSubmitSaveAudit").click(function () {
        saveAndAudit("haveAudit");
    });
    //播放声音
    soundReady();
    $('#cmbSupplierCode').combobox({
        url: GetComboxWSRRURL('ab830f30-75d8-44f8-a272-f96a5bb5001a', "") + "&XML=" + GetFormJson([], 'cmbGetSupplier'),
        type: 'post',
        mode: 'remote',
        valueField: 'id',
        textField: 'text',
        ...
        onSelect: function (rec) {
            ...
            $('#cmbPOCode').combobox({
                url: GetComboxALLColumnWSRRURL('ab830f30-75d8-44f8-a272-f96a5bb5001a') + "&XML=" + GetFormJson(data, 'cmbGetPo'),
                type: 'post',
                valueField: 'id',
                textField: 'text',
                panelHeight: '300',
                onSelect: function (rec) {
                    ...
                    data[data.length] = { "name": "txtwhere", "value": rec.id };
                    $('#cmbAPCode').combobox({
                        url: GetComboxWSRRURL('ab830f30-75d8-44f8-a272-f96a5bb5001a', "", "AP1_APCode", "AP1_APCode") + "&XML=" + GetFormJson(data, 'cmbGetAP'),
                        type: 'post',
                        valueField: 'id',
                        textField: 'text',
                        panelHeight: '300'
...
function Search() {
    ...
    if (SearchData[0]["value"] == "" && SearchData[1]["value"] == "" && SearchData[2]["value"] == "" && SearchData[3]["value"] == "" && SearchData[4]["value"] == "") {
        ...
        $('#tab_list').datagrid({
            url: GetWSRRURL('ab830f30-75d8-44f8-a272-f96a5bb5001a'),
...
function POReceiptWindows(type) {
    ...
    if (m_SelectRow == undefined && m_SelectDetail == undefined) {
        ...
        $('#cmbWareHouse').combobox({
            required: true,
            url: GetComboxWSRRURL('ab830f30-75d8-44f8-a272-f96a5bb5001a', "", "WH1_WareHouseCode", "WH1_Name") + "&XML=" + GetGetJson([], 'GetWH'),
            type: 'post',
            editable: false,
            valueField: 'id',
            textField: 'text',
        ...
        if (m_SelectDetail != undefined) {
            ...
            data[data.length] = { "name": "txtpocode", "value": m_SelectDetail.po1_pocode };
            data[data.length] = { "name": "txtapcode", "value": m_SelectDetail.rp1_ap1_apcode };
            data[data.length] = { "name": "txtrpcode", "value": m_SelectDetail.rp1_receiptcode };
            XMLData = GetFormJson(data, 'GetDataForRP'); //有收货单号
        }
        else {
            ...
            XMLData = GetFormJson(data, 'GetDataForRP');//无收货单号
        }
        ...
        if (m_type == 'Eligible') {
            ...
            if (type != "check") {
                //初始化不合格原因码
                $('#cmbFailure').combobox({
                    url: GetComboxWSRRURL('ab830f30-75d8-44f8-a272-f96a5bb5001a') + "&XML=" + GetFormJson([], 'cmbGetFailure'),
                    type: 'post',
                    editable: false,
                    valueField: 'id',
                    textField: 'text',
...
function saveDate(m_type, m_audit, haveAudit) {
    ...
    //    if (haveAudit != undefined && haveAudit != '') {
    //        saveDate = [$('#wintab_list').datagrid('getSelected')];
    //        if (saveDate.length == 0) {
    //            $.messager.alert('提示', '无任何数据需要保存或审核', 'error');
    //            return false;
    //        }
    //    }
    //    else {
    //        $.messager.alert('提示', '无任何数据需要保存', 'error');
    //        return false;
    //    }
    ...
    for (i = 0; i < saveDate.rows.length; i++) {
        if ('rp2_number' in saveDate.rows[i]) {
            //2015-07-02 添加判断雅斓是否超收 
            if (m_comp == 'GL') {
                if (saveDate.rows[i].diff * 1 < saveDate.rows[i].rp2_number * 1) {
                    $.messager.alert('提示', saveDate.rows[i].skucode + '不允许超收', 'warning');
                    return;
                }
            }
            else {
    ...
    var planNum = 0;
    for (i = 0; i < saveDate.rows.length; i++) {
        if ('rp2_number' in saveDate.rows[i] && (saveDate.rows[i].rp2_number != '' && saveDate.rows[i].rp2_number * 1 > 0)) {
            //计划数量
            planNum += saveDate.rows[i].plannumber * 1;
            sumThis += saveDate.rows[i].rp2_number * 1;
            //Detail
            arrayDetail[j] = { RP2_ID: "" };
            arrayDetail[j].RP2_CONO = "HYFG";
            arrayDetail[j].RP2_DIVI = m_comp;
    ...
    arrayHead[arrayHead.length] = { "name": "txtRP1_DIVI", "value": m_comp };
...
function btnAudit(m_recode) {
    ...
    //状态控制
    if (m_SelectDetail != undefined && m_SelectDetail.rp1_lstate && m_SelectDetail.rp1_lstate == '90') {
        $.messager.alert('提示', '已审核,不允许再次审核', 'warning');
        return false;
    }
    var data = [];
    ...
    data[data.length] = { "name": "txtDIVI", "value": m_comp };
...
function saveAndAudit() {
    var haveAudit = arguments[0];
    var bResult = saveDate(m_type, 'Audit', haveAudit);
