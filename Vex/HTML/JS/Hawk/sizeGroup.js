...
$(function () {
    ...
    var url = GetWSRRURL('7ea4d40d-02fa-43cf-ae3c-8231943f8124') + "&XML=" + GetFormJson('', 'Get') + "&Where=CT1_Code='";
...
function onSelect(index, row) {
    ...
    var url = GetWSRRURL('7ea4d40d-02fa-43cf-ae3c-8231943f8124') + "&XML=" + GetFormJson('', 'Get') + "&Where=CT1_Code='SIDE' AND CT1_fatherID='" + fatherID + "'";
    InitGridSize(url);
}
...
function btnSave(table) {
    ...
    if (changeRows.length > 0) {
        ...
        for (var i = 0; i < insertRow.length; i++) {  //去掉有空数据的行
            if (table == 'tabSize') {
                if (insertRow[i].ct1_keyid == '' || insertRow[i].ct1_keyname == '' || insertRow[i].ct1_options == '' || insertRow[i].ct1_optionsvalues == '') {
                    var dltRow = $('#' + table).datagrid('getRowIndex', insertRow[i]);
                    $('#' + table).datagrid('deleteRow', dltRow);//删除空数据的行
                }
                else {
                    inserts.push(insertRow[i]);
                }
            } else {
                if (insertRow[i].ct1_options == '' || insertRow[i].ct1_optionsvalues == '') {
                    var dltRow = $('#' + table).datagrid('getRowIndex', insertRow[i]);
                    $('#' + table).datagrid('deleteRow', dltRow);//删除空数据的行
                }
                else {
        ...
        for (var j = 0; j < updateRow.length; j++) {
            if (table == 'tabSize') {
                if (updateRow[j].ct1_keyid == '' || updateRow[j].ct1_keyname == '' || updateRow[j].ct1_options == '' || updateRow[j].ct1_optionsvalues == '') {
                    var dltRow = $('#' + table).datagrid('getRowIndex', updateRow[j]);
                    $('#' + table).datagrid('deleteRow', dltRow);//删除空数据的行
                }
                else {
                    updates.push(updateRow[j]);
                }
            } else {
                if (updateRow[j].ct1_options == '' || updateRow[j].ct1_optionsvalues == '') {                   
                    var dltRow = $('#' + table).datagrid('getRowIndex', updateRow[j]);
                    $('#' + table).datagrid('deleteRow', dltRow);//删除空数据的行
                }
                else {
        ...
        var editJSON = GetEditJson(inserts, updates, null);
        var data = [];
        ...
        $.ajax({
            url: GetWSRRURL('6a4c321b-07e4-41a6-8f30-51ba3b82a5fb'),
            type: 'post',
            async: false, //异步,
            data: { "XML": XMLData },
