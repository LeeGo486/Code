...
$(document).ready(function () {
    ...
    $('#cmbPC1_PaymentType').combobox({
        required: true,
        url: GetComboxWSRRURL('7ea4d40d-02fa-43cf-ae3c-8231943f8124', "", "CT1_KeyID", "CT1_KeyName") + "&XML=" + GetFormJson([{ "name": "txtzt", "value": m_compName }], 'GET') + "&Where=CT1_Code='" + "TEPY" + "'",
        type: 'post',
        editable: false,
        valueField: 'id',
        textField: 'text',
    ...
    $('#txtPC1_POTypeCode').combobox({
        required: true,
        url: GetComboxWSRRURL('7ea4d40d-02fa-43cf-ae3c-8231943f8124', "", "CT1_KeyID", "CT1_KeyName") + "&XML=" + GetFormJson([{ "name": "txtzt", "value": m_compName }], 'GET') + "&Where=CT1_Code='" + "POTP" + "'",
        type: 'post',
        editable: false,
        valueField: 'id',
        textField: 'text',
    ...
    $('#cmbPC1_Currency').combobox({
        required: true,
        url: GetComboxWSRRURL('7ea4d40d-02fa-43cf-ae3c-8231943f8124', "", "CT1_KeyID", "CT1_KeyName") + "&XML=" + GetFormJson([{ "name": "txtzt", "value": m_compName }], 'GET') + "&Where=CT1_Code='" + "CUCD" + "'",
        type: 'post',
        editable: false,
        valueField: 'id',
        textField: 'text',
    ...
    $('#cmbPC1_PaymentChannel').combobox({
        required: true,
        url: GetComboxWSRRURL('7ea4d40d-02fa-43cf-ae3c-8231943f8124', "", "CT1_KeyID", "CT1_KeyName") + "&XML=" + GetFormJson([{ "name": "txtzt", "value": m_compName }], 'GET') + "&Where=CT1_Code='" + "PYTP" + "'",
        type: 'post',
        editable: false,
        valueField: 'id',
        textField: 'text',
    ...
    var url = GetWSRRURL('ea48f25a-597d-43cb-9084-e8742e9b50ea') + "&XML=" + GetFormJson([], 'Get');
    initGird(url);
});
...
function contractWindow(type) {
    ...
    if (type == 'new') {
        ...
        $('#cmbPC1_SP1_SupplierCode').combogrid({
            disabled: false,
            panelWidth: 300,
            url: GetWSRRURL('ffda947c-e49f-4106-982a-dbda0664b282') + "&XML=" + GetFormJson([{ "name": "txtzt", "value": m_compName }], 'SupplierInfo'),
    ...
    if ($('#formWindow').form('validate')) {
        if (checkDate("Contract")) {
            ...
            if (contractCode == "系统生成") {
                ...
                data[data.length] = { "name": "txtpc1_StateCode", "value": "15" };
                ...
                $.ajax({
                    url: GetWSRRURL('ea48f25a-597d-43cb-9084-e8742e9b50ea'),
                    type: 'post',
                    async: false, //异步,
                    data: { "XML": XMLData },
                ...
                var selected_Row = $('#tab_list').datagrid('getSelected');
                if (selected_Row.pc1_statecode == "15") {        //1代表已保存状态
                    var data = $('#formWindow').serializeArray();
...
function deleteRows() {
    ...
    var xmlData = GetEditJsonbyType(deleted, "CheckLink");
    xmlData = escape(xmlData);
    if (deleted.length == 0) {
        alert('请选择需要删除的行');
    } else {
        $.ajax({
            url: GetWSRRURL('ea48f25a-597d-43cb-9084-e8742e9b50ea') + "&XML=" + xmlData,
            type: 'post',
            async: false, //异步,
            success: function (result) {
                try {
                    ...
                    if (result[0].Error) {
                        ...
                        if (checkStatus("15")) {        //合同状态 true or false;
...
function checkStatus(type) {
    ...
    if (type == "15") {
        for (i = 0; i < checkdata.length; i++) {
            if (checkdata[i].pc1_statecode != "15") {
                alert('请检查合同状态');
                return false;
            } else {
                return true;
        ...
        for (i = 0; i < checkdata.length; i++) {
            if (checkdata[i].pc1_statecode != "35") {
                alert('请检查合同状态');
                return false;
            } else {
                return true;
...
function auditContract(type) {
    ...
    if (update.length == 0) {
        ...
        if (type == "yes") {
            if (checkStatus("15")) {        //true or false;
            ...
            if (checkStatus("35")) {        //true or false;
                for (i = 0; i < update.length; i++) {
                    update[i].type = "35";
                };
                xmlData = GetEditJsonbyType(update, "Audit");
...
function searchData() {
    ...
    var url = GetWSRRURL('ea48f25a-597d-43cb-9084-e8742e9b50ea') + "&XML=" + GetFormJson(searchData, 'Search');
...
function CoPoLinkOpen() {
    ...
    if (selected_Row.pc1_statename == "已保存") {
        $.messager.alert("提示", "该合同未审核，禁止关联采购单", "warning");
        return;
    };
    if (selected_Row != null) {
        ...
        $('#tab_link').datagrid({
            url: GetWSRRURL('d2ac5391-d4af-4991-8ae8-55a23dacccdd') + "&XML=" + GetFormJson("", 'GET') + "&WHERE=" + url,
            title: '采购单列表',
            width: '90%',
            singleSelect: true,
            sortOrder: 'desc',
