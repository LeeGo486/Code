...
$(document).ready(function () {
    ...
    $('#txtCode').keydown(function (e) {
        if (e.keyCode == 13) {
            ...
            var result = $.ajax({
                url: GetWSRRURL('a844d50d-9b2b-4ada-81ac-070b1bd474acf'),
                data: { "XML": XMLData },
                type: 'post',
                async: false
            });
    ...
    $('#txtType').combobox({
        url: GetComboxWSRRURL('a844d50d-9b2b-4ada-81ac-070b1bd474acf', "", "ThemeCode", "ThemeName") + "&XML=" + GetFormJson([], 'Gettheme'),
        type: 'post',
        editable: false,
        valueField: 'id',
        textField: 'text',
...
function initGird(where) {
    $('#tab_list').datagrid({
        ...
        columns: [[
            ...
            { field: 'themename', title: '主题', width: 80 },
...
function getStyleData(style) {
    ...
    htmlobj = $.ajax({
        url: GetWSRRURL('1dd5ef1b-774e-49f7-934a-bf201b732b0a'),
...
function pic_onSubmit(id) {
    ...
    var test = new AjaxUpload(button, {
        ...
        action: '/Handler_Upload.ashx?filePath=GL_Goods&fileName=' + GV.Pic.picName,
        name: 'fileName',
...
function saveState() {
    ...
    $.messager.confirm("提示", "你确定要批量更新吗?", function (r) {
        if (r) {
            ...
            $.post(GetWSRRURL('a844d50d-9b2b-4ada-81ac-070b1bd474acf'), { "XML": XMLData }, function (data) {
