...
function initGird(where) {
    $("#tab_list").datagrid({
        url: GetWSRRURL('6bc75647-0350-468e-a10b-06dee4a9551e'),
        ...
        columns: [[
            ...
            { field: 'j_name', title: '取货店铺名称', width: 80 },
            { field: 'DEPT_ID', title: '店铺编号', width: 80 },
            { field: 'g_sf', title: '省', width: 80 },
            { field: 'g_cs', title: '市', width: 80 },
            { field: 'm_shoptel', title: '店铺电话', width: 120 },
...
function search() {
    ...
    //取货日期
    if (data[0].value != "") {
        where += " AND A.[DeliveryDate] BETWEEN '" + data[0].value + "' AND '" + data[1].value + "'";
    };
...
function sumbit(type) {
    $.messager.confirm("提示", "你确定要" + type + "订单吗?", function (r) {
        if (r) {
            ...
            $.post(GetWSRRURL('6bc75647-0350-468e-a10b-06dee4a9551e'), { "XML": XMLData }, function (data) {
                data = eval("[" + data + "]");
                if (data[0].rows[0].result == "True") {
                    $.messager.alert("提示", "更新成功", "ok");
                } else {
                    $.messager.alert("提示", "更新失败请检查", "error");
                };
