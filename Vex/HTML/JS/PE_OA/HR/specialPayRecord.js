...
function actSave() {
    ...
    if (type == 'new') {
        ...
        //跳转到提交页面——待办
        var curlo = 'specialPayRecord.html?' + 'v0=' + type + '&v1=' + m_MID + '&v2=' + m_ApproveID
...
function actApprove(type) {
    ...
    $.messager.confirm('确认框', '确认' + actinfo + '该特殊薪资申请吗？', function (r) {
        if (r) {
            //修改任务信息  
            ...
            if (!flag1) {
                alert('回写特殊薪资主信息失败，请截全屏图发给系统管理员！')
                return flag;
            }
            alert(actinfo + '成功！');
...
*/
function actClose() {
    var type = dataUtil_undefinedOrNull(basedata_getUrlParam("v3"), '');
    if (type == 'task') {
        parent.turnPage('A', 'tasks', '');
    } else if (type == 'list') {
        parent.turnPage('A', 'specialPay', '');
    }
}
