...
/***********************************************************************
  ...
  *  2013-04-18
  */
 function dateUtil_dateFomaterD(date) {
     if (date) {
         var y = date.getFullYear();
         var m = date.getMonth() + 1;
         var d = date.getDate();
...
$.extend($.fn.datagrid.methods, {
	...
	removeToolbarItem1 : function (jq, param) {  
		return jq.each(function () {
			...
			if (cbtn && cbtn.length > 0) {
				cbtn.closest('td').html(basedate_actSetNStr('&nbsp;',30)); 
				cbtn = null;
			}
		});
	}
 ...
 *  2013-04-18
 */ 
function dateUtil_dateboxFomaterCXK(dateboxID,ssplit)   
{   
	 $('#'+dateboxID).datebox({  
	 	  closeText:'关闭',  
	 	  editable:true, 
 ...
 *  2014-12-27
 */
function dateUtil_getPreMonth(date, ssplit) {
    if (date == '') {
        date = dateUtil_dateFomaterB(new Date(), ssplit);
    }
    var arr = date.split(ssplit);
 ...
 *  2014-12-27
 */
function dateUtil_getNextMonth(date, ssplit) {
    if (date == '') {
        date = dateUtil_dateFomaterB(new Date(), ssplit);
    }
    var arr = date.split('-');
 ...
 *  2015-04-20
 */
function dateUtil_getMonthByDay(date, ssplit, sday) {
    if (date == '') {
        date = dateUtil_dateFomaterB(new Date(), ssplit);
    }
    var arr = date.split(ssplit);
    ...
    var t2 = date;
    if (day * 1 < sday * 1) { 
         t2 = dateUtil_getPreMonth(date, ssplit);
    } 
    return t2;
}
 ...
 *  2015-07-20
 */
function dateUtil_DayNumOfMonth(Year, Month) {
