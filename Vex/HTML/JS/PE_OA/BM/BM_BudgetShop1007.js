...
$(function () {
    ...
    for (var i = 0; i < m_UserInfo.roles.length; i++) {
        var authorization_text = m_UserInfo.roles[i].authorization_text;
        var authorization_id = m_UserInfo.roles[i].authorization_id;
        if (authorization_id == '129') {
            m_Role = 'A'
        } else if (authorization_id == '130') {
            m_Role = 'B';
            break;
        }
    }
...
function open_rowdlg(type) {
    ...
    if (type == '新建记录') {
        ...
        $("#rbg3_com").attr('value', row.bg3_com);
        $("#rbg3_branch").attr('value', row.bg3_branch);
...
function close_rowdlg() {
    ...
    if (currow.bg3_id != undefined && currow.bg3_id != '') {
        currow.bg3_com = $("#rbg3_com").attr('value');
        currow.bg3_branch = $("#rbg3_branch").attr('value');
