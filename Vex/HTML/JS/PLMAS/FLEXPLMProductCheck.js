...
$(function () {
    ...
    $("#txtBeginDate")
        .datebox({width:150})
        .datebox("setValue", (new Date()).DateAdd('d', -1)
        .format("yyyy-MM-dd"));
    $("#txtEndDate")
        .datebox()
        .datebox("setValue", (new Date()).DateAdd('d', 0)
        .format("yyyy-MM-dd"));

    $("#Ssn").combobox({
        url: GetComboxWSRRURL('d763e76a-f7e0-4fbc-a2f2-9a9956ecccb9', "", "cbtx40", "cbtx40") + "&XML=" + GetFormJson([{ "name": "txtzt", "value": m_zt }], 'GetFlexPLMSeason'),
        valueField: 'id',
        textField: 'text',
        panelHeight: 'auto',
        editable: false
    ...
    $("#Prerequisite").combobox({
        ...
        onLoadSuccess: function () {
            $("#Prerequisite").combobox('setValue', 1);
            $("#date").show();
        },
...
function SearchClick() {
    ...
    var pre = $("#date").is(":hidden");

    if (!pre) {
        m_SearchData.push({ "name": "txtBeginDate", "value": $("#txtBeginDate").datebox("getText") });
        m_SearchData.push({ "name": "txtEndDate", "value": $("#txtEndDate").datebox("getText") });
    } else {
        m_SearchData.push({ "name": "txtPlmSeason", "value": $("#Ssn").combobox("getValue") });
    };
    ...
    $('#tabList').datagrid(
        {
            ...
            columns: [[
                ...
                { field: 'selectsample', title: '确认样衣', width: 125 },
...
function initBrand() {
    $("#Brand").combobox({
        ...
        onSelect: function (record) {
            $('#cmbDesigner').combobox({ 
                url: GetComboxWSRRURL('d763e76a-f7e0-4fbc-a2f2-9a9956ecccb9', "", "cbtx40", "cbtx40") + "&XML=" + GetFormJson([{ "name": "txtzt", "value": m_zt }, { "name": "txtOwnBrand", "value": escape(record.id) }], 'GetDesigner'),
                valueField: 'id',
                textField: 'text',
                width:150,
                panelHeight: 300,
    ...
    $("#Brand").combobox('setValue', ownedBrand);//把品牌赋值给HTML品牌

    $('#cmbDesigner').combobox({
        url: GetComboxWSRRURL('d763e76a-f7e0-4fbc-a2f2-9a9956ecccb9', "", "cbtx40", "cbtx40") + "&XML=" + GetFormJson([{ "name": "txtzt", "value": m_zt }, { "name": "txtOwnBrand", "value": escape(ownedBrand) }], 'GetDesigner'),
        valueField: 'id',
        textField: 'text',
        width:150,
        panelHeight: 300,
...
function applyDesign() {
    ...
    var barnd
        = $("#Brand").combobox('getValue');
    var season
        = mainGrid.plmseason;
    var productname
        = mainGrid.productname;
    var desginName
        = mainGrid.designer;
    var styleBrand
        = mainGrid.brand;
    ...
    var data = [];
    data.push({ "name": "txt操作类型", "value": "GETPRODUCTPROGRESS_WHOLE" });
    data.push({ "name": "txt实体", "value": "GetStyleProgressInfo" });
    data.push({ "name": "txt返回内容", "value": "成功" });
    data.push({ "name": "txtCONO", "value": "HFYG" });
    data.push({ "name": "txtDIVI", "value": m_zt });
    data.push({ "name": "txt标识", "value": "1" });
    data.push({ "name": "txt品牌", "value": barnd });
    data.push({ "name": "txt季节", "value": season });
    data.push({ "name": "txt品类", "value": "" });
    data.push({ "name": "txt样衣数", "value": "" });
    data.push({ "name": "txt设计师", "value": "" });
    data.push({ "name": "txt版师", "value": "" });
    data.push({ "name": "txt上货日期", "value": "" });
    data.push({ "name": "txt系列", "value": "" });
    data.push({ "name": "txt款号", "value": productname });
    data.push({ "name": "txt备注", "value": "" });
    data.push({ "name": "txt排序", "value": "designer" });
    ...
    //领料人
    $('#dycrpe').combobox({
        url: GetComboxWSRRURL('d763e76a-f7e0-4fbc-a2f2-9a9956ecccb9', "", "cbstky", "cbstky")
            + "&XML=" + GetFormJson([{ "name": "txtCBCRPE", "value": styleBrand }, { "name": "txtzt", "value": m_zt }], 'GetXiMaiApplication'),
        valueField: 'id',
        textField: 'text',
        panelHeight: 'auto',
        editable: false,
    ...
    $.post(url, function (data) {
        ...
        var rstData = resutl[0].rows[0];

        $.messager.progress('close');
        $("#measurementsid").val(rstData.measurementsid);
        $("#boduanseries").val(rstData.boduanseries);
        $("#boduan").val(rstData.boduan);
        $("#samplenum").val(rstData.samplenum);


        $("#sureApply").linkbutton({ plain: true })
            .unbind().bind('click', function () { submitApply(productname,season,desginName) });

        $("#closeApply").linkbutton({ plain: true })
...
function closeWinDesign() {
    ...
    $("#formApply input[type=radio]")
        .attr('checked', false);
    $("#formApply input[name=cmbaltermeasurement]")
        .attr("checked", false).attr("disabled", true);
    $("#formApply input[name=txtdyrezn]")
        .val("").attr("disabled", true);
    $("#applyDesign").window().window('close');
}
...
function monitor(value) {
    ...
    if (value == 1) {
        ...
        $("#tiaoban").attr("checked",true);
    }
}

function reson(box) {
    var bcheck = box.checked;
...
function detail() {
    ...
    data.push({ "name": "txt操作类型", "value": "GETPRODUCTPROGRESS_DETAIL" });
    data.push({ "name": "txt实体", "value": "GetStyleProgressInfo" });
    data.push({ "name": "txt返回内容", "value": "成功" });
    data.push({ "name": "txtCONO", "value": "HFYG" });
    data.push({ "name": "txtDIVI", "value": m_zt });
    ...
    $("#detailYangYi").datagrid({
            ...
            columns: [[
                { field: 'dycrdt', title: '申请时间', width: 100 },
                { field: 'dyjbdt', title: '开做时间', width: 100 },
                { field: 'dyjbpe', title: '样衣师', width: 100 },
                { field: 'dyyyno', title: '样板号', width: 100 },
                { field: 'endtime', title: '完成时间', width: 100 }
            ]]
              
    });
    $.messager.progress('close');
        $("#detailWinCls").linkbutton({
            plain: true
        }).unbind().bind('click', function () {
            $("#detailWin input").val("");
...
//打样申请提交
function submitApply(pName,pSeason,pDesigner) {

    //check
    if ($('#formApply').form('validate') == false) {
        return;
    };


    var dyversion = $("#formApply input[name=txtdyversion]").is(':checked');
    ...
    var data = $("#formApply").serializeArray();

    var dyVal = data[0].value;
    //无用，免得占用内存
    data = null;
    ...
    var strdyrezn = "";
    if (dyVal == 0) {
        $("#formApply input[type=checkbox]").each(function(){
            if (this.checked) {
                if (this.name == "DYREZN") {
                    strdyrezn += $("#otherReson").val();
                    strdyrezn += ";";
                }
                else {
                    strdyrezn += $(this).val() + ";";
    ...
    //领料人
    var dycrpe = $("#dycrpe").combobox('getValue');
    //打样数量
    var ssNum = $("#ss").numberspinner('getValue');
    //是否需要调版
    var tiaoban = 0;
    $("#formApply input[name=cmbaltermeasurement]").each(function(){
            if(this.checked){
                tiaoban = $(this).val();        
            };
    });

    var url = GetWSRRURL('ed414b8b-581f-4565-b5fb-66ce416bb66e')

    var xmlData = '<ProductName>' +pName+'</ProductName>';
    xmlData += '<PLMSeason>' +pSeason+ '</PLMSeason>';
    xmlData += '<DYVersion>' +dyVal+ '</DYVersion>';
    xmlData += '<DYREZN>'+strdyrezn+'</DYREZN>';
    xmlData += '<DYNumber>'+ssNum+'</DYNumber>';
    xmlData += '<DYType>初样</DYType>';
    xmlData += '<AlterMeasurement>'+tiaoban+'</AlterMeasurement>';
    xmlData += '<DYCRPE>'+dycrpe+'</DYCRPE>';
    xmlData += '<Designer>'+pDesigner+'</Designer>';
    xmlData += '<CONO>HFYG</CONO>';
    xmlData += '<DIVI>' + m_zt + '</DIVI>';
    xmlData = escape(xmlData);
    
        
    if (dyVal == 1) {
        $.messager.confirm("操作提示", "系统自动确认版型，并发起第一件样衣制作申请，是否继续？", function (r) {
            if(r){
                $.post(url, xmlData, function (data) {
        ...
        if(tiaoban == 1){
            $.messager.confirm("操作提示","系统将自动发起调版请求并通知版师调版，是否继续？",function(r){
                if(r){
                    $.post(url, xmlData, function(data){
                        var result = eval( '['+data+']');
                        try {
            ...
            $.messager.confirm("操作提示","系统将不发起调版请求，版师不会进行调版，是否继续？",function(r){
                if(r){
                    $.post(url, xmlData, function(data){
                        var result = eval( '['+data+']');
                        try {
...
function isYangYi() {
    ...
    $("#YangYiSure")
        ...
        .click(function () { submitYangYi(mainGrid) });

    $("#YangYiClose")
        .unbind()
        .linkbutton({ iconCls: 'icon-undo', plain: true })
        .click(function(){isYangYiWin('close')});

    getYangYiGrid(mainGrid);

    isYangYiWin('open');

}

function isYangYiWin(type) {
    $("#YangYiWin").window({
        top: 20,
        title: '样衣确认',
        modal: true,
...
function getYangYiGrid(selectedRow) {

    var data = [];
    var season = selectedRow.plmseason;
    var brand = selectedRow.brand;
    ...
    data.push({"name":"txtDIVI","value":m_zt});
    data.push({"name":"txt标识","value":1});
    data.push({"name":"txt品牌","value":brand});
    data.push({"name":"txt季节","value":season});
    data.push({"name":"txt品类","value":""});
    data.push({"name":"txt样衣数","value":""});
    data.push({"name":"txt设计师","value":""});
    ...
    $("#YangYiList").datagrid({
        ...
        singleSelect: true, //单行选择
        columns: [[
            { field: 'dyyyno', title: '样衣号', width: 125 },
            { field: 'dycrdt', title: '结构组发版时间', width: 100 },
            { field: 'dyjbdt', title: '样衣开始时间', width: 100, formatter: Formater.Date },
            { field: 'endtime', title: '样衣完成时间', width: 125, formatter: Formater.Date },
            { field: 'dyjbpe', title: '样衣师', width: 125 }
        ]]
    });
}

function submitYangYi(baseGradRow){

    var selYYRow = $("#YangYiList").datagrid("getSelected");

    if(selYYRow == null ){

        $.messager("警告","请选中需要样衣确认的行。","warning");
        return;
    };

    var YYNo = selYYRow.dyyyno;
    var season = baseGradRow.plmseason;
    var productname = baseGradRow.productname;

    $.messager.confirm('提示框', 
        "请确认选择"+YYNo+"作为进展厅的样衣吗？", 
        function (r) 
        {
            var data = [];
            data.push({ "name": "txtzt", "value": m_zt });
            data.push({"name":"txtProductname","value":productname});
            data.push({"name":"txtSeason","value":season});
            data.push({"name":"txtYYNo","value":YYNo});
           
            var xmlData = GetFormJson(data, 'sureSample');
            xmlData = escape(xmlData);

            $.post(GetWSRRURL("39ad2a7c-bbd2-4c92-9a69-efc17708c898") , { "XML": xmlData }, function (ret) {

                var data = eval('(' + ret + ')');

                $("#tabList").datagrid('reload');

                $.messager.alert("提示", data.rows[0].message, "ok");
