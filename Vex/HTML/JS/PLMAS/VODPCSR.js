...
var m_type = [{ "id": "1", "text": '新版类型' }, { "id": "2", "text": "新版品类" }
              , { "id": "5", "text": "调版品类" }]//统计类型6种 , { "id": "3", "text": "新版系列" }, { "id": "4", "text": "新版品类(自主)" }, { "id": "6", "text": "调版系列" }
var m_Nowdate;//当前日期
...
$(function () {
    ...
    $('#cmbSelectType').combobox({
        data: [{ "id": "date", text: "日期", selected: true }, { "id": "orderPlacing", text: "季节" }],
        valueField: 'id',
        textField: 'text',
        panelHeight: 'auto',
        editable: false,
    ...
    $("#txtbrand").combobox({
        ...
        onSelect: function (record) {
            ...
            $('#txtProductline').combobox({
                url: GetComboxWSRRURL('d763e76a-f7e0-4fbc-a2f2-9a9956ecccb9', "", "cbtx15", "cbtx15") + "&XML=" + GetFormJson([{ "name": "txtzt", "value": m_zt }], 'GetBrandLineDiff') + "&Where=CBTX40='" + record.text + "'",
                valueField: 'id',
                textField: 'text',
                panelHeight: 'auto',
                editable: false,
...
function SearchClick() {
    ...
    else if (selectType == "orderPlacing") {  //季节
...
function initGird() {
    ...
    var productLines = $('#txtProductline').combobox('getValues'); //获取产品线
    if (productLines.length > 0 && productLines[0] != '') {
    ...
    data[data.length] = { "name": "txt日期", "value": '' };
    ...
    data[data.length] = { "name": "txt产品线", "value": productLines };
    data[data.length] = { "name": "txt统计对象", "value": countType };

    XMLData = GetDBFrameAML(data);   //06d54b26-796a-4fa6-bbba-1ddcb4e671fc
    ...
    data1[data1.length] = { "name": "txt日期", "value": '' };
    ...
    data1[data1.length] = { "name": "txt产品线", "value": productLines };
    data1[data1.length] = { "name": "txt统计对象", "value": countType };


    XMLdata1 = GetDBFrameAML(data1); //  06d54b26-796a-4fa6-bbba-1ddcb4e671fc
...
function onDblClickRow(index, row) {
    ...
    var productlines = "";
    if (m_productline.length > 0) {
        productlines = m_productline.join("','")
    ...
    // m_productline = m_productline.replaceAll(",", "','");

    var data = [];
    data[data.length] = { "name": "txtzt", "value": m_zt };
    data[data.length] = { "name": "txtMUser", "value": escape(row.muser) };
    data[data.length] = { "name": "txtBrand", "value": escape(productlines) };
    data[data.length] = { "name": "txtPlmSeason", "value": escape(m_season) };
    data[data.length] = { "name": "txtBeginDate", "value": m_beginDate };
    data[data.length] = { "name": "txtEndDate", "value": m_endDate };

    var url = GetWSRRURL('d763e76a-f7e0-4fbc-a2f2-9a9956ecccb9') + "&XML=" + GetFormJson(data, 'GetDetailDataForVODPCSR');
    ...
    $('#tabDetail').datagrid({
        ...
        rownumbers: true,
        columns: [[
            { field: "producttype", title: "类型", width: 120, align: 'center' },
            { field: "measurementsid", title: "样板号", width: 120, align: 'center' },
            { field: "category", title: "品类", width: 120, align: 'center' },
            { field: "designer", title: "设计师", width: 120, align: 'center' }
        ]],
        onLoadSuccess: function (data) {
            if (data.rows.length == 1 && data.rows[0].producttype == "") {
                ClearGrid("#tabDetail");
            }
        }
    });
