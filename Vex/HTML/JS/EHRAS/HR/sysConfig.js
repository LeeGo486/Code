...
//类型选择
var m_Type = [{ typeid: '未配置', typevalue: '未配置' }, { typeid: '已配置', typevalue: '已配置' }, { typeid: '全部', typevalue: '全部' }];
...
$(function () {
    ...
    //同一人负责多个公司
    m_Filter1 = ' AND hr9_type = \'1\' ';
    if (m_UserInfo.actor == 'E') {
        m_Filter1 += 'AND  hr9_ehrorgid in (' + dataUtil_undefinedOrNull(m_UserInfo.comIDs, '') + '-2)';
    }
    //alert(m_Filter1);
    //基础数据获取 
    ...
    setCombobox('#state', 70, 70, 68, false, false, 'typeid', 'typevalue', m_Type);
...
function actLoadData(page, number) {
    ...
    if (state =='未配置') {
        cons = cons + ' AND ( hr9_ehrunitid is null )';
    } else if (state == '已配置') {
        cons = cons + ' AND ( hr9_ehrunitid is not null )';
    }
