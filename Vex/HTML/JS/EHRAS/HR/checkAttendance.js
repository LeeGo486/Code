...
$(function () {
    ...
    if (filter.type == 'Attend') { //恢复到跳转前参数
        setCons(filter);
    }
    else {
        clearText();
...
function setCons(filter) {
    ...
    for (var i = 0; i < rows.length; i++) {
        if (filter.sid.toUpperCase() == rows[i].hr11_id.toUpperCase()) {
...
function setDataGrid() {
    ...
    $(m_DatagridID).datagrid({
        ...
        columns: [[
            ...
            { field: 'hr11_shopid', title: '店铺ID', width: 70 },
            { field: 'hr11_shopname', title: '店铺名称', width: 280 },
            { field: 'hr11_atmonth', title: '月份', width: 70 },
            { field: 'hr11_state', title: '状态', width: 60 },
            { field: 'hr11_rguser', title: '创建人', width: 70 },
            {
                field: 'hr11_rgdt', title: '创建时间', width: 130,
            ...
            { field: 'hr11_lmuser', title: '修改人', width: 70 },
            {
                field: 'hr11_lmdt', title: '修改时间', width: 130,
                formatter: function (val, row, index) {
                    if (val) {
                        return dateUtil_dateFomaterA(new Date(val), '-');
                    }
            ...
            { field: 'hr11_uptno', title: '修改次数', width: 60,align:'center' }
        ]],
        rownumbers: true,
        singleSelect: true,
        onDblClickRow: function (rowIndex) {
            ...
            if (m_UserInfo.actor == 'A') {
                ...
                curl = 'checkAttendanceAdd.html?' + 'v0=' + v0 + '&v1=' + v1
                    + '&v2=' + v2 + '&l3=考勤查询' + '&s=' + dateUtil_dateFomaterA(new Date(), '-');
            } //else {
            //    // 跳转到明细页 
            //    var v0 = "old";
            //    //待办or查询
...
function actAdd() {
    ...
    if (curl != '') {
        parent.menus_menus_turnPage('B', 'checkAttendance', curl);
    } else {
        alert('url不能为空！');
    }
}
