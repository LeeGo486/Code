...
namespace HZY.COM.WS.DLWXSite
{
    ...
    public class ThemeRepository : HZYLogicBase, ILogicGetData
    {
        ...
        #region 内部变量
        private Dbconn conn = new Dbconn("DLWXWZ");
        #endregion
        ...
        protected virtual DataSet GetList(DataTable pDataTable)
        {
            string strSql = @"
                SELECT  [TID]
                    ...
                    ,[ThemeState]
                    ,[UpdateTime]
                FROM [dbo].[DL_Theme] Where 1=1 " + m_hzyPageInfo.Where;
        ...
        protected virtual bool Save(DataTable pDataTable)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(themeId)) //如果为空，说明是插入数据
                {
                    ...
                    conn.Insert("DL_Theme", dtAdd); //插入数据
                }
                else
                {
                    //更新
                    ...
                    conn.Update("DL_Theme", dtAdd, strWhere); //更新数据
                }
        ...
        protected virtual bool Delete(DataTable pDataTable)
        {
            try
            {
                ...
                conn.Delete("DL_Theme", strWhere);    //删除参数
