...
$(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (eval("[" + data + "]")[0].Error) {
                ...
                window.location.href = "login_SPWX.html";
                return;
            }
    ...
    if ($.cookie('easyuiThemeName1' + window.hzy_WebName)) {
        ...
        $("#styleTheme").combobox('setValue', 'defaultForDepartmentStore');
    }
...
function Logout() {
    $.ajax({
        ...
        success: function (data) { // 请求成功后处理函数。
            ...
            if (data.Result != null) {
                //$("#login_msg").html("登录成功");
                window.location = "login_SPWX.html";
...
function getDepotInfo() {
    var url = GetWSRRURL('3a9cf88e-fc42-485d-9dff-96c35a8a1750')
        + "&XML=" + GetFormJson([], 'getDepotInfo');

    $.post(url, function (data) {

        $.cookie('depotInfo', data, {path:'/'});
