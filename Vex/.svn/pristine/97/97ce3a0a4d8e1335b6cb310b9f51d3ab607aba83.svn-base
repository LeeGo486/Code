    ...
    public class Contract : HZYLogicBase, ILogicGetData
    {
        ...
        private void Get(string SQLWhere)
        {
            string strSQL = @"
                            ...
                            LEFT JOIN [M01_CONF].[CT0_CSYTAB] [b] ON [a].[pc1_paymenttype] = [b].[CT1_OptionsValues] AND [b].[CT1_Code] = 'TEPY'
                            LEFT JOIN [M01_CONF].[CT0_CSYTAB] [c] ON [a].[pc1_currency] = [c].[CT1_OptionsValues] AND [c].[CT1_Code] = 'CUCD'
                            LEFT JOIN [M01_CONF].[CT0_CSYTAB] [d] ON [a].[pc1_paymentchannel] = [d].[CT1_OptionsValues] AND [d].[CT1_Code] = 'PYTP'
                            LEFT JOIN [B01_MDM].[SP1_Supplier] [e] ON [a].[PC1_SP1_SupplierCode] = [e].[SP1_SupplierCode]
                            WHERE  1=1 AND [a].[PC1_StateCode] <> '3'" + SQLWhere + m_hzyPageInfo.Where;
        ...
        private void Insert()
        {
            ...
            m_RequestXML.Tables["list"].Rows[0]["pc1_ContractCode"] = AutoKeyCode.GetMaxKeyCode(strCodyType, strRegionCode, strPOTypeCode, "", conn);
        ...
        private void CheckLink()
        {
            for (int i = 0; i < m_RequestXML.Tables["list"].Rows.Count; i++)
            {
                ...
                string strSql = "SELECT 1 FROM [B02_BILL].[PO1_POHead] where PO1_PC1_ContractCode = '" + strCO + "'";
