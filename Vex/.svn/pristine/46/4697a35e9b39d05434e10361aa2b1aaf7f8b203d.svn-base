...
function actsgmxlbcsh1(dgid) {
    $(dgid).datagrid({
			    ...
			    , editor: {
			        type: 'combobox', options: {
			            ...
			            onSelect: function (record) {
			                ...
			                //参考价
			                var ed2 = $(dgid).datagrid('getEditor', { index: m_LastIndex, field: 'applyprice' });
        ...
        onClickRow: function (rowIndex) {
            ...
            //绑定员工号带出姓名
            getEmpNameByCode(rowIndex, 'usercode', 'username');
        ...
        toolbar: [{
            ...
            handler: function () {
                ...
                //绑定员工号带出姓名
                getEmpNameByCode(m_LastIndex, 'usercode', 'username');
            }
        }, '-', {
            text: '复制',
            iconCls: 'icon-design',
...
function setbillInfo(ctype, cdata) {
    if (ctype == 'new') {
        ...
        if (m_UserInfo.deptstree.length == 1 && (m_UserInfo.deptstree[0].children == undefined
            ...
            $('#CONO').attr('value', m_UserInfo.deptstree[0].attributes.OZ1_Brand);
            $('#DIVI').attr('value', basedata_getCommonData('DIVI'));  
        } else { 
            $('#CONO').attr('value', '');
            $('#DIVI').attr('value', '');
        ...
        $('#DeptID').combotree('setText', cdata.fa1_deptname);
...
function setGDZCxx() {
    ...
    for (var i = 1; i < rowsData.length + 1; i++) {
        ...
        if (row.sgnumber * 1 > row.bg1_appfulnum * 1) {
...
function turnToCheck() {
    ...
    if (!actSaveZ()) {
        ...
        //预算校验(申请人提交时)
        var vstate = basedata_getCodename(m_APPState, $('#State').attr('value'), 'ct1_codename', 'ct1_code'); 
        if (vstate != 1 && vstate != 99 && vstate != 98
            && $('#BudgetType').combobox('getValue') == 0) {
 ...
 *  DLY 
 */
function getEmpNameByCode(rowIndex, codefield, namefield) {
    var ed1 = $(m_SGMXdgId).datagrid('getEditor', { index: rowIndex, field: codefield });
    if (ed1) {
        $(ed1.target).bind("keydown", function (event) {
            if (event.keyCode == 13) {
                var ed2 = $(m_SGMXdgId).datagrid('getEditor', { index: rowIndex, field: namefield });
                ed2.target.combobox('setValue', '');
                var cXML = '<Page>1</Page><Num>10</Num>'
                         + '<Cons>' + ' AND ee1_empCode = \'' + ed1.target.val() + '\''
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data && data.rows && data.rows.length>0) {
                            var empname = dataUtil_undefinedOrNull(data.rows[0].ee1_empname, '');
                            if (empname != '') {
                                ed2.target.combobox('setValue', empname); 
                                flag = 1;
                            }
                        } 
                        if (flag == 0) {
