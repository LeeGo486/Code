var m_columns = null;
var editIndex;
var m_ViewTableName = "V_L0013_FOR_Depot_Businessdevelopment_JM";
...
function onClickCellOldShopOptimization() {
    ...
    if (selected.地址 == '') {
        ...
        $('#txt地址').val(selected.地址)
    }


    $('#txt楼层').numberbox('setValue', '');
    $('#txt合同面积').numberbox('setValue', '');
...
function YH_Submit() {
    $('#form_Edit').form('submit', {
        onSubmit: function (e) {
            ...
            data[data.length] = { "name": "txt拓展类型", "value": '老店优化' }
...
function Revoke_save() {
    ...
    qwe = selected.店铺名称
    $.messager.confirm('提示框', '你确定要将  ' +qwe+ '  撤店? 此操作是不可逆的，请谨慎操作。', function (r) {
        if (r) {
