...
/****** Object:  StoredProcedure [API_GLERP].[Sp_PRAD_ArrivalPlanDetail_TEST]    Script Date: 09/12/2013 10:34:09 ******/
SET ANSI_NULLS ON
GO
...
CREATE proc [API_GLERP].[Sp_PRAD_ArrivalPlanDetail_TEST]
...
-----自动生成	
    ...
    DECLARE @PRAD_LmUser  VARCHAR(50)
...
--设置帮助
	if @InputFormat in ('AML','【,】')  
		begin
			...
			'【操作类型,NEW】【实体,B02_Bill.PRA_ArrivalPlan】【新增内容,必填】【条件内容,】【《W》GET_XML,/ROOT/LIST/ROW】'+
      ...
      PRINT @PRAD_PRA_ArrivalPlanId
		...
		BEGIN
		     ...
		     set @MapString1 = 
                    ...
                    '【供应商款号,		        B.DC0_DictName1~SKUSU_DictName ,	String,'													   +'】' + 
                    '【款号,					C.DC0_DictName1~SKU_DictName,		String,'													   +'】' + 
		...
		PRINT @SelectFieldList1	
			...
			,PR1D_Total,PR1D_POriginalPrice,PR1D_PCurrencyPrice,PR1_PurchaseTotal,PR1_PurchaseAmount,PR1D_SCurrencyPrice,B.DC0_DictName1,PR1_SU0_SupplierId,C.DC0_DictName1,D.DC0_DictName1'
...
if @mdl='NEW'  
	...
	begin
      ...
      --FROM  B02_Bill.PRA_ArrivalPlan WHERE PRA_ArrivalPlanId=@PRA_ArrivalPlanId
		...
		set @List=isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《W》到货计划明细Get_XML'),'')
 
		exec [~FrameDB].API_XML.SelectXMLAsTable 
		@List
		,'/ROOT/LIST/ROW'
		,2
		,'( PRAD_PerchaseTotal varchar(50),PRAD_ArrivalTotal varchar(50),PRAD_PlanArrival varchar(50),PRAD_POriginalPrice varchar(50),
			PRAD_PCurrencyPrice varchar(50),PRAD_PD0_ProductId  varchar(50),PRAD_UnitID varchar(50),PRAD_PR1D_OrderDetailId varchar(50),
			PRAD_SCurrencyPrice varchar(50),PRAD_ArrivalDate varchar(50),PRAD_PlanTotal varchar(50))'
		,@XmlTempTableName output
    
 
		SET @SqlString='INSERT INTO  B02_Bill.PRAD_ArrivalPlanDetail(PRAD_CONO,PRAD_DIVI,	
		PRAD_ArrivalPlanDetailId,PRAD_UptNo,PRAD_RgDt,PRAD_LmDt,PRAD_TotalDiff,PRAD_PlanArrivalAmount,PRAD_PRA_ArrivalPlanId,PRAD_PD0_ProductId,' 
 		+'PRAD_UnitID,PRAD_PR1D_OrderDetailId,PRAD_POriginalPrice,PRAD_PCurrencyPrice,PRAD_SCurrencyPrice,PRAD_ArrivalDate,PRAD_PerchaseTotal,'
 		+'PRAD_ArrivalTotal,PRAD_PlanTotal,PRAD_PlanArrival,PRAD_Status,PRAD_RgUser,PRAD_LmUser)'
		+'  SELECT '''+@PRAD_CONO+''','''+@PRAD_DIVI+''',newID(),''0'','''+@PRAD_RgDt+''','''+@PRAD_LmDt+''','
		+'cast(PRAD_PerchaseTotal as decimal(20,0))-cast(PRAD_ArrivalTotal as decimal(20,0))-cast(PRAD_PlanArrival as decimal(20,0)),' 
		+'cast(cast(PRAD_POriginalPrice as decimal(20,6))*cast(PRAD_PlanArrival as decimal(20,6)) as decimal(20,6)),'''+@PRA_ArrivalPlanId+''',PRAD_PD0_ProductId,PRAD_UnitID,PRAD_PR1D_OrderDetailId,PRAD_POriginalPrice,'
		+'PRAD_PCurrencyPrice,PRAD_SCurrencyPrice,PRAD_ArrivalDate,PRAD_PerchaseTotal,PRAD_ArrivalTotal,PRAD_PlanTotal,PRAD_PlanArrival,'''+@PRA_Status+''','
		+''''+@PRA_RgUser+''','''+@PRA_RgUser+''' '
...
if @mdl='UPDATE'  
	begin
		...
		set @PRAD_LmUser=isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《N》最后修改人'),'')
 	...
 	set @List=isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《W》到货计划明细Get_XML'),'')
		...
		,'/ROOT/LIST/ROW'
		,2
		,'( PRAD_ArrivalPlanDetailId varchar(50),PRAD_PerchaseTotal varchar(50),PRAD_ArrivalTotal varchar(50),PRAD_PlanArrival varchar(50),PRAD_POriginalPrice varchar(50),
		    PRAD_ArrivalDate varchar(50),PRAD_PlanTotal varchar(50),PRAD_Status varchar(50))'
		,@XmlTempTableName output
	
		 print @List
	 print @XmlTempTableName
 
		SET @SqlString='UPDATE  A SET A.PRAD_UptNo=CAST(A.PRAD_UptNo AS INT)+1,A.PRAD_Status=B.PRAD_Status, '
		+'A.PRAD_LmDt='''+@PRAD_LmDt+''', A.PRAD_TotalDiff=cast(B.PRAD_PerchaseTotal as decimal(20,0))-cast(B.PRAD_ArrivalTotal as decimal(20,0))-cast(B.PRAD_PlanArrival as decimal(20,0)),'
		+'A.PRAD_PlanArrivalAmount=cast(cast(B.PRAD_POriginalPrice as decimal(20,6))*cast(B.PRAD_PlanArrival as decimal(20,6)) as decimal(20,6)),'
		+'A.PRAD_PlanArrival=B.PRAD_PlanArrival,A.PRAD_ArrivalDate=B.PRAD_ArrivalDate,PRAD_LmUser='''+@PRAD_LmUser+'''  FROM B02_Bill.PRAD_ArrivalPlanDetail A,'+ @XmlTempTableName+ ' B '
		+' WHERE A.PRAD_ArrivalPlanDetailId=B.PRAD_ArrivalPlanDetailId'
 ...
 if @mdl='DELETE'  
	begin
 	 ...
 	 exec [~FrameDB].API_XML.SelectXMLAsTable 
		...
		,'/ROOT/LIST/ROW'
		,2
		,'( PRAD_ArrivalPlanDetailId varchar(50),PRAD_Status varchar(50))'
		,@XmlTempTableName output
 
 
		SET @SqlString='UPDATE  A SET A.PRAD_UptNo=CAST(A.PRAD_UptNo AS INT)+1,A.PRAD_Status=B.PRAD_Status, '
		+'A.PRAD_LmDt='''+@PRAD_LmDt+''',PRAD_LmUser='''+@PRAD_LmUser+''''
		+' FROM B02_Bill.PRAD_ArrivalPlanDetail A,'+ @XmlTempTableName+ ' B '
		+' WHERE A.PRAD_ArrivalPlanDetailId=B.PRAD_ArrivalPlanDetailId'
