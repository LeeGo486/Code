...
/****** Object:  StoredProcedure [API_GLERP].[Sp_FNP_ArrivalPlanPayApply_TEST]    Script Date: 09/12/2013 10:31:18 ******/
SET ANSI_NULLS ON
GO
...
CREATE proc [API_GLERP].[Sp_FNP_ArrivalPlanPayApply_TEST]
...
-----自动生成	
	...
	DECLARE @PRA_ArrivalPlanId	    VARCHAR(MAx)
	DECLARE @PRA_ArrivalPlanId1    VARCHAR(MAx)
    ...
    DECLARE @FNP_PayRate		VARCHAR(50)
    DECLARE @FNP_PayAmount		VARCHAR(50)
	DECLARE @FNP_DuesAmount		VARCHAR(50)
...
--设置默认值
	...
	begin
		...
		BEGIN	
           ...
           -- '【合同GUID,		 PR0_ContractId~PR0_ContractId,				String,'														     +'】' + 
            ...
            '【《W》到货计划GUID,	FNP_PR0_ContractId,				String,'+ isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《W》到货计划GUID'),'')		+'】' +
		...
		begin
		 ...
		 where  FNP_Status<>''E3CA9B7A-8E17-4E52-B667-39689445D4A1 ORDER BY FNP_LmUser'' ' 
	...
	*/
	...
	set @PRA_ArrivalPlanId= isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《W》到货计划GUID'),'')	 
    set  @PRA_ArrivalPlanId1=@PRA_ArrivalPlanId
    
   
	set @i=1
	set @setValue=''
	 
	while len(@PRA_ArrivalPlanId) >10
	 begin
	  -- print '1' + @PRA_ArrivalPlanId
	   set @getValue= substring(@PRA_ArrivalPlanId,charindex('/',@PRA_ArrivalPlanId,@i)+1, charindex('/',@PRA_ArrivalPlanId,2)-2)
       set @setValue=@setValue+ ','+''''+@getValue+''''
       
      -- print '2' + @setValue
       
       set @PRA_ArrivalPlanId=substring(@PRA_ArrivalPlanId,len(@getValue)+2,len(@PRA_ArrivalPlanId)-len(@getValue)-1)
	...
	-- print '3' + @setValue
		...
		set @MapString = 
			'【集团,		     PRA_CONO~DC0_CONO,								String,'														     +'】' + 
            '【事业体,		     PRA_DIVI~DC0_DIVI,								String,'														     +'】' + 
            '【申请单ID,		 ''''~FNP_PayApplyId,							String,'														     +'】' + 
            '【申请单编号,		 ''''~FNP_PayApplyCode,							String,'														     +'】' + 
            '【到货计划GUID,		 '''+@PRA_ArrivalPlanId1+'''~PRA_ArrivalPlanId,	String,'														     +'】' + 
            '【供应商GUID,		 SU0_SupplierId~SU0_SupplierId,					String,'														     +'】' + 
            '【申请单类型,		 ''''~FNP_Type,									String,'														     +'】' + 
            '【财务水单号,		 ''''~FNP_BankReceipt,							String,'														     +'】' + 
			'【付款总金额,		 SUM(PRA_PlanAmount)~FNP_PayAmount,				String,'														     +'】' + 
            '【付款百分比,		 ''''~FNP_PayRate,								String,'														     +'】' + 
			'【应付款金额,		 ''''~FNP_DuesAmount,							String,'														     +'】' +     
            '【是否已付款,		 ''''~FNP_IsPayed,								String,'														     +'】' +             
            '【状态,		     PRA_Status~FNP_Status,								String,'														     +'】' + 
			...
			'【银行账户,		 B.SU0_BankAccount~SU0_BankAccount,				String,'														     +'】' +   
			'【银行SWIFT号,		 B.SU0_BankSwift~SU0_BankSwift,					String,'														     +'】' +           
			'【状态名称,         C.DC0_DictName1~StatusDictName,			String,'														     +'】' +  
            ...
            '【《W》到货计划GUID,	PRA_ArrivalPlanId,		String,'+ isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《W》到货计划GUID'),'')		+'】' +
            '【《W》供应商GUID,	    PRA_SU0_SupplierId,		String,'+ isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《W》供应商GUID'),'')		+'】' +
		...
		Set @SqlString='select ' +  @SelectFieldList + ' 
         ...
         LEFT JOIN  B01_MDM.SU0_Supplier  B       ON    PRA_ArrivalPlanId=B.SU0_SupplierId
         LEFT JOIN  M01_CONF.DC0_Dictionary C     ON    PRA_Status=C.DC0_DictId    	      	    
		 where PRA_Status<>''E3CA9B7A-8E17-4E52-B667-39689445D4A1'' and PRA_ArrivalPlanId IN('+@setValue+')'+
		 'GROUP BY PRA_CONO,PRA_DIVI,SU0_SupplierId,B.SU0_SupplierCode,PRA_Status,B.SU0_Bank,B.SU0_BankAccount,B.SU0_BankSwift'
...
if @mdl='NEW'  
	begin
		...
		set @MapString =  
	      ...
	      -- '【申请单主表ID,				FNP_PayApplyId,			String,'+ isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《N》收货单主表ID'),'')	+'】' + 
            '【到货计划GUID,		PRA_ArrivalPlanId,			String,'+ isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《N》到货计划GUID'),'')	    +'】'+
           -- '【付款百分比,			FNP_PayRate,				String,'+ isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《N》付款百分比'),'')	     +'】' + 
		    '【付款总金额,			FNP_PayAmount,				String,'+ isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《N》付款总金额'),'')	     +'】' 
		...
		set @PRA_ArrivalPlanId=isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《N》到货计划GUID'),'')	
		...
		set @FNP_PayRate=isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《N》付款百分比'),'')
		set @FNP_PayAmount=isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《N》付款总金额'),'')
 
		SELECT @FNP_DuesAmount=CAST(CAST(@FNP_PayRate AS DECIMAL(20,2))*CAST(@FNP_PayAmount AS DECIMAL(20,2)) AS DECIMAL(20,2))
 ...
 PRINT @setValue
			...
			Set @SqlString1='INSERT INTO   B02_Bill.FNPD_PayApplyDetail(FNPD_CONO,FNPD_DIVI,FNPD_PayApplyDetailId,FNPD_FNP_PayApplyId,FNPD_FNP_PayApplyCode,
				...
				'''+@FNP_Type+''',PRA_ArrivalPlanId,'''+@FNP_Status+''','''+@FNP_RgUser+''','''+@FNP_RgDt+''','''+@FNP_RgUser+''','''+@FNP_LmDt+''','''+@FNP_UptNo+'''
				FROM B02_Bill.PRA_ArrivalPlan 
				WHERE PRA_ArrivalPlanId IN ('+@setValue+')'
			--	print @SqlString1
			EXEC sp_executesql @SqlString1
		...
		-- END 
		 ...
		 Set @SqlString='INSERT INTO  B02_Bill.FNP_PayApply(FNP_CONO,FNP_DIVI,FNP_PayApplyId,FNP_UptNo,FNP_RgDt,FNP_LmDt,FNP_PayApplyCode,FNP_DuesAmount,'
		 Set @SqlString=@SqlString+''+@InsertToFieldNameList+') VALUES('''+@FNP_CONO+''','''+@FNP_DIVI+''','''+@FNP_PayApplyId+''','		
		 Set @SqlString=@SqlString+''''+@FNP_UptNo+''','''+@FNP_RgDt+''','''+@FNP_LmDt+''','''+@FNP_PayApplyCode+''','''+@FNP_DuesAmount+''','+@InsertFromValueList+')'
...
if @mdl='UPDATE'  
	begin
		...
		Set @SqlString='UPDATE B02_Bill.FNP_PayApply SET FNP_UptNo=CAST(FNP_UptNo AS INT)+1,FNP_LmDt='''+@FNP_LmDt+''',FNP_DuesAmount='''+@FNP_DuesAmount+''',
