...
var m_OrderBy1 = 'CompanyID, PlanID, SalesTarget1'; //排序方式
...
function setDataGrid() {
    ...
    $(m_DatagridID).datagrid({
        ...
        columns: [[
            ...
            { field: 'planid', title: '提成方案ID', width: 100 },
            ...
            { field: 'remark', title: '提成方案描述', width: 330 },
...
function setDataGrid1() {
    ...
    $(m_DatagridID1).datagrid({
        ...
        columns: [[
            ...
            { field: 'salestarget1', title: '目标值起(%)', width: 100, align: "right"
				, editor: { type: 'numberbox', options: { precision: 2, min:0 }}},
            { field: 'salestarget2', title: '目标值止(%)', width: 100, align: "right"
				, editor: { type: 'numberbox', options: { precision: 2, min: 0 }}},
            { field: 'dedratio', title: '提成点(%)', width: 100, align: "right"
				, editor: { type: 'numberbox', options: { precision: 2, min: 0 }}}
        ]],
        rownumbers: true,
        singleSelect: true, 
...
function actLoadData(page, number) {
    ...
    var rows = hr_actSP_SalesDeductPlan(SPCons, m_UserInfo.userName); 
...
function actLoadData1(type) {
    ...
    if (type == 'edit') { 
        ...
        var res = hr_getSalesDeductPlan(cons, '1', '100', m_OrderBy1);
        if (res == '') {
            alert('查询方案明细失败！');
        } else { 
            rows = res.rows;
...
function upCfgdlg() {
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        var row1 = {};
        row1.id = rows[i].id;
        row1.salestarget1 = rows[i].salestarget1;
        row1.salestarget2 = rows[i].salestarget2;
        row1.dedratio = rows[i].dedratio;
    ...
    flag = hr_saveSalesDeductPlan(cXML1, m_UserInfo.userName);
    if (flag) {
        $('#cfgdlg').dialog('close');
        //刷新主数据
        actLoadData(m_PageNumber, m_PageSize);
