...
$(function () {
    ...
    m_data[0] = { "name": "txtdivi", "value": m_divi };
    initGird();
...
function initGird() {
    ...
    data1[data1.length] = { "name": "txtDIVI", "value": m_divi };
...
function savePR1_PaymentRequisit(type) {
    ...
    if (type == "insert") {
        ...
        data[data.length] = { "name": "txtpr1_divi", "value": m_divi };
...
function savePB1_PaymentBill(type) {
    ...
    if (type == "insert") {
        ...
        data[data.length] = { "name": "txtpb1_divi", "value": m_divi }
        ...
        $.messager.confirm('提示框', '是否确认保存', function (r) {
            if (r) {
                ...
                data[data.length] = m_data[0]; //添加事业体条件
                //如果选择完结按钮勾上了，就赋值85
...
function initGirdSP2_SupplierPool(name, psp2_sp1_suppliercode) {
    $(name).datagrid(
        {
            title: '资金池', //表格标题  7f2be735-a699-49e7-a0c2-99343ea2f0f7
            url: GetWSRRURL('7f2be735-a699-49e7-a0c2-99343ea2f0f7') + "&XML=" + GetFormJson([], 'GET') + "&WHERE=" + " SP2_SP1_SupplierCode = '" + psp2_sp1_suppliercode + "' AND SP2_DIVI='" + m_divi + "'",
            //url: GetWSRRURL('e2ae5182-359b-45fa-b268-f6ee6e5c3c88') + "&XML=" + GetFormJson(m_data, 'GET') + "&WHERE=" + " SP2_SP1_SupplierCode = '" + psp2_sp1_suppliercode + "'",
            //sortName: 'po1_pocode', //排序字段
...
function saveEndHead() {
    ...
    if (selectRowForPoHead.unrptotalamount == 0) {
        ...
        htmlobj = $.ajax({
            url: GetWSRRURL('7f2be735-a699-49e7-a0c2-99343ea2f0f7'),
            data: { "XML": xmlData, "WHERE": "SP2_Currency='" + selectRowForPoHead.currency + "'  and SP2_SP1_SupplierCode = '" + selectRowForPoHead.suppliercode + "' AND SP2_DIVI='" + m_divi + "'" },
            type: 'post',
            async: false
        });
        var result = $.parseJSON(htmlobj.responseText);
