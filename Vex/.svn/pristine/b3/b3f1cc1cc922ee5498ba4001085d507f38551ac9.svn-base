...
var m_data = [];
var m_tabs = {
    "年份": "YEAR", "款式类别": "ITCL", "状态": "SATE", "单位": "UNIT", "国家": "COTY", "执行标准": "STRD", "安全技术类别": "STCL", "大类": "ITGR", "小类": "ITGL",
    "币种(汇率)": "CUCD", "税率": "VTCD", "付款方式": "TEPY", "供应商分类": "SORT", "供应商类别": "CATE", "供应商特性": "FEAT", "批准文号": "RNOA", "季节": "SSON"
}
//初始化
$(function () {
...
function InitGrid(table, where, myTitle) {
    var url = GetWSRRURL('597aac46-2fc7-43e7-81a6-cddf9ba869b8') + "&XML=" + GetFormJson(m_data, 'Get') + "&Where=CT1_Code='" + where + "'";
    $('#' + table).datagrid({
        url: url,
...
function btnEdit(table, name) {
    ...
    var html = "<tr><td style=\"text-align:right\">编  号:</td><td><input id=\"keyId\" style=\"width:80px\" disabled=\"disabled\" /></td></tr>" +
    ...
    //初始化状态
    $("#cmbState").combobox({
        url: GetComboxWSRRURL('597aac46-2fc7-43e7-81a6-cddf9ba869b8') + "&XML=" + GetFormJson(m_data, 'GET') + "&Where=CT1_Code ='SATE'",
        valueField: 'id',
        textField: 'text',
        panelHeight: 'auto',
        editable: false,
...
function saveEdit(id, name) {
    ...
    //var url = GetWSRRURL('597aac46-2fc7-43e7-81a6-cddf9ba869b8') + "&XML=" + GetFormJson(data, 'Edit');
