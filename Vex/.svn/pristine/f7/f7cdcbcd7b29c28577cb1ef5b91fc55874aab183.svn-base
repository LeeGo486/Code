    ...
    public class CheckWorkAttendance : HZYLogicBase, ILogicGetData
    {
        ...
        public DataSet GetData()
        {
            try
            {
                ...
                else if (dt_OPTYPE.Rows[0][0].ToString().ToUpper() == "PromiseEdit".ToUpper())//行动计划新增
                {
                    PromiseEdit(dtList);
                }

                else if (dt_OPTYPE.Rows[0][0].ToString().ToUpper() == "PromiseSearch".ToUpper())//查询行动计划
                {
                    PromiseSearch(dtList);
                }
        ...
        private void CheckDologin(DataTable pdt)
        {
            ...
            string strCheckDologin = @"  
                                    SELECT  t2.C_PASSWORD ,
                                            t1.C_OID ,
                                            t1.C_EMPLOYEEID ,
                                            t1.C_CODE ,
                                            t1.C_IDCARD ,
                                            t1.C_NAME ,
                                            t1.C_EMPLOYEEID C_OID_USER ,
                                            t1.C_JOBNAME ,
                                            CASE WHEN CHARINDEX('\', REVERSE(t1.C_UNITname)) > 0
                                                 THEN REVERSE(SUBSTRING(REVERSE(t1.C_UNITname), 1,
                                                                        CHARINDEX('\', REVERSE(t1.C_UNITname)) - 1))
                                                 ELSE t1.C_UNITname
                                            END C_UNITname
                                    FROM    ehr.TB_INF_EMPLOYEE t1
        ...
        private void ApplyOvertime(DataTable pdt)
        {
            ...
            if (strSP1 != "" && strSP2 != "")//如果有2个审批人的时候
            {
                ...
                //22
                string strSQL22 = @"
                       update ehr.TB_TMG_APPROVEFLOW set C_APPROVEDATE= @Param0 , C_REMARK= @Param1 , C_RESULT= @Param2 , C_APPROVEEMPID= @Param3, C_NXTAPPOVEID= @Param4 , C_PREAPPOVEID= @Param5 , 
        ...
        private void ApplyVacation(DataTable pdt)
        {
            ...
            if (strSP1 != "" && strSP2 != "")//如果有2个审批人的时候
            {
                ...
                //22
                string strSQL22 = @"
                       update ehr.TB_TMG_APPROVEFLOW set C_APPROVEDATE= @Param0 , C_REMARK= @Param1 , C_RESULT= @Param2 , C_APPROVEEMPID= @Param3 , C_NXTAPPOVEID= @Param4 , C_PREAPPOVEID= @Param5 , C_OPERATOR= @Param6 ,
        ...
        /// 行动计划查询
        /// </summary>
        private void PromiseEdit(DataTable pdt)
        {
            bool bResult = false;
            DateTime timeNowPromise = DateTime.Now;
            string strActionPlan1 = pdt.Rows[0]["actionplan1"].ToString();
            string strActionPlan2 = pdt.Rows[0]["actionplan2"].ToString();
            string strActionPlan3 = pdt.Rows[0]["actionplan3"].ToString();
            string strc_unitname = pdt.Rows[0]["c_unitname"].ToString();
            string strC_Code = pdt.Rows[0]["c_code"].ToString();
            string strC_Jobname = pdt.Rows[0]["c_jobname"].ToString();
            string strUserNameC = pdt.Rows[0]["UserNameC"].ToString();

            string strflagC_Code = m_conn.GetDataTableFirstValue(@"
                             SELECT count(*) FROM dbo.Promise with(nolock)
                             WHERE c_code = @Param0", new string[] { strC_Code }).ToString();

            pdt.Columns.Add("updatename");
            pdt.Columns.Add("updatetime");
            pdt.Rows[0]["updatename"] = strUserNameC;
            pdt.Rows[0]["updatetime"] = timeNowPromise;


            m_conn.BeginTransaction();
            if (strflagC_Code == "1")
            {
                bResult = m_conn.Update("Promise", pdt, " c_code='" + strC_Code + "'");
            }
            else
            {
                pdt.Columns.Add("createname");
                pdt.Columns.Add("createtime");
                pdt.Rows[0]["createname"] = strUserNameC;
                pdt.Rows[0]["createtime"] = timeNowPromise;
                bResult = m_conn.Insert("Promise", pdt);
            }
            m_conn.CommitTransaction();
            if (bResult)
            {
        ...
        /// 行动计划查询
        /// </summary>
        private void PromiseSearch(DataTable pdt)
        {
            string strPromiseWhere = pdt.Rows[0]["PromiseWhere"].ToString();
            string strPromiseSQL = @"SELECT*FROM promise ";
            ds_Return = m_conn.GetDataSet(strPromiseSQL + strPromiseWhere, new string[] { });
        }
        #endregion
