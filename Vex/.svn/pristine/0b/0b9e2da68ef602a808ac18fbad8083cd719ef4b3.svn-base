...
/****** Object:  StoredProcedure [API_GLERP].[Sp_FNP_ReturnPayApply]    Script Date: 09/12/2013 10:32:55 ******/
SET ANSI_NULLS ON
GO
...
CREATE proc [API_GLERP].[Sp_FNP_ReturnPayApply]
...
-----自动生成	
	...
	DECLARE @PRC_ReturnId	    VARCHAR(MAx)
	DECLARE @PRC_ReturnId1    VARCHAR(MAx)
...
--设置默认值
	...
	begin
		...
		BEGIN	
           ...
           -- '【合同GUID,		 PR0_ContractId~PR0_ContractId,				String,'														     +'】' + 
            ...
            --'【《W》退货单GUID,		FNP_PR0_ContractId,				String,'+ isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《W》到货计划GUID'),'')		+'】' +
	...
	*/
	...
	set @PRC_ReturnId=isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《W》退货单GUID'),'')	 
    set  @PRC_ReturnId1=@PRC_ReturnId
    
   
	set @i=1
	set @setValue=''
	 
	while len(@PRC_ReturnId) >10
	 begin
	  -- print '1' + @PRA_ArrivalPlanId
	   set @getValue= substring(@PRC_ReturnId,charindex('/',@PRC_ReturnId,@i)+1, charindex('/',@PRC_ReturnId,2)-2)
       set @setValue=@setValue+ ','+''''+@getValue+''''
       
      -- print '2' + @setValue
       
       set @PRC_ReturnId=substring(@PRC_ReturnId,len(@getValue)+2,len(@PRC_ReturnId)-len(@getValue)-1)
	...
	-- print '3' + @setValue
		...
		set @MapString = 
			'【集团,		     PRC_CONO~FNP_CONO,								String,'														     +'】' + 
            '【事业体,		     PRC_DIVI~FNP_DIVI,								String,'														     +'】' + 
            '【申请单ID,		 ''''~FNP_PayApplyId,							String,'														     +'】' + 
            '【申请单编号,		 ''''~FNP_PayApplyCode,							String,'														     +'】' + 
            '【退货单GUID,		 '''+@PRC_ReturnId1+'''~PRC_ReturnId,			String,'														     +'】' + 
            '【供应商GUID,		 SU0_SupplierId~SU0_SupplierId,					String,'														     +'】' + 
            '【申请单类型,		 ''''~FNP_Type,									String,'														     +'】' + 
            '【财务水单号,		 ''''~FNP_BankReceipt,							String,'														     +'】' + 
			'【付款总金额,		 SUM(PRC_Amount)~FNP_PayAmount,				    String,'														     +'】' + 
            '【付款百分比,		 ''1''~FNP_PayRate,								String,'														     +'】' + 
			'【应付款金额,		 SUM(PRC_Amount)*''1''~FNP_DuesAmount,			String,'														     +'】' +     
            ...
            '【《W》退货单GUID,		PRC_ReturnId,			String,'+ isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《W》退货单GUID'),'')		+'】' +
            '【《W》供应商GUID,	    PRC_SU0_SupplierId,		String,'+ isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《W》供应商GUID'),'')		+'】' +
		...
		Set @SqlString='select ' +  @SelectFieldList + ' 
         ...
         LEFT JOIN  B01_MDM.SU0_Supplier  B       ON    PRC_SU0_SupplierId=B.SU0_SupplierId
         LEFT JOIN  M01_CONF.DC0_Dictionary C     ON    PRC_Status=C.DC0_DictId    	
         	    
		 where PRC_Status<>''E3CA9B7A-8E17-4E52-B667-39689445D4A1'' AND PRC_ReturnId IN('+@setValue+')'+
		 'GROUP BY PRC_CONO,PRC_DIVI,SU0_SupplierId,B.SU0_SupplierCode,B.SU0_Bank,B.SU0_BankAccount,B.SU0_BankSwift'
...
if @mdl='NEW'  
	begin
		...
		set @MapString =  
	      ...
	      -- '【申请单主表ID,				FNP_PayApplyId,			String,'+ isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《N》收货单主表ID'),'')	+'】' + 
            '【退货单GUID,			PRC_ReturnId,			String,'+ isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《N》退货单GUID'),'')	    +'】'
           -- '【付款百分比,			FNP_PayRate,				String,'+ isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《N》付款百分比'),'')	     +'】' + 
		    --'【付款总金额,			FNP_PayAmount,				String,'+ isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《N》付款总金额'),'')	     +'】' 
		...
		set @PRC_ReturnId=isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《N》退货单GUID'),'')	
 ...
 PRINT @setValue
			...
			Set @SqlString1='INSERT INTO   B02_Bill.FNPD_PayApplyDetail(FNPD_CONO,FNPD_DIVI,FNPD_PayApplyDetailId,FNPD_FNP_PayApplyId,FNPD_FNP_PayApplyCode,
				...
				'''+@FNP_Type+''',PRC_ReturnId,'''+@FNP_Status+''','''+@FNP_RgUser+''','''+@FNP_RgDt+''','''+@FNP_RgUser+''','''+@FNP_LmDt+''','''+@FNP_UptNo+'''
				FROM B02_Bill.PRC_Return 
				WHERE PRC_ReturnId IN ('+@setValue+')'
			 --	print @SqlString1
			EXEC sp_executesql @SqlString1
...
if @mdl='UPDATE'  
	begin
		...
		Set @SqlString='UPDATE B02_Bill.FNP_PayApply SET FNP_UptNo=CAST(FNP_UptNo AS INT)+1,FNP_LmDt='''+@FNP_LmDt+''',
