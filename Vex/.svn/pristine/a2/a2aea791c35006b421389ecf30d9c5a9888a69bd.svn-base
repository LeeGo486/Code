...
//WSID:03eb4ea1-8706-4ec3-b50a-5f9bf0d92846
namespace HZY.COM.WS.GroupERP
{
    public class Goods : HZYLogicBase, ILogicGetData
    {
        ...
        private void GetGoodsInfo(string pstrQ)
        {
            string strSQL = @"
                                ...
                                FROM    [B01_MDM].[ST2_StyleColor] WITH(NOLOCK) t1
                                        ...
                                        SP1_Name LIKE '%" + pstrQ + "%' Order by ST1_LmDt AND ST1_DIVI='" + m_hzyMessage.Env_SN + "' " + m_hzyPageInfo.Where;
        ...
        private void GoodsInfoForPageList()
        {
            ...
            if (ds.Tables["list"].Columns.Contains("year"))
            {
                strWhere = " AND ST1_Years>='" + ds.Tables["list"].Rows[0]["ST2_SKCCode"].ToString() + "'";
            }
            string strSQL = @"
                              SELECT  * FROM    ( SELECT    t1.ST2_ImagePath ,--图片路径取SKC表
                                                                ...
                                                                ( SELECT CT1_KeyName
																  ...
																  AND CT1_Code='ITCL' AND CT1_DIVI=t3.ST1_DIVI ) AND CT1_OptionsValues=t3.ST1_LargeClass
																 )ST1_LargeClassName,
                                                                t3.ST1_SmallClass ,--款号表小类
                                                                ( SELECT CT1_KeyName
																  FROM [M01_CONF].[CT0_CSYTAB]
																  WHERE  CT1_OptionsValues=t3.ST1_SmallClass AND CT1_CodeName='小类' AND CT1_DIVI=t3.ST1_DIVI
																 )ST1_SmallClassName,
                                                                t3.ST1_Provenance , --款号表产地
                                                                t1.ST2_Ingredients ,  --款号成分
                                                                t3.ST1_Grade ,--款号等级
        ...
        private void EDITGoods()
        {
            string strBrand = "";
            //雅斓的brand为AW
            if (hzyMessage.Env_SN.ToUpper() == "GL")
            {
                strBrand = "AW";
            }
            else
            {
                strBrand = hzyMessage.Env_SN;
            }
            ...
            string strSQL = @"
                   SELECT COUNT(1) FROM [B01_MDM].[ST2_StyleColor] WITH(NOLOCK)
            ...
            if (strST1 == "" && strST2 == "") //新增
            {
                ...
                if (m_hzyMessage.Env_SN == "LX" || m_hzyMessage.Env_SN == "RS")  //添加斓绣独有的属性
                {
            ...
            else if (strST2 != "" && iCount == 0) //只新增ST2_StyleColor表，修改style表
            {
                ...
                string sqlSalePrice = @"
                   SELECT AVG(ST2_SalePrice) FROM [B01_MDM].[ST2_StyleColor] WITH(NOLOCK)
                ...
                if (decSalePrice == Convert.ToDecimal(ds.Tables["list"].Rows[0]["ST2_SalePrice"].ToString()))
                {
                    ...
                    dsCreatDS.Tables["dtST1"].Rows[0]["ST1_LmUser"] = this.hzyMessage.User_Name;
                    dsCreatDS.Tables["dtST1"].Rows[0]["ST1_DIVI"] = m_hzyMessage.Env_SN;
                    if (m_hzyMessage.Env_SN == "GL") //雅斓独有的属性
                    {
                        dsCreatDS.Tables["dtST1"].Rows[0]["ST1_Season"] = ds.Tables["list"].Rows[0]["ST1_Season"].ToString();
                    }
                    if (m_hzyMessage.Env_SN == "LX" || m_hzyMessage.Env_SN == "RS")//斓绣独有的属性
                    {
                        dsCreatDS.Tables["dtST1"].Rows[0]["ST1_Usedmethod"] = ds.Tables["list"].Rows[0]["ST1_Usedmethod"].ToString();   //添加使用方法
                        dsCreatDS.Tables["dtST1"].Rows[0]["ST1_Attention"] = ds.Tables["list"].Rows[0]["ST1_Attention"].ToString(); //注意事项
                        dsCreatDS.Tables["dtST1"].Rows[0]["ST1_PermitNo"] = ds.Tables["list"].Rows[0]["ST1_PermitNo"].ToString(); //批准文号
            ...
            else if (strST2 != "" && iCount != 0)  //修改
            {
                ...
                if (m_hzyMessage.Env_SN == "LX" || m_hzyMessage.Env_SN == "RS")
                {
                    dsCreatDS.Tables["dtST1"].Rows[0]["ST1_Usedmethod"] = ds.Tables["list"].Rows[0]["ST1_Usedmethod"].ToString();   //添加使用方法
                    dsCreatDS.Tables["dtST1"].Rows[0]["ST1_Attention"] = ds.Tables["list"].Rows[0]["ST1_Attention"].ToString();     //注意事项
                    dsCreatDS.Tables["dtST1"].Rows[0]["ST1_PermitNo"] = ds.Tables["list"].Rows[0]["ST1_PermitNo"].ToString();       //批准文号
        ...
        private void AddSKU()
        {
            ...
            for (int i = 0; i < ds.Tables["list"].Rows.Count; i++)
            {
                ...
                {
                    string strSQL = @"  SELECT COUNT(*) FROM B02_BILL.PO2_PODetail WITH(NOLOCK) 
                                        WHERE PO2_PD1_ProductCode='" + ds.Tables["list"].Rows[i]["st3_st2_skccode"].ToString()
                                                                     + ds.Tables["list"].Rows[i]["st3_size"].ToString()
                                                                     + "' AND PO2_DIVI='" + strEVN + "' AND PO2_State<'99'";
                    int n = Convert.ToInt32(conn.GetDataTableFirstValue(strSQL).ToString());
                    if (n < 1)
                    {
