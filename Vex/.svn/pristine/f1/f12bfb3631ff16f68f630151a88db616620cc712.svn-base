    ...
    public class POReceipt : HZYLogicBase, ILogicGetData
    {
        ...
        private void onExpandRow()
        {
            ...
            string strSql = @"
                            ...
                            LEFT JOIN B02_BILL.RP2_POReceiptDetail t4 WITH(NOLOCK) ON t1.RP1_ReceiptCode = t4.RP2_ReceiptCode
                            LEFT JOIN B02_BILL.PO1_POHead t2 WITH(NOLOCK) ON t1.RP1_PO1_POCode = t2.PO1_POCode
                            " + strWhere;
            dsReturn = conn.GetDataSet(strSql);
        }
        #endregion
        ...
        private void Search()
        {
            ...
            string strSQL = @"
                        ...
                        LEFT JOIN B02_BILL.RP1_POReceiptHead T2 WITH(NOLOCK) ON [T1].[PO1_POCode] = [T2].[RP1_PO1_POCode]
                        LEFT JOIN B02_BILL.RP2_POReceiptDetail T3 WITH(NOLOCK) ON T2.RP1_ReceiptCode = T3.RP2_ReceiptCode AND T3.RP2_State = '90'
        ...
        private void winGetDetailOld()
        {
            ...
            string strSQLTemp = @"
                          ...
                          LEFT JOIN B02_BILL.RP2_POReceiptDetail T2 WITH (NOLOCK) ON T1.PO2_LISeq = T2.RP2_LINumber 
                          ...
                          SELECT DISTINCT t1.AP2_APCode
	                        ...
	                        JOIN B02_BILL.AP1_APOHEAD t2 WITH (NOLOCK) ON t1.AP2_APCode = t2.AP1_APCode
	                        LEFT JOIN [B02_BILL].[RP1_POReceiptHead] t3 WITH (NOLOCK) ON t1.AP2_APCode = t3.RP1_AP1_APCode
	                        LEFT JOIN [B02_BILL].[RP2_POReceiptDetail] t4 WITH (NOLOCK) ON t3.RP1_ReceiptCode = t4.RP2_ReceiptCode
	                        AND t4.[RP2_State] = '90'
        ...
        private void winGetDetailNew()
        {
            string strPOCode = ds.Tables["list"].Rows[0]["pocode"].ToString();
            string strAPCode = ds.Tables["list"].Rows[0]["apcode"].ToString();
            string strWhere = " 1=1 PO2_DIVI = '" + strComp + "' ";
            string strSQL = "";
            if (strAPCode == "")
            {
        ...
        private void GetWH() {
            ...
            dsReturn.Tables.Add(dt_EditResult.Copy());
        }
        #endregion
