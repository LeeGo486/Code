    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (m_OwnedBrand.length > 0 && m_OwnedBrand[0].id) {
                ...
                getSampleWorker(m_OwnedBrand[0].text);
                GetCGSource(m_OwnedBrand[0].text);
...
function getPDReview(record) {
    //图稿评审人
    $.ajax({
        url: GetComboxALLColumnWSRRURL('d763e76a-f7e0-4fbc-a2f2-9a9956ecccb9', "", "derwpe", "derwpe") + "&XML=" + GetFormJson([{ "name": "txtDEBRAN", "value": record }, { "name": "txtzt", "value": m_zt }], 'GetPDReview'),
        async: true,
        success: function (data) {
...
function GetCGSource(record) {
    ...
    //采购来源
    $.ajax({
        url: GetComboxALLColumnWSRRURL('d763e76a-f7e0-4fbc-a2f2-9a9956ecccb9', "", "cbtx40", "cbtx15") + "&XML=" + GetFormJson([{ "name": "txtPLMKey", "value": record }, { "name": "txtzt", "value": m_zt }], 'GetCGSource'),
        async: true,
        success: function (data) {
...
function SearchClick() {
    ...
    var brand = $("#cmbBrand").combobox("getValue");
    m_Brand = brand;
    var start = "";
    var end = "";
    var designId = "";
    ...
    $('#tabList').datagrid(
        {
            ...
            columns: [[
                ...
                { field: 'orderseason', title: '订货会季节', width: 140 },
...
function getSample() {
    var rowData = $("#tabList").datagrid("getSelected"),
        smpData = [rowData.orderseason, rowData.measurementsid, m_UserID, rowData.brand],
        data = initArray(),
            ...
            if (checkMsg != "undefind") {
                if (rltLength == 1) {
                    if (rltJson.strt == "") {
                        ...
                        getBomInfo(smpData[0], smpData[1], strt, smpData[2]);
                    }
                } else {
                    getProduct(smpData[0], rltJson.rows);
                };
            } else {
                $.messager.progress("close");
...
function getBomInfo(_season, _sample, _strt, _muser) {
    var data = initArray(),
        xmlData = "",
        url = GetWSRRURL('adeb3eda-43df-4c82-a5b3-38f18c216ed8');
        ...
        try {
            ...
            if (rltLength > 0) {
                $("#win_list").datagrid({
                    ...
                    toolbar: [
                      {
                          ...
                          text: '确认样打印',
                          iconCls: 'icon-print',
                          handler: function () { printSample(_season, _muser); }
                      }, {
                          id: 'btnSampClose',
                          text: '关闭',
                          iconCls: 'icon-undo',
                    ...
                    onDblClickRow: function () { printSample(_season, _muser); }
                });
                $.messager.progress("close");
            };
        }
...
function printSample(_season,_muser) {
    var data = $("#win_list").datagrid("getSelected"),
    ...
    if (isObj && data.dyyyno != "") {
        ...
        $.messager.progress({ title: '请稍后', msg: '处理中' });
        initPrintGird(_muser);
    } else {
        $.messager.alert("警告", "请选择样衣进行打印。", "warning");
    };
}

function getProduct(_season,_source) {
    var win = new initWin();
    win.title = "采购来源";
        ...
        toolbar: [
          {
              ...
              text: '选择采购来源',
              iconCls: 'icon-search',
              handler: function () { affirmProduct(_season) }
          },
          {
              id: 'btnProClose',
              text: '关闭',
...
function affirmProduct(_season) {
    ...
    if (isObj && getRow.strt != "") {
        ...
        getBomInfo(_season, getRow.measurementsid, getRow.strt, m_UserID);
    }
    else {
