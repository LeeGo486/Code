...
function stateContral(state) { 
    if (state == '已审核' || state == '已取消') { 
        ...
        $('#fa3_department').prop("disabled", true);
    } 
}
...
function openDeptDlg() {
    if (deptdlgFlag == 0) {
        ...
        $(m_DatagridId1).datagrid({
            ...
            singleSelect: true,
            onDblClickRow: function (rowIndex, rowData) {
                $('#fa3_cono').attr('value', rowData.oz1_cono);
                $('#fa3_divi').attr('value', rowData.oz1_divi);
                $('#fa3_department').attr('value', rowData.oz1_orgname);
                $('#bmdlg').dialog('close');
            },
        });
        deptdlgFlag = 1;
