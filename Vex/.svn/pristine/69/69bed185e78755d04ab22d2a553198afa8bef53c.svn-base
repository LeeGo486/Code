...
$(function () {
    ...
    var url = GetWSRRURL('d2ac5391-d4af-4991-8ae8-55a23dacccdd') + "&XML=" + GetFormJson("", 'GETAPOHEAD') + "&WHERE=" + " AP1_DIVI = '" + m_divi + "'";
    InitGird_APO_Head(url);
});

//主页到货计划主表数据加载
function InitGird_APO_Head(url) {
    ClearGrid('#APOHeadList');
        ...
        toolbar: [
                {
                    ...
                    handler: function () {
                        ...
                        //var url = GetWSRRURL('d2ac5391-d4af-4991-8ae8-55a23dacccdd') + "&XML=" + GetFormJson(data, 'GETPODDETAIL') + "&WHERE=1=10000";
                        InitGrid_GoodList('');
                    }
                },
                '-',
...
function InitGird_APO_Detail(url) {
    $('#APODetailList').datagrid({
        ...
        columns: [[
                ...
                { field: 'ap2_amount', title: '到货计划金额' },
                { field: 'ap2_aplandt', title: '本次计划到货日期', width: 100, formatter: Formater.date },
...
function InitGrid_GoodList(url) {
    $('#tabGoodList').datagrid({
        ...
        columns: [[
            ...
            { field: "po2_pd1_supperiercolor", title: "供应商颜色/简" },
            { field: "po2_pd1_supperiersize", title: "供应商尺码" },
            { field: "po2_pd1_productcode", title: "SKU" },
            { field: "po2_dcurrencyprice", title: "采购价" },
            { field: "po2_number", title: "采购数量", formatter: function (value, row, index) { return parseFloat(value) } },
            { field: "total", title: "已做到货计划数量" },
            { field: "ap1_aplannumber", title: "本次计划到货数量", editor: { type: 'numberbox' }, required: true },
            { field: "ap2_aplandt", title: "本次计划到货日期", editor: { type: 'datebox', options: { formatter: DateFormatter, parser: DateParser } }, required: true }
        ]],
        toolbar: [
            {
                id: 'btn_AddAP',
...
function onClickRow(RowIndex, RowData) {
    ...
    var url = GetWSRRURL('d2ac5391-d4af-4991-8ae8-55a23dacccdd') + "&XML=" + GetFormJson(data, 'GETAPOLIST') + "&WHERE=" + " PO2_POCode = '"
...
function btnPO2_PODetailList() {
    ...
    if (PodCode != "" || PodCode != undefined) {
        ...
        var url = GetWSRRURL('d2ac5391-d4af-4991-8ae8-55a23dacccdd') + "&XML=" + GetFormJson(data, 'GETPODDETAIL') + "&WHERE=" + " PO2_POCode = '" + PodCode + "' AND PO2_DIVI='" + m_divi + "'";
...
function btnEditAP2_APODetail() {
    ...
    if (PurchaseCode == "" || PurchaseCode == undefined) {
        ...
        data[data.length] = { "name": "txtpurchasecode", "value": PurchaseCode };
        data[data.length] = { "name": "txtdivi", "value": m_divi };
        var url = GetWSRRURL('d2ac5391-d4af-4991-8ae8-55a23dacccdd') + "&XML=" + GetFormJson(data, 'GETAPOLIST') + "&WHERE=" + " AP2_APCode = '" + apcode + "'";
...
function InitGrid_AP2_APODetail(url) {
    $('#tabAddAPOdetail').datagrid({
        ...
        fit: true,
        rownumbers: true,
        onDblClickRow: onClickRowAP, //单击行进入编辑模式
        columns: [[
            { field: 'ck', checkbox: true, width: 15 },
            { field: "ap2_apcode", title: "到货计划编号" },
            { field: "ap2_state", title: "状态", width: 35 },
            ...
            { field: "ap2_aplandt", title: "本次计划到货日期", width: 120, editor: { type: 'datebox', options: { formatter: DateFormatter, parser: DateParser } } },
...
function btnSearch() {
    ...
    if (sku != undefined && sku != "") {
        strWhere = strWhere + " and AP1_APCode in (Select AP2_APCode FROM B02_BILL.AP2_APODetail WHERE AP2_DIVI='" + m_divi + "' AND AP2_PD1_ProductCode='" + sku + "')";
    }
    var url = GetWSRRURL('d2ac5391-d4af-4991-8ae8-55a23dacccdd') + "&XML=" + GetFormJson("", 'GETAPOHEAD') + "&WHERE=" + " AP1_DIVI = '" + m_divi + "'" + strWhere;
...
function onClickRowAP(index, row) {
    ...
    if (row.rpnumbersums == 0 && row.ap2_state <= 35) {  //rpnumbersums==0 表示没有收货，可以编辑到货数量和日期
        m_rowIndexAP = index;
...
function UpdateAPDetail() {
    ...
    if (updateRow.length < 1) {
        ...
        for (var i = 0; i < updateRow.length ; i++) {
            if (updateRow[i].po2_number < updateRow[i].othernum * 1 + updateRow[i].ap2_aplannumber * 1) {
                alert('不能超过采购数量');
                return false;
            }
        }
