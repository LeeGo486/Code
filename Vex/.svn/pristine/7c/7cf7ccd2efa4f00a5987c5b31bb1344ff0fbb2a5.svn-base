    ...
    public class FLEXPLMProduct : HZYLogicBase, ILogicGetData
    {
        ...
        private void GET()
        {
            ...
            if (dtList.Rows.Count > 0 && dtList.Columns.Contains("BeginDate"))
            {
                ...
                {
                    strSQL += " AND CONVERT(varchar(10),CreateDate,21)>= CONVERT(varchar(10),@param0,21)";
                }
            }
            ...
            if (dtList.Rows.Count > 0 && dtList.Columns.Contains("EndDate"))
            {
                ...
                if (strEndCreateDate != "")
                {
                    strSQL += " AND CONVERT(varchar(10),CreateDate,21)<= CONVERT(varchar(10),@param1,21)";
                }
            }
            ...
            if (strUserPost == "设计师")
            {
                if (dtList.Rows.Count > 0 && dtList.Columns.Contains("Designer"))
                {
					...
					";
                        string UserNameC1 = conn.GetDataTableFirstValue(@"
                                                                        SELECT  UserNameC FROM    Userinfo 
                                                                        WHERE   username = @Param0
                                                                        ", new string[] { m_hzyMessage.User_Name }).ToString();
            ...
            if (dtList.Rows.Count > 0 && dtList.Columns.Contains("DesignerId"))
            {
                ...
                if (strDesignerID != "")
                {
                    strSQL += " AND DesignerID= @param3";
                }

            }


            ds_Return = conn.GetDataSetForPageList(strSQL, new string[] { strBeginCreateDate, strEndCreateDate, strDesigner, strDesignerID, m_hzyMessage.User_Name }, m_hzyPageInfo);
        ...
        private void GetMaterial()
        {
            ...
            string strSQL = @"
                        ...
                        FROM [dbo].[PDFINFO] 
                        LEFT JOIN [B01_MDM].[MITMAS] ON [dbo].[PDFINFO].DFITNO = [B01_MDM].[MITMAS].MMITNO
        ...
        private void SearchDg()
        {
            ...
            if (checkCodeInMITMAS > 0)
            {
                ...
                WHERE MMSTAT = '20' AND [MMCONO] = '" + strCONO + "' AND [MMITGR] = '" + strITGR + "' AND [MMITNO] LIKE '%" + strITNO + @"%'
                GROUP BY  MITMAS.MMITNO,M3MITTRA.MTITNO,M3MITLOC.MLITNO,M3MITLOC.MLSTQT,M3MITTRA.MTACRF";
            };
        ...
        private void materialSave()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                conn.Update("[dbo].[PDFINFO]", dt, listKey);
            };
