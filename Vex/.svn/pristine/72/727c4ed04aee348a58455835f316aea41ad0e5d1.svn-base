...
///WSID:6539426e-4509-40a1-add8-3fc6b1712aa3
///LeeGo
///

namespace HZY.COM.WS.GroupERP
{
    class Defective : HZYLogicBase, ILogicGetData
    {
        #region 构造函数
        ...
        private void Get()
        {
            string strSQL = @"
                        ...
                        WHERE DD1_DIVI = '" + strComp + "' AND (DD1_RgDt BETWEEN DATEADD(mm, DATEDIFF(mm, 0, GETDATE()), 0) AND GETDATE())";
        ...
        private void GetOnExpandRow()
        {
            ...
            string strSQL = @"
                        ...
                        WHERE [DD1_DIVI] = '" + strComp + "' AND [DD1_ST1_StyleCode] = '" + strDDStyCode + "' AND [DD1_ST2_Color] = '" + strDDColor + @"'
        ...
        private void GetSKC()
        {
            ...
            //            string getSizeGroupSQL = @"SELECT ST2_SizeGroup 
            //                                      FROM [B01_MDM].[ST2_StyleColor] 
            //                                      WHERE [ST2_DIVI] = '" + strComp + @"' 
            //                                      AND [ST2_ST1_StyleCode]  LIKE '%" + strStyle + "%' AND ST2_DIVI='" + m_hzyMessage.Env_SN + "'";
            ...
            //                    WHERE CT1_DIVI = '" + strComp + @"' AND CT1_Code = 'SIDE' AND 
            //                    CT1_FatherID = (
            //                            SELECT CT1_ID FROM [M01_CONF].[CT0_CSYTAB] 
            //                            WHERE CT1_KeyID = '" + strSizeGroup + "' AND CT1_DIVI='" + m_hzyMessage.Env_SN + "') order by ct1_optionsvalues";

            string strGetSizeSQL = @" SELECT DISTINCT ST3_Size FROM B01_MDM.ST3_StyleColorSize 
                                      WHERE ST3_DIVI='" + m_hzyMessage.Env_SN + "' AND ST3_ST1_StyleCode='" + strStyle + "' ";
            ...
            string strSQL = @"
                        SELECT [ST2_ST1_StyleCode]
                              ...
                              ,[ST2_SizeGroup]
                        FROM [B01_MDM].[ST2_StyleColor]
                        WHERE [ST2_DIVI] = '" + strComp + @"' AND [ST2_State] = '20' 
                        AND [ST2_SupperierColor] NOT IN ('G','V')  
                        AND [ST2_ST1_StyleCode]  LIKE '%" + strStyle + "%'";
        ...
        private void SetDefective()
        {
            ...
            for (int i = 0; i < dt_DefList.Rows.Count; i++)
            {
                ...
                dt_DefList.Columns.Remove("st2_state");
        ...
        #region CretaeTable
        /// <summary>
        /// 用于创建降级品所需要的Ds,降级品的DataTable + 款式表
        /// </summary>
        /// <returns></returns>
            ...
            )
        {
            string strSQL0 = @"
                        ...
                        FROM [B03_TRANS].[DD1_Demotion]
                        WHERE [DD1_DIVI] = '" + strComp + "' AND [DD1_ST1_StyleCode] = '" + pstrStyCode + @"' 
                        AND [DD1_ST2_Color] = '" + pstrColor + @"' 
            ...
            string strSQL2 = @"
                        ...
                        FROM [B03_TRANS].[DD1_Demotion]
                        WHERE [DD1_DIVI] = '" + strComp + "' AND [DD1_ST2_SKCCode] = '" + pstrSKCCode + @"'
                        AND [DD1_Sources] = '" + pstrSou + "'";
            ...
            if (Geths["style"].ToString() == "0" && Geths["SKC"].ToString() == "0" && Geths["SKU"].ToString() == "0")
            {
                ...
                if (iSKC == 0)
                {
                    ...
                    //供应商SKC
                    pdsBulidDs.Tables["dtSC"].Rows[pdsBulidDs.Tables["dtSC"].Rows.Count - 1]["ST2_SupperierSKC"] = strSupplierCode 
                        + pdsBulidDs.Tables["dtSC"].Rows[pdsBulidDs.Tables["dtSC"].Rows.Count - 1]["ST2_SupperierColor"];
                };
                ...
                pdsBulidDs.Tables["dtDe"].Rows[iNum]["DD1_CONO"] = "HYFG";
                pdsBulidDs.Tables["dtDe"].Rows[iNum]["DD1_DIVI"] = strComp;
                ...
                pdsBulidDs.Tables["dtSCZ"].Rows[pdsBulidDs.Tables["dtSCZ"].Rows.Count - 1]["ST3_CONO"] = "HYFG";
                pdsBulidDs.Tables["dtSCZ"].Rows[pdsBulidDs.Tables["dtSCZ"].Rows.Count - 1]["ST3_DIVI"] = strComp;
        ...
        private void GetPrintSearch()
        {
            ...
            string strSQL = @"
                        SELECT [DD1_DemoteSKU]
                              ,[DD1_DemoteBarcode]
                        FROM [B03_TRANS].[DD1_Demotion]
                        WHERE [DD1_DIVI] = '" + strComp + "' AND [DD1_ST2_SKCCode] = '" + strSKC + @"' 
                        AND [DD1_ST3_Size] = '" + strSize + "' AND [DD1_Sources] = '" + strSour + "'";
            m_ReturnData = conn.GetDataSet(strSQL);
        }
        #endregion
        ...
        private DataSet dsData(string pstrStyle, string pstrSC)
        {
            ...
            string strSqlStyle = @"
                                SELECT [ST1_ID]
                                    ...
                                    WHERE [ST1_DIVI] = '" + strComp + @"' 
                                    AND [ST1_StyleCode] = '" + pstrStyle + "'";
            ...
            string strSqlSC = @"
                                    SELECT [ST2_ID]
                                          ...
                                          ,[ST2_LmUser]
                                      FROM [B01_MDM].[ST2_StyleColor]
                                      WHERE [ST2_DIVI] = '" + strComp + "' AND [ST2_SKCCode] = '" + pstrSC + "'";
        ...
        private Hashtable GetGoods(string pstrStyle, string pstrSKC, string pstrSKU)
        {
            ...
            strSqlSKC = @"
                        ...
                        FROM [B01_MDM].[ST2_StyleColor]
                        WHERE [ST2_DIVI] = '" + strComp + @"' 
                        AND [ST2_SKCCode] = '" + pstrSKC + "'";
            strSqlSKU = @"
                        SELECT [ST3_ID]
                        FROM [B01_MDM].[ST3_StyleColorSize]
                        WHERE [ST3_DIVI] = '" + strComp + @"' 
                        AND [ST3_SKUCode] = '" + pstrSKU + "'";
