    ...
    public class ThemeRepository : HZYLogicBase, ILogicGetData
    {
        ...
        public DataSet GetData()
        {
            try
            {
                ...
                for (int i = 0; i < dtOptype.Rows.Count; i++)
                {
                    ...
                    if (dtOptype.Rows[0][0].ToString().ToUpper() == "GET") //查询恩派雅库存量
                    {
                        dsReturn = GetList(dsRequest.Tables[i + 1]);
                    }
                    else if (dtOptype.Rows[0][0].ToString().ToUpper() == "GETALL") //查询恩派雅库存量
                    {
        ...
        protected virtual DataSet GetList(DataTable pDataTable)
        {
            string strSql = @"
                SELECT  [TID]
                    ...
                    ,[UpdateUser]
                    ,[UpdateTime]
                FROM [dbo].[NPaia_Theme] Where 1=1 " + m_hzyPageInfo.Where;
        ...
        protected virtual DataSet GetAllList(DataTable pDataTable)
        {
            string strSql = @"
                SELECT  [TID]
                    ...
                    ,[UpdateUser]
                    ,[UpdateTime]
                FROM [dbo].[NPaia_Theme] ORDER BY CreateTime DESC";
        ...
        protected virtual bool Save(DataTable pDataTable)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(themeId)) //如果为空，说明是插入数据
                {
                    ...
                    conn.Insert("NPaia_Theme", dtAdd); //插入数据
                }
                else
                {
                    //更新
                    ...
                    conn.Update("NPaia_Theme", dtAdd, strWhere); //更新数据
                }
        ...
        protected virtual bool Delete(DataTable pDataTable)
        {
            try
            {
                ...
                string strSql = @"
                        SELECT  *
                        FROM [dbo].[NPaia_View] WHERE ThemeID= '" + strThemeId + "'";
                int count = conn.GetDataTableRowCount(strSql);
                if (count > 0)
                {
                    throw new Exception("该主题下已有形象不能删除！");
                }
                ...
                conn.Delete("NPaia_Theme", strWhere);    //删除参数
