...
function initGird(XMLDate) {
    $('#tab_list').datagrid({
        url: GetWSRRURL('a93a14a4-fca6-4e6d-8660-96938adbb6bf'),
...
function btnReturn() {
    if (m_headRow == undefined) {
        ...
        XMLData = GetFormJson(data, "Handle");
        $.messager.confirm('提示框', '确认要将该单据进行退货吗？', function (r) {
            if (r) {
                $.ajax({
                    url: GetWSRRURL('a93a14a4-fca6-4e6d-8660-96938adbb6bf'),
                    type: 'post',
