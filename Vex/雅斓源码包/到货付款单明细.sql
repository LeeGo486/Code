...
/****** Object:  StoredProcedure [API_GLERP].[Sp_FNP_ArrivalPlanPayApplyDetail]    Script Date: 09/12/2013 10:32:02 ******/
SET ANSI_NULLS ON
GO
...
CREATE proc [API_GLERP].[Sp_FNP_ArrivalPlanPayApplyDetail]
...
--设置默认值
	...
	begin
		...
		BEGIN	
            ...
            '【采购金额,		 A.PRA_PlanAmount~PRA_PlanAmount,					String,'													 +'】' +                          
			'【采购数量,		 A.PRA_PlanTotal~PRA_PlanTotal,						String,'																 +'】' +           
            '【单据类型,		 C.DC0_DictName1~TypeDictName,						String,'														     +'】' +   
            '【状态对照,		 E.DC0_DictName1~StatusDictName,					String,'														     +'】' +                                    
			'【采购币种,		 F.DC0_DictName1~CurrencyDictName,					String,'														     +'】' +                                    
		...
		begin
			...
			LEFT JOIN B02_Bill.PRA_ArrivalPlan A     ON   FNPD_RefBillDetailID=A.PRA_ArrivalPlanId  
			LEFT JOIN  M01_CONF.DC0_Dictionary C     ON    FNPD_RefBillDetailType=C.DC0_DictId  
			LEFT JOIN  B02_Bill.PR1_Order D			 ON    A.PRA_PR1_OrderId=D.PR1_OrderId     
			LEFT JOIN  M01_CONF.DC0_Dictionary E     ON    FNPD_Status=E.DC0_DictId   
			LEFT JOIN  M01_CONF.DC0_Dictionary F     ON     D.PR1_CurrencyTypeId=F.DC0_DictId	    
			where  FNPD_Status<>''E3CA9B7A-8E17-4E52-B667-39689445D4A1  ORDER BY FNPD_LmUser'' ' 
