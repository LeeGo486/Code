...
/****** Object:  StoredProcedure [API_GLERP].[Sp_PRBD_ReceiveDetail]    Script Date: 09/12/2013 10:34:47 ******/
SET ANSI_NULLS ON
GO
...
CREATE proc [API_GLERP].[Sp_PRBD_ReceiveDetail]
	...
	--	set @PRAD_CONO=[~FrameDb].[API_Common].[ConvertNameListToSql](@MapString,'Select'	,'【集团】',0	)
		...
		BEGIN
		     ...
		     set @MapString1 = 
					...
					'【《W》收货单主表ID,		PRBD_PRB_ReceiveId,				String,'+ isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《W》收货单主表ID'),'')			+'】'  
		...
		begin
			...
			LEFT JOIN   B01_MDM.PD0_Product A			ON A.PD0_ProductId=PRBD_PD0_ProductId
			LEFT JOIN  M01_CONF.DC0_Dictionary  B ON	PRBD_Status=B.DC0_DictId    	     
			LEFT JOIN   M01_CONF.DC0_Dictionary D        ON PRBD_UnitID=D.DC0_DictId  
			LEFT JOIN   B02_Bill.PRB_Receipt E			 ON E.PRB_RECEIPTID=PRBD_PRB_ReceiveId
 ...
 print @SqlString
		...
		BEGIN
		  ...
		  set @MapString1 = 
                    ...
                    '【储位,					''''~PRBD_StorageId,									String,'													   +'】' + 
                    '【计划到货数量,			PRAD_PlanArrival,										String,'													   +'】' + 
                    '【计划到货金额,			PRAD_PlanAmount,										String,'													   +'】' +                     
...
if @mdl='UPDATE'  
	begin
		...
		set @MapString = 
           ...
           '【《W》到货计划明细ID,	PRBD_PRAD_ArrivalPlanDetailId,	String,'+ isnull([~FrameDB].API_COMMON.[GetValueFromList](@ConditionStr,'《W》到货计划明细ID') ,'')	+'】' + 
  ...
  if @mdl='DELETE'  
   ...
   IF @mdl<>'GET'  
    begin   
      ...
      SELECT @PRB_ReceiptTotal=SUM(ISNULL(CAST(PRBD_Total AS DECIMAL(20,0)),'0')),              
             @PRB_ReceiptAmount=SUM(CAST(PRBD_Total AS DECIMAL(20,2))*CAST(PRBD_POriginalPrice AS DECIMAL(20,6)))                                            
...
*/
     ...
     UPDATE  B02_Bill.PRAD_ArrivalPlanDetail SET PRAD_ArrivalTotal=@PRBD_Total
     WHERE   PRAD_ArrivalPlanDetailId=@PRBD_PRAD_ArrivalPlanDetailId
