    ...
    public class POReceipt : HZYLogicBase, ILogicGetData
    {
        ...
        public DataSet GetData()
        {
            try
            {
                ...
                else if (dt_OPTYPE.Rows[0][0].ToString().ToUpper() == "cmbGetAP".ToUpper())
                {    //下拉框获取AP
                    cmbGetAP();
                }
        ...
        private void cmbGetFailure()
        {
            string strSQL = @"
                            ...
                            WHERE [CT1_Code] = 'UQCD' AND [CT1_CONO] = 'HYFG' AND [CT1_DIVI] = 'LX'
        ...
        private void cmbWareHouse()
        {
            ...
            //储位
            string LocationCode = ds.Tables["LIST"].Rows[0]["LocationCode"].ToString();

            string strSQL = @"SELECT 
                                        [WH1_WareHouseCode]
                                       ,[WH1_Name] 
                                    FROM [B01_MDM].[WH1_WareHouse] 
                                    WHERE WH1_Indentify='DRP' AND WH1_WareHouseCode IN 
                                    (SELECT SA1_WH1_WareHouseCode FROM [B01_MDM].[SA1_Location] 
                                            WHERE SA1_LocationCode='" + LocationCode + "')";
