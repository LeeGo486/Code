...
namespace HZY.COM.WS.Hawk
{
    public class BasicData : HZYLogicBase, ILogicGetData
    {
        ...
        public DataSet GetData()
        {
            ...
            else if (dt_OPTYPE.Rows[0][0].ToString().ToUpper() == "isAPO".ToUpper()) //在反审核采购单子行的时候检查该商品是否已经做了到货计划 yjw-2015-4-8
            {
                isAPO();
            }
        ...
        private void GetSupplierPool()
        {
            ...
            string strCurrency = ds.Tables["list"].Rows[0]["po1_currency"].ToString();
            string strSql = @"  
                                SELECT [SP2_ID]
                                      ,[SP2_CONO]
                                      ,[SP2_DIVI]
                                ...
                                WHERE  SP2_DIVI='" + strDIVI + "' AND SP2_SP1_SupplierCode='" + strSupplierCode + "' AND SP2_Currency='" + strCurrency + "'"; //+m_hzyPageInfo.Where;
            ds_Return = m_conn.GetDataSet(strSql);
        }
        #endregion
        ...
        private void isAPO()
        {
            ...
            string strSQL = @"
                                SELECT COUNT(*) count
                                  FROM [Hawk].[B02_BILL].[AP2_APODetail]
                                  LEFT JOIN [Hawk].[B02_BILL].[AP1_APOHead] ON AP2_APCode=AP1_APCode AND AP2_CONO=AP1_CONO AND AP2_DIVI=AP1_DIVI
