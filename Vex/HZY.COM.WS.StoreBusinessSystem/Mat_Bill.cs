            ...
            try
            {
                ...
                for (int i = 0; i < ds.Tables["OPTYPE"].Rows.Count; i++)
                {
                                     ...
                                     SELECT *
                                                 FROM   ( SELECT    t5.j_com 公司 ,
                                                          ...
                                                          FROM      [mat].[mat_bill] t1
                                                                    ...
                                                                    LEFT JOIN [MDM].[MDM_Market2].[dbo].V_L0013_BASE t5 ON t5.j_depotid = t1.depotid
                                                        ) t1
                    ...
                    else if (dt_OPTYPE.Rows[i][0].ToString().ToUpper() == "SAVE_UPDATE")
                    {
                        ...
                        string strBillid = dt_List.Rows[0]["billid"].ToString();
                        DateTime timeNow = DateTime.Now;
                        string strtype = dt_List.Rows[0]["state"].ToString();
                        ...
                        {
                            dt_List.Columns.Add("updatetime");
                            dt_List.Columns.Add("end_crdate");
                            dt_List.Columns.Add("end_crname");
                            dt_List.Rows[0]["end_crdate"] = timeNow;
                            dt_List.Rows[0]["updatetime"] = timeNow;
                            dt_List.Rows[0]["end_crname"] = hzyMessage.User_Name_CN;
                        }
                        dt_List.Columns.Remove("depotid");
                        dt_List.Columns.Remove("billid");
                    ...
                    {
                        string strSQL = @" SELECT  DISTINCT j_dq j_dq1 ,j_dq FROM    [MDM].[MDM_Market2].[dbo].V_L0013_BASE " + m_hzyPageInfo.Where;

                        ds_Return = conn.GetDataSet(strSQL);
                    }

                    else if (dt_OPTYPE.Rows[i][0].ToString().ToUpper() == "GETCOM")
                    {
                        string strSQL = @"SELECT  DISTINCT j_com j_com1 ,j_com FROM    [MDM].[MDM_Market2].[dbo].V_L0013_BASE " + m_hzyPageInfo.Where;
                    ...
                    else if (dt_OPTYPE.Rows[i][0].ToString().ToUpper() == "CHEACKINSERT")
                    {
                        string strSQL = @" SELECT  COUNT(*)  COUNT
                                            FROM    [mat].[mat_bill]
                                            WHERE   EXISTS ( SELECT *
                                                                FROM   mat.mat_bill
                                                                WHERE  depotid = '" + strComList + "'AND state = 8 )";
                        ds_Return = conn.GetDataSet(strSQL);
                    }
