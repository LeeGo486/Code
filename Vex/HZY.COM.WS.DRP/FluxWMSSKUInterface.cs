...
{
    public class FluxWMSSKUInterface : HZYLogicBase, ILogicExecute
    {
        #region 构造函数
        public FluxWMSSKUInterface()
        {
        ...
        public Boolean Execute()
        {
            try
            {
                ...
                m_conn.Excute("update [dbo].[Put_SKU] set WMSFlag=2 where isnull(WMSFlag,0)=0 and SKU_GROUP4 NOT IN ('P','X','LX','RS','E','G','F','J','K','N','S','T','Z','D','B','Q')");
...
--						  )
                          ...
                          ORDER BY CreateTime DESC,Timestamps DESC 
                ...
                if (dtData.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dtData.Rows.Count; i++)
                    {
                        ...
                        skuHeader.RESERVEDFIELD06 = dtData.Rows[i]["RESERVEDFIELD06"].ToString();
                        skuHeader.RESERVEDFIELD07 = dtData.Rows[i]["RESERVEDFIELD07"].ToString();
                        skuHeader.RESERVEDFIELD08 = dtData.Rows[i]["styleid"].ToString();
