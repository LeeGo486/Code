...
// 创建人：刘洪
// 创建日期：2015-04-16
// WSID：cc7a0435-0968-4371-a845-00ef9e37d544
            ...
            try
            {
                ...
                else if (pstrCodeType == "新增收货单编号")
                {
                    ...
                    FROM    [B02_BILL].[RP1_POReceiptHead] WITH(NOLOCK)
                            where RP1_PO1_POCode = '" + pstrParam0 + "' AND RP1_DIVI='" + pstrParam1 + "'").ToString();
                ...
                else if (pstrCodeType == "新增退货单编号")
                {
                    ...
                    SELECT MAX(RT1_RetreatCode)
                    FROM [B02_BILL].[RT1_RetreatHEAD]
                    WHERE RT1_DIVI = '" + pstrParam0 + @"' 
                    AND RT1_RetreatCode LIKE 'RT" + pstrParam1 + "%'").ToString();
                    if (strMaxCliamCode != "")
                    {
                        string strNum = "";
                        if (strMaxCliamCode.Length == 15)
                        {
                            int nNum = Convert.ToInt32(strMaxCliamCode.Substring(11, 4)) + 1;
                        ...
                        else if (strMaxCliamCode.Length == 11)
                        {
                        ...
                        {
                            int nNum = Convert.ToInt32(strMaxCliamCode.Substring(11, 2)) + 1;
                    ...
                    {
                        strMaxCliamCode = "RT" + pstrParam1 + "0001";
                    }
                ...
                else if (pstrCodeType == "获取SKC最大号")
                {
                    string strMaxSKCCode = m_conn.GetDataTableFirstValue(@"
                                        SELECT ISNULL(MAX(ST1_StyleCode),'') maxskc
                                                ...
                                                WHERE ST1_StyleCode LIKE '" + pstrParam0 + "%'").ToString(); //2015-07-06  去掉环境 ST1_DIVI=pstrParam1
                    if (strMaxSKCCode == "")
                    {
                        if (pstrParam1 == "GL")
                        {
                ...
                else if (pstrCodeType == "获取SKC最大号2")  //新商品编码规则:雅斓两位流水码;斓绣和斓居两位流水码
                {
                    string strMaxSKCCode = m_conn.GetDataTableFirstValue(@"
                                        SELECT ISNULL(MAX(ST1_StyleCode),'') maxskc
                                                FROM [B01_MDM].[ST1_Style]
                    ...
                    else
                    {
                        ...
                        {
                            if ((Convert.ToInt32(strMaxSKCCode.Substring(7, 2)) + 1) < 100)
                            {
                                strMaxSKCCode = strMaxSKCCode.Substring(0, 7) + (Convert.ToInt32(strMaxSKCCode.Substring(7, 2)) + 1).ToString("00");
                            }
                            else
                            {
                                strMaxSKCCode = "";
                                string strSQL = @"SELECT LEFT(ST1_StyleCode,LEN(ST1_StyleCode)-1) ST1_StyleCode
                                                FROM [B01_MDM].[ST1_Style]
                                                WHERE ST1_StyleCode LIKE '" + pstrParam0 + "%' AND ST1_DIVI='" + pstrParam1 + "'";
                                DataTable dt = m_conn.GetDataTable(strSQL);
                                for (int i = 1; i < 100; i++)
                                {
                                    DataRow[] dr = dt.Select("ST1_StyleCode='" + pstrParam0 + i.ToString("00") + "'");
                                    if (dr.Length == 0)
                                    {
                                        strMaxSKCCode = pstrParam0 + i.ToString("00");
                                        break;
                                    }
                                }
                                if (strMaxSKCCode == "")
                                {
                                    strMaxSKCCode = "流水码超过最大值";
                                }
                            }
                        }
                ...
                else if (pstrCodeType == "新增退仓单号")
                {
                    ...
                    string strMaxBillCode = m_conn.GetDataTableFirstValue(@"
                                        ...
                                        WHERE SR1_CONO = 'HYFG' AND SR1_DIVI = '" + pstrParam0 + "' AND SR1_ShopRetreatBillCode LIKE '%" + strBill + "%'").ToString();
