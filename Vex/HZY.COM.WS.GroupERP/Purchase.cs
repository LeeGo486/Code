...
namespace HZY.COM.WS.GroupERP
{
    public class Purchase : HZYLogicBase, ILogicGetData
    {
        ...
        private void GET()
        {
            ...
            string strSQL = @"
                                    SELECT*FROM (SELECT  t1.PO1_ID ,
                                                ...
                                                LEFT JOIN B01_MDM.SP1_Supplier t2 WITH(NOLOCK)  ON t1.PO1_SupplierCode =t2.SP1_SupplierCode AND t1.PO1_DIVI=t2.SP1_DIVI
                                                LEFT JOIN B01_MDM.BR1_Brand t3 WITH(NOLOCK) ON t1.PO1_BR1_BrandCode = t3.BR1_BrandCode AND t1.PO1_DIVI=t3.BR1_DIVI) t9
                                           where 1=1 and PO1_LState <> '99'  " + m_hzyPageInfo.Where;
            //如果前台传过来季节或者年份，传到数据库SQL语句添加条件
            if (strSeason != "")
            {
                strSQL += " AND EXISTS (SELECT 1  FROM [B01_MDM].ST1_Style WITH(NOLOCK) WHERE ST1_Season=@param0 )";
            }

            if (strYear != "")
            {
                strSQL += " AND EXISTS (SELECT 1  FROM [B01_MDM].ST1_Style WITH(NOLOCK) WHERE ST1_Years=@param1 )";
            }
            ...
            string strSQL = @"
                                     SELECT t3.ST2_SizeGroup ,
                                        ...
                                        FROM   [B02_BILL].[PO2_PODetail] t1 WITH(NOLOCK)
                                            LEFT JOIN [B01_MDM].[ST3_StyleColorSize] t2 WITH(NOLOCK) ON t1.PO2_PD1_ProductCode = t2.ST3_SKUCode AND t1.PO2_DIVI=t2.ST3_DIVI
                                            LEFT JOIN [B01_MDM].[ST2_StyleColor] t3 WITH(NOLOCK) ON t2.ST3_ST2_SKCCode = t3.ST2_SKCCode AND t2.ST3_DIVI=t3.ST2_DIVI
                                            LEFT JOIN [M01_CONF].[CT0_CSYTAB] t4 WITH(NOLOCK) ON t3.ST2_Color = t4.CT1_KeyID AND t4.CT1_DIVI=t3.ST2_DIVI
                                        WHERE  1 = 1
        ...
        {
            string strSQL = @"SELECT * into #TEMP FROM (
                                     SELECT t3.ST2_SizeGroup ,
                                            t2.ST3_ST2_SKCCode ,
                                            t2.ST3_Size ,
                                            t1.PO2_ID ,
                                            ...
                                            t3.ST2_ImagePath
                                        FROM   [B02_BILL].[PO2_PODetail] t1 WITH(NOLOCK)
                                            LEFT JOIN [B01_MDM].[ST3_StyleColorSize] t2 WITH(NOLOCK) ON t1.PO2_PD1_ProductCode = t2.ST3_SKUCode
                                            LEFT JOIN [B01_MDM].[ST2_StyleColor] t3 WITH(NOLOCK) ON t2.ST3_ST2_SKCCode = t3.ST2_SKCCode
                                            LEFT JOIN [M01_CONF].[CT0_CSYTAB] t4 WITH(NOLOCK) ON t3.ST2_Color = t4.CT1_KeyID AND t4.CT1_DIVI=t3.ST2_DIVI
                                        WHERE  1 = 1
        ...
        private void EDIT()
        {
            ...
            else
            {
                ...
                ds.Tables["List"].Rows[0]["PO1_LmUser"] = hzyMessage.User_Name;
                ds.Tables["List"].Rows[0]["PO1_LmDt"] = timeNow;
                bResult = m_conn.Update("[B02_BILL].[PO1_POHead]", ds.Tables["List"], "PO1_POCode='" + strPO1_POCode + "' AND PO1_DIVI='" + strDIVI + "'");
            }
        ...
        private void EDITDETAIL()
        {
                ...
                for (int i = 0; i < dtPoDetailOptype.Rows.Count; i++)
                {
                    ...
                    if (dtPoDetailOptype.Rows[i][0].ToString().ToUpper() == "INSERT")
                    {
                        ...
                        for (int k = 0; k < dtOrderPoDetailListForInsert.Tables["List"].Rows.Count; k++)
                        {
                            ...
                            dtOrderPoDetailListForInsert.Tables["List"].Rows[k]["PO2_RgUser"] = hzyMessage.User_Name;
                            dtOrderPoDetailListForInsert.Tables["List"].Rows[k]["po2_lmuser"] = hzyMessage.User_Name;
                    ...
                    else if (dtPoDetailOptype.Rows[i][0].ToString().ToUpper() == "UPDATE")
                    {
                        ...
                        for (int k = 0; k < dtPoDetailListForUpdate.Tables[i + 1].Rows.Count; k++)
                        {
                            ...
                            dtPoDetailListForUpdate.Tables[i + 1].Rows[k]["po2_lmuser"] = hzyMessage.User_Name;
            ...
            if (bResult)
            {
                ...
                string strUptSQL = @"
                                    UPDATE B02_BILL.PO1_POHead SET PO1_PZNumber=TOTALNUM,PO1_OCurrencySums=TOTALMONEY,PO1_DCurrencySums=TOTALAMOUNT FROM 
                                        (SELECT PO2_POCode,PO2_DIVI,SUM(ISNULL(PO2_Number,0)) TOTALNUM,SUM(ISNULL(PO2_OCurrencyAmount,0)) TOTALMONEY, SUM(ISNULL(PO2_DCurrencyAmount,0)) TOTALAMOUNT
                                        FROM [B02_BILL].[PO2_PODetail] WITH(NOLOCK)
            ...
            for (int i = 0; i < dtAPOOptype.Rows.Count; i++)
            {
                if (dtAPOOptype.Rows[i][0].ToString().ToUpper() == "INSERT")
                {
                    ...
                    for (int l = 0; l < dsAPO.Tables[i + 1].Rows.Count; l++)
                    {
                        ...
                        m_conn.ExcuteQuerryByTran(strSQLD, new string[] { 
                                ...
                                strAp2_State,strAp1_Aplandt,timeNow.ToString(),hzyMessage.User_Name,
                    ...
                    drAPOhead["AP1_RgUser"] = hzyMessage.User_Name;
                    drAPOhead["AP1_LmDt"] = timeNow;
                    drAPOhead["AP1_LmUser"] = hzyMessage.User_Name;
                ...
                else if (dtAPOOptype.Rows[i][0].ToString().ToUpper() == "UPDATE")
                {
                    ...
                    for (int l = 0; l < dsAPO.Tables[i + 1].Rows.Count; l++)
                    {
                        ...
                        drAPOH["ap2_lmuser"] = hzyMessage.User_Name;
                    ...
                    dr["ap1_lmdt"] = timeNow;
                    dr["ap1_lmuser"] = hzyMessage.User_Name;
        ...
        private void EditAPODetail()
        {
            ...
            for (int i = 0; i < dList.Rows.Count; i++)
            {
                ...
                dr["AP2_RgDt"] = timeNow;
                dr["AP2_RgUser"] = m_hzyMessage.User_Name;
                dr["AP2_LmDt"] = timeNow;
                dr["AP2_LmUser"] = m_hzyMessage.User_Name;
                dr["AP2_UptNo"] = "0";
            ...
            if (bResult)
            {
                ...
                if (rcount < 1)
                {
                    ...
                    dr1["AP1_RgUser"] = m_hzyMessage.User_Name;
                    dr1["AP1_LmDt"] = timeNow;
                    dr1["AP1_LmUser"] = m_hzyMessage.User_Name;
                ...
                else
                {
                    ...
                    if (dtResult.Rows.Count == 1)
                    {
                        ...
                        uptdr["AP1_LmUser"] = m_hzyMessage.User_Name;
            ...
            string strSQL = @"
                                SELECT * FROM 
                                    (SELECT  TOP 1000
                                        ...
                                        t1.ST1_BrandID,
								        t4.BR1_Name
                                    FROM  [B01_MDM].[ST3_StyleColorSize] t3 WITH(NOLOCK)
								        LEFT JOIN [B01_MDM].[ST2_StyleColor] t2 WITH(NOLOCK) ON t3.ST3_ST2_SKCCode=t2.ST2_SKCCode  
                                        LEFT JOIN [B01_MDM].[ST1_Style] t1 WITH(NOLOCK) ON t1.ST1_StyleCode = t2.ST2_ST1_StyleCode 
								        LEFT JOIN [B01_MDM].[BR1_Brand] t4 WITH(NOLOCK) ON t4.BR1_BrandCode = t1.ST1_BR1_BrandCode AND t4.BR1_DIVI=t1.ST1_DIVI
                                    WHERE t3.ST3_State='20' AND t2.ST2_State='20' " + m_hzyPageInfo.Where + " ) T ";
            ds_Return = m_conn.GetDataSetForPageList(strSQL, new string[0], m_hzyPageInfo);
            //ds_Return = m_conn.GetDataSetForPageList(strSQL, new string[0], m_hzyPageInfo);
        ...
        private void GETAPOLIST()
        {
            ...
            string strSQL = @"            
                                ...
                                INTO #TEMP_PO
                                FROM B02_BILL.PO2_PODetail WITH(NOLOCK) WHERE PO2_POCode=@Param0 AND PO2_CONO='HYFG' AND PO2_DIVI=@Param1

                                SELECT A.AP2_APCode,AP2_LINumber,AP2_LSNumber,AP2_APlanNumber,AP2_PD1_ProductCode
                                        ,Convert(varchar(10),A.AP2_APlanDt,120) AP2_APlanDt,Convert(varchar(10),B.AP1_APlanDt,120) AP1_APlanDt
                                        ,AP1_RgDt,AP1_RgUser,B.AP1_PO1_PurchaseCode,A.AP2_ID,A.AP2_State
                                INTO #temp_AP
                                FROM B02_BILL.AP2_APODetail A WITH(NOLOCK),  B02_BILL.AP1_APOHead B WITH(NOLOCK)
                                ...
                                SELECT TS1_PD1_ProductCode,TS1_LINumber RP2_LINumber,TS1_LSNumber RP2_LSNumber,SUM(TS1_TransNumber) RPNumberSUMS
                                INTO #TEMP_RP
                                FROM B03_TRANS.TS1_Transaction WITH(NOLOCK)
                                WHERE TS1_TransType='PPS' AND TS1_AP1_APCode IN(SELECT DISTINCT AP2_APCode FROM #temp_AP) 
                                GROUP BY TS1_PD1_ProductCode,TS1_LINumber,TS1_LSNumber
                                ...
                                REPLACE(E.ST3_ST2_SKCCode,E.ST3_ST1_StyleCode,'') COLORID,B.APlanNumberSums,ISNULL(C.RPNumberSUMS,0) RPNumberSUMS,D.PO2_Number,ST2_Color,D.PO2_State,D.PO2_OCurrencyPrice,D.PO2_DCurrencyPrice,CAST(A.AP2_APlanNumber*D.PO2_OCurrencyPrice AS FLOAT) AP2_AMOUNT
                                FROM #temp_AP A
        ...
        private void DELETEPOHEAD()
        {
            ...
            dr["PO1_LmDt"] = timeNow;
            dr["PO1_LmUser"] = hzyMessage.User_Name;
        ...
        private void ENDPOHEAD()
        {
            ...
            drDetail["PO2_LmUser"] = hzyMessage.User_Name;
            ...
            bResult = m_conn.Update("[B02_BILL].[PO2_PODetail]", dtEndPoDetail, "PO2_POCode='" + strPO1_POCode + "' AND PO2_DIVI='" + strDIVI + "'");
            m_conn.CommitTransaction();
        ...
        private void EXAMINEPODETAIL()
        {
            ...
            for (int l = 0; l < ds.Tables["list"].Rows.Count; l++)
            {
                ...
                dr["po2_state"] = "35";
                dr["PO2_LmDt"] = timeNow;
                dr["PO2_LmUser"] = hzyMessage.User_Name;
        ...
        private void ENDAPODTETAIL()
        {
            ...
            for (int l = 0; l < ds.Tables["list"].Rows.Count; l++)
            {
                ...
                dr["AP2_State"] = "75";
                dr["AP2_LmDt"] = timeNow;
                dr["AP2_LmUser"] = hzyMessage.User_Name;
        ...
        private void GETAPOHEAD()
        {
            string strSQL = @"    
                                ...
                                FROM    ( SELECT    a1.* ,
                                                    ...
                                                    a2.PO1_RgDt ,
                                                    a2.PO1_RgUser,
                                                    CAST(AP1_AMOUNT AS FLOAT) AP1_AMOUNT
                                          FROM      [Hawk].[B02_BILL].[AP1_APOHead] a1 WITH ( NOLOCK )
                                                    JOIN [Hawk].[B02_BILL].[PO1_POHead] a2 WITH ( NOLOCK ) ON a1.AP1_PO1_PurchaseCode = a2.PO1_POCode AND A1.AP1_DIVI=A2.PO1_DIVI
                                                    JOIN  ( SELECT AP2_CONO,AP2_DIVI,AP2_APCode,SUM(AP2_APlanNumber*PO2_OCurrencyPrice) AP1_AMOUNT 
							                                FROM B02_BILL.AP2_APODetail WITH(NOLOCK) 
                                                            JOIN B02_BILL.AP1_APOHead WITH(NOLOCK) ON AP1_CONO=AP2_CONO AND AP1_DIVI=AP2_DIVI AND AP1_APCode=AP2_APCode
								                            JOIN B02_BILL.PO2_PODetail WITH(NOLOCK) ON AP2_DIVI=PO2_DIVI AND AP2_LINumber=PO2_LISeq AND AP2_LSNumber=PO2_LSSeq 
                                                                AND AP2_PD1_ProductCode=PO2_PD1_ProductCode AND AP1_PO1_PurchaseCode=PO2_POCode
							                                GROUP BY AP2_CONO,AP2_DIVI,AP2_APCode) T ON A1.AP1_APCode=T.AP2_APCode AND A1.AP1_CONO=T.AP2_CONO AND A1.AP1_DIVI=T.AP2_DIVI 
                                        ) b1
                                        LEFT JOIN (--获取已入库总数
                                                    SELECT TS1_DIVI RP1_DIVI,TS1_AP1_APCode RP1_AP1_APCode,SUM(TS1_TransNumber) RP2_Number_Total 
					                                FROM B03_TRANS.TS1_Transaction WITH(NOLOCK)
					                                WHERE TS1_TransType='PPS'
					                                GROUP BY TS1_DIVI,TS1_AP1_APCode) b2 ON b1.AP1_APCode = b2.RP1_AP1_APCode AND b1.AP1_DIVI=b2.RP1_DIVI
        ...
        private void GETPODDETAIL()
        {
            ...
            string strSQL = @"
                                SELECT [PO2_ID]
                                      ...
                                      ,@param0 ap2_aplandt
                                      ,TOTAL
                                  FROM [Hawk].[B02_BILL].[PO2_PODetail] t1 with(nolock)
                                  JOIN (
		                                  SELECT ST3_SKUCode,ST2_Color,ST3_ST1_StyleCode,ST3_Size,ST2_PurchasePrice
		                                  FROM [Hawk].[B01_MDM].ST2_StyleColor t2 with(nolock)
			                                JOIN [Hawk].[B01_MDM].ST3_StyleColorSize t3 with(nolock) ON t2.ST2_SKCCode=t3.ST3_ST2_SKCCode) t4 ON t1.PO2_PD1_ProductCode=t4.ST3_SKUCode 
                                  LEFT JOIN (  SELECT AP1_PO1_PurchaseCode,AP1_DIVI,AP1_CONO,AP2_PD1_ProductCode,AP2_LINumber,AP2_LSNumber,SUM(ISNULL(AP2_APlanNumber,0)) TOTAL
                                                  FROM B02_BILL.AP2_APODetail with(nolock) LEFT JOIN B02_BILL.AP1_APOHead with(nolock) ON AP2_APCode=AP1_APCode AND AP2_DIVI=AP1_DIVI AND AP2_CONO=AP1_CONO 
                                                  WHERE AP2_State>='35'
                                                  GROUP BY AP1_PO1_PurchaseCode,AP1_DIVI,AP1_CONO,AP2_PD1_ProductCode,AP2_LINumber,AP2_LSNumber) T5 
                                                    ON T5.AP1_PO1_PurchaseCode=T1.PO2_POCode AND T5.AP2_PD1_ProductCode=T1.PO2_PD1_ProductCode AND T5.AP2_LINumber=T1.PO2_LISeq AND T5.AP2_LSNumber=T1.PO2_LSSeq 
